...
export interface SprintStatus {
  ...
  // Done and Superseded are kept DISTINCT (INV-C1-7) — never collapsed into one "done" bucket.
  counts: { done: number; superseded: number; inProgress: number; qa: number; planned: number; total: number };
...
export function deriveSprintStatus(sprint: ScenarioUnit, idx: ScenarioIndex): SprintStatus {
  ...
  const counts = { done: 0, superseded: 0, inProgress: 0, qa: 0, planned: 0, total: 0 };
  ...
  const terminalAll = counts.total > 0 && counts.done + counts.superseded === counts.total;
...
export function resolveSprintPin(idx: ScenarioIndex): SprintPin {
  ...
  const rows = sprints
    .map((s) => {
      ...
      return { uuid: String((s.model as any).uuid), num: sprintNumOf(s), name: String((s.model as any).name || ''), st };
    })
    .filter((r): r is { uuid: string; num: number; name: string; st: SprintStatus } => r.num != null);
