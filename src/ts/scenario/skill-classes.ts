...
export class Chain {
  ...
  /** taskChain.updateMatrixRow — update one row in the traceability matrix */
  updateMatrixRow(reqUuid: string, matrixPath: string): { updated: boolean; row: string } {
    const result = this.followUp([reqUuid]);
  ...
  /** taskChain.generateMatrix — regenerate the full traceability matrix from canonical data */
  generateMatrix(matrixPath: string, sprint?: string): { rows: number; complete: number; total: number; excluded: number } {
    const reqUuids = sprint ? this.filterBySprint(sprint) : [];
    const result = this.followUp(reqUuids);

    const lines = [
      '# RawBin Traceability Matrix',
      '',
      `> Generated by \`taskChain generateMatrix\` — canonical source: Chain.followUp()`,
