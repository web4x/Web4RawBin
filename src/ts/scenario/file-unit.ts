...
// Sprint 41 (T41.1): the fully-qualified IOR composition + GUARD#2 (empty-origin throw) is OWNED by the `Ior`
// value-object; the File/Folder CLASSES ask it (file.ownIor()/folder.ownIor()). The old free-fn composeUnitIor was
// DELETED (its knowledge collapsed INTO Ior — no shim/facade, PO no-half-state ruling). This factory ASKS the object.
import { File as FileClass } from './file.js';
import { Folder as FolderClass } from './folder.js';

export interface FileUnitInput {
  name: string;
  origin?: string;                // Sprint 41: the server's canonical origin (selfHost) → the File/Folder object composes a fully-qualified selfIor (ownIor). Absent = legacy bare create UNCHANGED (zero-migration).
  ...
  parent?: string | null;         // T37.21 folder nesting: the parent folder unit's ior (null = top-level under Files)
  mimeType?: string;
...
export function createFileUnit(idx: ScenarioIndex, input: FileUnitInput, publish: PublishFn): ScenarioUnit {
  ...
  // room files dir beside the file siblings. The ONE become-a-room-unit path taking a folder (path changed to accept it, per
  // Tron — NOT a folder-shaped copy). Caller keeps the real mkdir + calls room.addFileUnit(uuid), exactly as a file upload does.
  // RETIRES the MODEL_STORE folder mint (that second representation was the actual bug). parent = the nesting parent's ior (null = top-level).
  if (input.kind === 'folder') {
    ...
    if (input.roomUuid && (input.fsKey || input.uploaderToken)) {
      ...
      unitLinks.push(path.join('..', 'data', 'users', fsKey, 'rooms', input.roomUuid, 'files', uuid + '.scenario.json')); // room symlink (same convention as a file)
    }
    if (input.extraUnitLinks) unitLinks.push(...input.extraUnitLinks);
    const folderUnit: ScenarioUnit = {
      ior: iorClass('Folder'),
      model: { uuid, name: input.name, kind: 'folder', location: input.location || '', children: [], parent: input.parent ?? null, unitLinks, ...(input.origin ? { selfIor: new FolderClass({ uuid, origin: input.origin }).ownIor() } : {}) }, // Sprint 41: clean path (origin supplied) asks the Folder object for its fully-qualified selfIor; legacy (no origin) UNCHANGED
  ...
  const unit: ScenarioUnit = FileLoader.create({
    ...
    model: {
      ...
      ...(input.origin ? { selfIor: new FileClass({ uuid, origin: input.origin }).ownIor() } : {}), // Sprint 41: clean path (origin supplied) asks the File object for its fully-qualified selfIor; legacy (no origin) UNCHANGED (zero-migration)
    },
    ownerIor: input.roomUuid ? iorInstance(input.roomUuid) : (input.uploaderToken ? iorInstance(input.uploaderToken) : null), // Sprint 41 OWNER-DEFAULT: owner from the minting context — roomUuid→room, else the acting identity (uploaderToken; Tron as root when he acts). Ownerless-impossible for a real create; guard #1 backstops the residual null.
  });
