...
function mofChildren(idx: ScenarioIndex, uuid: string): MofNode[] | null {
  ...
  if (uuid === 'rawbin:ts' || uuid.startsWith('dir:')) { // R33.10: ts/ = the FULL src/ directory tree (ALL 123 .ts + folder hierarchy), not just the ~25 files with generated M1 elements (INV-T1/T2 completeness+folders)
    ...
    const dirRel = uuid === 'rawbin:ts' ? rawbinToRepoDir('rawbin:ts') : uuid.slice('dir:'.length); // R37.33/Option-B: rawbin:ts→'src' via the ONE rawbinToRepoDir source (shared with resolveFolderRefToDir); dir: children are dir:src/… → uuid.slice gives 'src/…' by construction
...
// [impl:uuid:8c6c7f69-ae21-47a2-88a6-244bd5c3da2f] chat lazy-load (split for server.ucScopedMethodResolve)
    ...
    if (req.method === 'POST' && filepath.startsWith('/api/room/') && filepath.endsWith('/folder')) { // T37.21 defect-2: room Files add-folder
      const roomId = filepath.split('/')[3];
      let fbody = '';
      req.on('data', (chunk: Buffer) => { fbody += chunk; });
      req.on('end', () => {
        try {
          const { name, nestedPath, playerToken } = JSON.parse(fbody || '{}');
          // AUTH mirrors the upload route: a live player token (tokenToClient) — member liveness, the same gate room writes already use.
    ...
    if ((req.method === 'POST' || req.method === 'PUT') && filepath.startsWith('/api/room/') && filepath.endsWith('/upload')) { // SLICE-A: PUT = idempotent unit-JSON ingress; POST = native-file multipart edge (both hit this handler)
      ...
      req.on('end', async () => {
        try {
          ...
          // R40.90 BOUNDARY-EXTRACTION HARDENING — the whole fix (tester r4090 root-isolation, architect 401b92d09). iOS Safari
          // QUOTES the boundary (boundary="----X") or appends params (; charset=…); the old `.split('boundary=')[1]` kept the
          // quote/param → '--'+boundary never matched the raw body delimiter → 0 parts → token+file unparsed → 401 (Tron, a week).
          // Dequote + strip trailing ;params + trim. The BODY split below (body.toString('binary').split) is DELIBERATELY UNCHANGED:
          // the tester proved it binary-safe + order-independent (V1 file-first + V2 token-first binary both → exact 473521b) — a
    ...
    if (filepath.startsWith('/api/trace/children/')) {
      ...
      try {
        ...
        // T37.21 defect-2: rcKind may be 'members', 'files' (root) OR 'files/<nested>' (a folder created under Files) → the
        // Files node must surface real nested folders too, else an Add-folder'd folder is invisible (never had a button before).
        if (rcRoom && (rcKind === 'members' || rcKind === 'files' || rcKind.startsWith('files/'))) {
          ...
          if (rcKind === 'members') {
            const arr = Array.isArray(rmodel.members) ? rmodel.members as any[] : [];
            const kids = arr.map((m: any) => ({ uuid: String(m.ior || m.uuid || m.token || '').replace('ior:instance:', ''), type: 'Member', name: String(m.name || '?'), description: String(m.status || m.role || ''), hasChildren: false }));
            res.writeHead(200, { 'Content-Type': 'application/json', 'Cache-Control': 'no-cache' });
...
function handleMessage(clientId: string, ws: WebSocket, msg: any): void {
  ...
  switch (msg.type) {
    ...
    case MSG.DELETE_ROOM: {
      ...
      delRoom.deleted = true; // R40.106 INC-7b: STOP all further persists on this room BEFORE composite/teardown — else member-leave/removeRoom re-persist RESURRECTS the scenario unit the composite deletes (bite-3/4 root; architect deleted-flag fix)
