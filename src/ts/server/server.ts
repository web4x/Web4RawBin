  ...
  try {
    ...
    if (filepath.startsWith('/md/') && filepath.endsWith('/')) {
      ...
      try {
        ...
        const scenarioLink = (e: any) => { if (!relPath.startsWith('scenario/sprints.md/') || !e.name.endsWith('.md')) return ''; const slug = e.name.replace('.md', ''); const sjDir = path.join(PROJECT_ROOT, 'scenario', 'sprints.json'); try { for (const sp of fsSync.readdirSync(sjDir)) { const spDir = path.join(sjDir, sp); let jp = path.join(spDir, `${slug}.json`); if (fsSync.existsSync(jp)) return ` <a href="/edit/scenario/sprints.json/${sp}/${slug}.json" style="text-decoration:none;font-size:0.8em" title="Scenario JSON">🔗</a>`; try { for (const cd of fsSync.readdirSync(spDir).filter((d: string) => { try { return fsSync.statSync(path.join(spDir, d)).isDirectory(); } catch { return false; } })) { jp = path.join(spDir, cd, `${slug}.json`); if (fsSync.existsSync(jp)) return ` <a href="/edit/scenario/sprints.json/${sp}/${cd}/${slug}.json" style="text-decoration:none;font-size:0.8em" title="Scenario JSON">🔗</a>`; } } catch {} } } catch {} return ''; };
        const breadcrumb = (rel: string): string => { if (!rel) return '<h1>/</h1>'; const parts = rel.replace(/\/$/, '').split('/'); const crumbs: string[] = []; for (let i = 0; i < parts.length; i++) { const href = '/md/' + parts.slice(0, i + 1).join('/') + '/'; const isLast = i === parts.length - 1; if (isLast) { crumbs.push(`<span>${parts[i]}</span>`); } else { crumbs.push(`<a href="${href}" style="color:#667eea;text-decoration:none">${parts[i]}</a>`); } } return `<h1 style="font-size:1.1rem">${crumbs.join('<span style="color:#555;margin:0 2px">/</span>')}</h1>`; };
