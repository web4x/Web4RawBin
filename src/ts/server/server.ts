  ...
  try {
    ...
    if (filepath.startsWith('/md/') && filepath.endsWith('/')) {
      ...
      try {
        ...
        const scenarioLink = (e: any) => { if (!relPath.startsWith('scenario/sprints.md/') || !e.name.endsWith('.md')) return ''; const slug = e.name.replace('.md', ''); const sjDir = path.join(PROJECT_ROOT, 'scenario', 'sprints.json'); try { for (const sp of fsSync.readdirSync(sjDir)) { const jp = path.join(sjDir, sp, `${slug}.json`); if (fsSync.existsSync(jp)) return ` <a href="/edit/scenario/sprints.json/${sp}/${slug}.json" style="text-decoration:none;font-size:0.8em" title="Scenario JSON">🔗</a>`; } } catch {} return ''; };
