...
import { loadRevokedTokens, isRevoked, EXPECTED_REVOKED_COUNT } from './revoked-tokens.js';
...
const REVOKED_TOKENS_PATH = path.join(DATA_DIR, 'revoked-tokens.json');
...
// console.log (not addLog) — serverLogs is declared later; addLog here would hit its TDZ at boot.
console.log(`[boot] revoked-tokens: loaded ${revokedTokens.size} (expected ${EXPECTED_REVOKED_COUNT} when armed; 0 = fail-open, not armed)`);
