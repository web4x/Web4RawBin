...
import { statusNext } from '../scenario/task-policy.js'; // C4.3: approve DELEGATES the Done-advance to the ONE controller (single Done-writer). ★ Importing this WIRES the C4 controller + C4.1 self-heal at boot → go-live is on the (HELD) restart; build-not-deploy while Tron mid-batch on his verdicts.
...
// R40.22 step-3: FROZEN list of the 116 auth-invalidated dormant dev/test Device tokens, stored as SALTED
// HASHES (not raw tokens) so it is TRACKED in git (deploy-durable) without being a credential. Loaded ONCE
// at boot (fail-open if absent); the running server never re-derives it (can't silently widen). Reversible.
// Generated by scripts/gen-revoked-tokens.ts; guarded by scripts/check-revoked-tokens.ts.
const REVOKED_TOKENS_PATH = path.join(__dirname, '../../../revoked-token-hashes.json');
let revokedHealthError: string | null = null; // (B) LOUD-ABSENCE: set at boot when ARMED but the list is absent/short → /api/health reports unhealthy.
...
revokedTokens = loadRevokedTokens(REVOKED_TOKENS_PATH);
// (B) LOUD-ABSENCE: fail-open for an UNLISTED token (correct), but a MISSING/EMPTY/SHORT list WHILE ARMED
// is LOUD — else the revocation silently evaporates on a fresh deploy. console.* (not addLog — serverLogs'
// TDZ is later). When unhealthy we record revokedHealthError so /api/health refuses to report healthy.
revokedHealthError = revokedArmedHealth(revokedTokens.size);
if (revokedHealthError) {
  console.error(`[boot][UNHEALTHY] ${revokedHealthError}`);
} else {
  console.log(`[boot] revoked-tokens: loaded ${revokedTokens.size} (armed=${REVOKED_ARMED}; expected ${EXPECTED_REVOKED_COUNT} when armed; unlisted token = fail-open)`);
}
  ...
  try {
    ...
    if (filepath === '/server-manager' || filepath.startsWith('/api/server-manager/') || filepath.startsWith('/api/unit/')) {
      ...
      if (req.method === 'GET' && filepath === '/api/server-manager/tree') { // R31.3/R31.4 read-only otmux tree
        ...
        // R40.11 slice-2: ONE shared emitter (OtmuxBridge.buildRootedTree) builds the re-rooted tree —
        // the inline duplicate that used to live here is removed. The session subtree is byte-identical
        // (buildRootedTree was extracted from this block); only the deployment-ref rows change: they now
        // carry the REAL minted typed-unit iors (resolved by sourceRole ∈ the node's deploymentRef roles),
        // NOT the synthetic 'depref:'+role that missed in the drawer. Fail-open (missing node / error) is
        // inside buildRootedTree. NOTE: resolve-by-scan of the index (5 typed units); tree route is
        // owner-open, not hot-path — a future optimisation could link the node to its unit uuids directly.
        const NODE_UUID = 'fc327458-03d1-4b90-847d-ab52a7d82237';
        let roots: any[];
        try {
          const treeIdx = new ScenarioIndex(path.join(__dirname, '../../../scenario/index'));
          const nodeUnit = treeIdx.get(NODE_UUID);
          const roles = new Set<string>(((nodeUnit?.model as any)?.deploymentRefs || []).map((d: any) => String(d.role)));
          const typedUnits: { uuid: string; name: string; m2Type?: string }[] = [];
          if (roles.size) {
            for (const key of treeIdx.list()) {
              const m: any = treeIdx.get(key)?.model;
              if (m && typeof m.sourceRole === 'string' && roles.has(m.sourceRole) && typeof m.uuid === 'string')
                typedUnits.push({ uuid: String(m.uuid), name: String(m.name || m.uuid), m2Type: String(m.instanceOf || '') });
            }
            typedUnits.sort((a, b) => a.uuid.localeCompare(b.uuid)); // deterministic row order
          }
          roots = OtmuxBridge.buildRootedTree(sessions, nodeUnit, NODE_UUID, typedUnits);
        } catch (e: any) {
          console.warn(`[server-manager] WARN: WODA.prod re-root failed (${e?.message || e}) → tree DEGRADED to flat session list (fail-open).`);
          roots = OtmuxBridge.buildRootedTree(sessions, null, NODE_UUID, []);
        }
        res.writeHead(200, { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' });
...
// [impl:uuid:8c6c7f69-ae21-47a2-88a6-244bd5c3da2f] chat lazy-load (split for server.ucScopedMethodResolve)
    ...
    if (filepath === '/api/health') {
      ...
      const base = { uptime, version: getVersion(), connections: wsClients.size, rooms: roomManager.size, revoked: { armed: REVOKED_ARMED, loaded: revokedTokens.size }, storageRekey: { applied: REKEY_APPLIED, mapSize: Object.keys(storageMap.map).length } };
      const healthErr = revokedHealthError || storageMapHealthError;
      if (healthErr) {
        res.writeHead(503, { 'Content-Type': 'application/json', 'Cache-Control': 'no-cache' });
        res.end(JSON.stringify({ status: 'unhealthy', reason: healthErr, ...base }));
        return;
      }
      res.writeHead(200, { 'Content-Type': 'application/json', 'Cache-Control': 'no-cache' });
