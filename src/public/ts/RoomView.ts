...
export class RoomView {
  ...
  private openRoomEditor(): void {
    ...
    modal.querySelectorAll(".re-option").forEach((row) => { // R31.12 belt (WebKit-reliable): tapping the padded row sets its radio + fires change — guards a flaky native label-tap on iOS; skips disabled (read-only non-host)
      ...
      row.addEventListener("click", set); row.addEventListener("pointerup", set);
    });
