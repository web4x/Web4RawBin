...
export class RoomView {
  ...
  private openRoomEditor(): void {
    ...
    modal.innerHTML = `<h3 style="margin-bottom:16px">${isHost ? 'Edit ' : ''}Room Settings</h3><label style="display:block;margin-bottom:12px">Name<br><input id="re-name" type="text" style="width:100%;padding:8px;margin-top:4px;background:rgba(255,255,255,0.1);color:#fff;border:1px solid rgba(255,255,255,0.2);border-radius:6px"></label><fieldset style="border:none;padding:0;margin-bottom:12px"><legend>Visibility</legend><label class="re-option" style="display:block;padding:10px 8px;cursor:pointer"><input type="radio" name="re-vis" value="public" ${v==="public"?"checked":""}> Public</label><label class="re-option" style="display:block;padding:10px 8px;cursor:pointer"><input type="radio" name="re-vis" value="by-invite" ${v==="by-invite"?"checked":""}> By-Invite</label><label class="re-option" style="display:block;padding:10px 8px;cursor:pointer"><input type="radio" name="re-vis" value="private" ${v==="private"?"checked":""}> Private</label></fieldset><fieldset style="border:none;padding:0;margin-bottom:16px"><legend>Mode</legend><label class="re-option" style="display:block;padding:10px 8px;cursor:pointer"><input type="radio" name="re-mode" value="live" ${m==="live"?"checked":""}> Live</label><label class="re-option" style="display:block;padding:10px 8px;cursor:pointer"><input type="radio" name="re-mode" value="persistent" ${m==="persistent"?"checked":""}> Persistent (default)</label></fieldset><div style="display:flex;gap:8px;justify-content:flex-end"><button id="re-cancel" class="btn btn-secondary">Cancel</button><button id="re-save" class="btn btn-primary">Save</button></div>`;
    ...
    modal.querySelectorAll(".re-option").forEach((row) => { // R31.12 belt DE-DUPED: pointerup-ONLY + preventDefault → exactly ONE selection/tap (was click+pointerup = 2× fire → flicker-no-stick); preventDefault kills the native synthetic-click double; set() once (r.checked auto-unchecks siblings); skips disabled (read-only)
      const set = () => { const r = row.querySelector("input[type=radio]") as HTMLInputElement | null; if (r && !r.disabled) { r.checked = true; r.dispatchEvent(new Event("change", { bubbles: true })); } };
      row.addEventListener("pointerup", (e) => { e.preventDefault(); set(); });
    });
