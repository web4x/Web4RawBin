const CACHE_NAME = 'rawbin-v0.8.105';
...
const STATIC_SHELL = [
  ...
  '/icon-512.png',
  '/trace',
  '/dist/trace-page-BMN2Q4QM.js',
  '/scenario',
  '/dist/scenario-view-SLAN5JMZ.js',
  '/dist/app-YCUKVPIT.js',
];
...
async function networkFirst(request) {
  try {
    const response = await fetch(request);
    if (response.ok && request.method === 'GET') { // BUG C: Cache.put rejects non-GET (POST → TypeError); only GET is cacheable
      const cache = await caches.open(CACHE_NAME);
      cache.put(request, response.clone());
    }
    return response;
