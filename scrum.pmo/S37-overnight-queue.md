...
4. **rename** as a SECOND registered action [✓ req minted R40.104 `069b1f8d`; boarded **T37.42** `af84c07e`, owner expert, TIER-A-behind-Move]
   - ✓ **increment #3 (class-owns-its-name / derivedName) — GO GIVEN + BOARDED: req minted R40.105 `fc201015`, planner boarded Task 37.43 `c7f5f582` (owner expert; expert marks against fc201015)**. req RULED it needs its OWN requirement (measured NOT covered — R40.99 has no name-derivation AC; R37.15 is store-once, a different concern). It's the DERIVATION half (each class computes its display name: Image←capture, Email←subject, Contact←full-name, CalendarEntry←title+date, File←clean basename), extends R40.99, PAIRS R40.104 (derivedName←class computes / displayName←user overrides). ★ **NOT new scope — a SCENARIO-FIRST BACKFILL**: the expert is ALREADY building this (#3) because it was ordered before a req was routed = build ran AHEAD of the req (the orphaned-criteria pattern we cleaned up twice tonight). Minting makes in-flight work TRACEABLE; it adds ZERO work. **req mints scenario-first → planner boards increment #3** (slots between #2 Move and #4 rename). Governance held: req recommended + PO ranked, neither minted unilaterally.
