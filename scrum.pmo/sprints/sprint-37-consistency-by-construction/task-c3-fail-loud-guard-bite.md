...
R-C3 (build 3rd). A FAIL-LOUD guard folded into ci:gates FAILS the build when pin/board/files disagree: (a) pin != computed-current-from-files; (b) ANY sprint's planning.md/task-md/requirements.md != regenerated (round-trip byte-match — extend check:sprint-md to FAIL on drift, not just report); (c) a task-status in a unit != its board checkbox. The fail-loud is PROVEN by a real drift-injection BITE, NOT asserted.
