...
# Task C4.2: CONTROLLER single transition entry (statusNext) — guard + evidence-precondition + tick-checklist + DERIVE + persist + emit
...
- [ ] (functional) statusNext TICKS the CHECKLIST and lets deriveStatusEnum produce the status — it NEVER hand-sets the status enum (status stays DERIVED, R-C5).
