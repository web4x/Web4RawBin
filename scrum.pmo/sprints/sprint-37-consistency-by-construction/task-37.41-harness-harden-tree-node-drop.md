...
✓ PARTIAL (v0.8.207): the tester MACHINE-VERIFIED both halves of the drag-onto-tree — CLIENT (synthetic unit-ref drop fires reparentUnitsIntoContainer, stub-must-fail GREEN) + SERVER (resolveDropContainer resolves the 'folder:' target + re-parents). That let T37.20.4 return to QA-Review (caveat removed). ▶ REMAINING (this task's completion, NON-BLOCKING for T37.20.4 accept): the SINGLE END-TO-END run (one run exercising client->server together on a specific folder) — currently blocked by a WEBKIT TREE-RENDER FLAKE; handle the flake, then land the single e2e run. This is why the row was accepted as a composition-of-two-measured-halves (they meet at the same interface) rather than one gesture. --- PRIOR: ★★ NOW URGENT (PO 2026-09-06) — NOT a nicety: Tron tested the folder move on his iPHONE (v0.8.206) and the file does NOT MOVE on a real iOS TOUCH drag. This task exists precisely to catch THIS class, and it now GATES T37.20.4's return to QA-Review. ★ SCOPE EXPANDED: it must cover the TOUCH gesture (iOS touch-drag delivers the unit-ref payload to the tree-folder handler so reparentUnitsIntoContainer fires), NOT ONLY a synthetic desktop drag. If a real touch-drag is genuinely un-mockable in-harness (unlike the desktop synthetic drag), that is a finding to escalate — but attempt the touch-gesture machine-verification first. OWNER=TESTER (pull ahead of other TIER-A). On GREEN -> the expert's iOS-touch fix is proven + T37.20.4 returns to QA-Review. Covers R37.20 AC-tree-folder-drop-target-routes-contract. 0 Done till Tron.
