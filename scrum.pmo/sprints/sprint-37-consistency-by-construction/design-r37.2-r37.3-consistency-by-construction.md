...
## ★ WORKED-EXAMPLE PRESERVED (do NOT regenerate until R37.3 build)
The T37.20 task-md showing 6 ACs while the unit json has 7 is a LIVE `view != f(units)` drift, kept INTENTIONALLY as the clean worked-example for the R37.2/R37.3 build (req + architect agreed 2026-09-06). **Do NOT regenerate T37.20's task-md** (it would erase the example) until the R37.3 reconcile-all pass runs — at which point clearing it IS the demonstration that view==f(units) holds. If you are a rewound agent or the reconcile-all migration: this drift is a fixture, not an oversight; regenerate it as part of R37.3, not before.
