...
- [~] PART 2 ADD-FOLDER-PHYSICAL (Tron 'BOTH' 4-assert) — SPLIT (create-half green, client-half in-flight): ✅ CREATE-HALF GREEN: unit persisted + real filesystem DIRECTORY mkdir'd (correct path post R37.33 resolveDirRefAbs v0.8.165, the src/ts bug fixed) + confine traversal-rejected + WS FRAME reaches the passive 2nd browser; 12/12 ATOMICITY backstop incl a GENUINELY-EXERCISED rollback. ⏳ CLIENT-HALF = PENDING-IN-FLIGHT (the no-reload update [browser-1] + 2nd-browser-over-WS DOM live-insert): the tester's CLEAN re-run is RUNNING IN SCRATCH right now (post R37.33-fix + harness-fix), NOT YET VERDICTED — recorded as PENDING-IN-FLIGHT, NOT done and NOT failed. If it greens before Tron wakes -> flips to complete; if not, he sees EXACTLY ONE clearly-marked open item, not a surprise. [R40.70 542946c4]
