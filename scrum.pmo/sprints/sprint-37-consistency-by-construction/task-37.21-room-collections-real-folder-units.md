...
- [~] PART 2 ADD-FOLDER-PHYSICAL (Tron 'BOTH' 4-assert) — SPLIT (create-half green, client-half = the ONE clear open): ✅ CREATE-HALF GREEN: unit persisted + real filesystem DIRECTORY mkdir'd (correct path post R37.33 resolveDirRefAbs v0.8.165) + confine traversal-rejected + WS FRAME reaches the passive 2nd browser; 12/12 ATOMICITY backstop incl a GENUINELY-EXERCISED rollback. ❌ CLIENT-HALF = ONE CLEARLY-MARKED OPEN (verdict LANDED 2026-09-02 = tester CLEAN REAL-RED commit 5ec2323c7, proven in SCRATCH localhost:4643 NOT prod — expert correctly REFUSED to route around the prod credential guard; setup fully clean: both browsers render+subscribe dir:src/ts, create-200 unit c398d245 + real dir + confine OK): (2c-FRAME) b2 received WS frame=TRUE but (2c-DOM) b2 LIVE-INSERT=FALSE (Tron's exact ask: 2nd browser doesn't update on its own) + (2b) b1-shows=FALSE -> server broadcast+create WORK, the CLIENT doesn't consume the frame into a live-insert = a REAL piece-2 client bug. FIX PATH: expert fixes piece-2 -> tester re-runs -> GREEN -> tester stamps marker -> req mints P2 Test -> I verify chain-complete. NO Test on RED (#126). NOT done, NOT hidden = the ONE clear open Tron sees at QA-Review. [R40.70 542946c4]
...
- [x] ★ PART 4b SUNBURST-SIZE = ON-DISK BYTES: TESTER-GATED (verified 2026-09-01, tester first-hand not-relayed — commit c816ae635, gate test/visual/r4021c-room-files-sunburst-gate.mjs, DET-3x @390 real-WebKit): rendered .dv-sunburst arcs measured + tied to /api/trace/children on-disk sizes Tron sees (png 10916416B->largest arc, mp3->mid, doc 43B->sliver, max/min 7.58); equal-angle STUB->1.00->RED = able-to-fail. Arc size = REAL ON-DISK BYTES = Tron's corrected metric (NOT childCount). ★ Tron corrected the metric 2026-09-01 (childCount->bytes, req AC-B-sunburst-size-is-on-disk-bytes) — now SATISFIED at the rendered gate; the earlier childCount green did NOT hide it. ✓✓ CHAIN-COMPLETE-TO-TEST (I verified BOTH-DIR on disk 2026-09-01, not relayed): Impl a34f1a68 (renderChildSizeSunburst, sourceFile sunburst.ts) tests[]=[f805cfe0] <-> Test f805cfe0 (gate r4021c, markerProvenance=ADOPTED) implementations[]=[a34f1a68] + ownerIor; tester two-key GREEN. P4b FULLY CLOSED to the QA-Review CEILING. @390 TEAM-VERIFIED (rewordProvenance 2026-09-05, customer-not-tester): the tester's real-WebKit @390 DET-3x IS our verification; Tron ACCEPTS delivered verified work — @390 is NOT a Tron-run check.
- [x] PART 5a PUML-PHYSICAL-TREE STRUCTURE: tester-GATED (the four-distinct-dirs reveal). [R40.77 70c6b806]
- [x] PART 5b derived dir-folder SUNBURST: GREEN — proportional proven on DIR FOLDERS (ratio 3.59 across 9 arcs; PO-relayed 2026-09-02, confirming the tester marker for chain-complete). Was HELD on the sourceDirTree 'src' hardcode -> now UNBLOCKED+green by R37.33 resolveDirRefAbs v0.8.165. Non-blocking (not a Tron-5-ask). [R40.77]
