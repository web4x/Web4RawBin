# R-C6 — sprints.overview.md generated + preserved-narrative + G5 frozen-legacy (visible)

**Author:** robbin-architect · 2026-08-07 (S37, LAST S37 design). PO-dispatched after R-C1/R-C3. Completes the consistency-by-construction sprint: the ONE remaining hand-maintained board (`sprints.overview.md`) becomes generated-where-it-can-drift + preserved-where-it's-narrative, and the deliberate legacy scope-boundary is made VISIBLE. Design → req mints → expert builds → I backstop. Includes the R-C3 vacuous-BITE.

## MEASURED ground truth (disk, HEAD v0.8.65)
- **`sprints.overview.md`** = hand-authored narrative (Back-link, Team-Achievements, "Durable index…" prose, Status legend) + a durable **sprint index** (the drift-prone part — every sprint's status, maintained BY HAND by planner). NO `GENERATED_HEADER` → the OWNED-OUTPUT guard SKIPS it (never clobbered) = why R-C2 deferred it.
- **Generator mechanisms to reuse:** `GENERATED_HEADER` marker (generate-sprint-md.ts:29); OWNED-OUTPUT guard (:213-236, never clobbers a file lacking the header); `--check --all` walks files starting with `GENERATED_HEADER` (:265-273). All FILE-granularity. R-C6 needs **REGION** granularity (mixed narrative + generated index in one file).
- **R-C1 `resolveSprintPin`** (current/last/next) + **R-C5 rollup** (per-sprint status, supersededBy-aware) = the data the index renders. R-C6 is a pure consumer.
...
- **`generateOverview()`:** read existing file → PRESERVE all bytes outside the markers → REGENERATE only the between-markers index from Sprint units (number-ordered) + R-C1 pin (mark current/last/next) + R-C5 rollup status per sprint (Done / Active / QA-pending / Planned / **Closed** with `Done + supersededBy` counts distinct). First run (markers absent): insert the marker pair at the canonical index location, preserve all surrounding prose.
...
1. **10 design-doc `planning.md` (S01-09)** — KEEP-HAND-AUTHORED (R-C7 classification); these are design briefs, not unit-generated boards. G5 narrative-exclude.
...
## Part C — vacuous-BITE (per R-C3 INV-C3)
...
- **INV-C6-2 index-from-units:** the index = Sprint units number-ordered + R-C1 pin + R-C5 rollup; can't drift; `--check` enforces.
...
- **INV-C6-4 fail-closed-vacuous (R-C3):** empty sprint set / unresolvable pin / missing markers / vacuous frozen-list → refuse + name.
- **INV-C6-5 no-status-invention:** index status = R-C5 rollup; `Done` and `supersededBy` counted DISTINCTLY (INV-C1-7 carried up); overview never invents Done-ness.
...
- **vacuous-BITE (R-C3):** empty sprint set / missing markers / vacuous frozen-list → REFUSE with reason.
...
- Sequence: R-C6 = LAST S37 design → **S37 design phase COMPLETE** → architect goes to backstop-on-ship posture.
