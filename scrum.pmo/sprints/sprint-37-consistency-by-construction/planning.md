...
- [ ] [Task 37.4.1: MODEL self-heal on read — pin/board/task objects validate on init/read (fresh-or-refuse), never return a silently-drifted value](./task-37.4.1-model-self-heal-on-read.md)
- [ ] [Task 37.4.2: CONTROLLER single transition entry (statusNext) — guard + evidence-precondition + tick-checklist + DERIVE + persist + emit](./task-37.4.2-controller-statusnext.md)
- [ ] [Task 37.4.3: CONTROLLER single-source Done — statusNext OWNS the Done transition, R40.10 approve DELEGATES, tronApprove folds in (no second writer)](./task-37.4.3-controller-single-source-done.md)
