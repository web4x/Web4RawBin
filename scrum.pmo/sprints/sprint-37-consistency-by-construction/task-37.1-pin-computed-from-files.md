...
# Task 37.1: Pin is COMPUTED from files (never hand-set) [R37.1]
...
## Traceability
  ...
  - up
    ...
    - Requirement R37.1 `[requirement:uuid:91486de1-0cdd-4d57-b07e-9a5ce945702a]`
  - down
    - None (atomic task)

## Task Description

R37.1 (build 2nd). BOTH the CurrentSprint pin AND the current-TASK pointer ('📌 Current: Task N') are DERIVED by a resolver from the scenario units on disk (sprint = the sprint with active work; task = the active in-progress task) - NEITHER has a hand-editable stored value. 'Advance' becomes RUNNING the resolver. Fixes BOTH measured stale pointers: the 3-sprint-stale sprint-pin (S33 while work on S36) AND the ~5-sprint-stale task-pointer ('Task 31.1') - same drift class.
