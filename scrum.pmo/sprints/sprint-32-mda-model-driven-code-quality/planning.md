...
- [x] 🏁 [Task 32.6: Relationship views (attribute/getter/setter whose type is another unit)](./task-32.6-relationship-views.md)
- [x] 🏁 [Task 32.7: PUML serializer/parser (diagram <-> .puml, no-dup, same-UUID round-trip)](./task-32.7-puml-serializer.md)
- [x] 🏁 [Task 32.8: Action-driven M1/M2 sync (TS <-> model <-> PUML, same-UUID, no drift)](./task-32.8-m1-m2-sync.md)
...
- [x] 🏁 [Task 32.9: FeatureManager auto-discovers Features — MDA feature auto-appears + launchable @390, membership-gated (Tron device-QA REOPEN)](./task-32.9-feature-manager-registration.md)
- [x] 🏁 [Task 32.10: /model page — select tree node opens drawer (node-detail + reachable diagram / method signature) (Tron device-QA)](./task-32.10-model-page-drawer.md)
- [x] 🏁 [Task 32.11: In-diagram drag-a-class-to-add-view works on /model (DnD is the vision, was not wired)](./task-32.11-dnd-drop-into-diagram.md)
