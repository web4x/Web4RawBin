...
`/edit/scrum.pmo/sprints/sprint-8-monaco-editor/planning.md` opens the editor with that file loaded. The path after `/edit/` is the relative file path from PROJECT_ROOT. This enables direct linking to files — share a URL to open a specific file in the editor.
