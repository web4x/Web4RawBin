...
## R35.1 RESIDUALS + preview-unification CALL (architect 2026-08-04)
Expert confirmed both residuals = file-type DUPLICATES (provider's type==='file' keying CORRECT, no has-file-metadata needed): (1) rb-detail-view:134 createFilePreviewButton is a DEAD fallback (guarded type==='file' + !closest(rb-file-detail), but tagMap ALWAYS routes file→rb-file-detail → never fires) — REMOVE (+ renderFilePreview + dead markers 1a5ad916/71954a38 → req re-points). (2) rb-file-detail:54 cv-newtab = open-newtab duplicate — REMOVE (+ add data-url/uuid/mime/name/token to cv-actions so the bar handler reads it); pz-reset STAYS (in-pane zoom).
### DECISION = (A) unify file-preview to the TOGGLE mechanism
rb-file-detail's pane is always-on; the type-conditional provider shows `preview-file` for EVERY file detail. (B) always-on + no-op = the bar action is PRESENT-BUT-DEAD in rb-file-detail → FAILS @390 'each action FIRES per type' + inconsistent UX. (A) align rb-file-detail to the content-preview toggle (pane starts hidden + `.cv-preview-content`) → `preview-file` toggles it UNIFORMLY = ONE file-preview mechanism, fires meaningfully in every file context, correct-by-construction (DRY, one mechanism). CHOOSE (A).

## R35.4 REOPENED — DRY sprint-structure fix (architect 2026-08-05, Tron: reuse, don't reinvent)
DIAGNOSIS (root, measured): `traceabilityRoots()` (server.ts:1251, dispatched by mofChildren b6c88d83 @:1294) iterates EVERY `ior:class:Requirement` → a FLAT list of 497 req roots sorted by NAME → STRIPS sprint structure+order. That is the bug (Test 83abce21 gated the flat shape = insufficient).
### THE EXISTING STRUCTURE TO REUSE (measured — one source of truth, do NOT re-fork)
- Ordered sprint list = `/api/trace/sprints` (server.ts:2090-2101): enumerates `ior:class:Sprint` units, **sorts by `model.number` ASC** (project order S1→S35). SAME source sprints.overview.md tabulates.
- The /trace page renders the current+sprints-overview via `rb-trace-tree.renderCurrentSprintEagerLazy` (rb-trace-tree.ts:447, impl e649a695), driven by trace-page.ts `data-eager-lazy` (:29): TWO parts — (1) **CurrentSprint** node (the CurrentSprint singleton, 3 slots) + (2) **Sprints 01-N collection** (eager sprint-nodes IN number-order, each sprint's children lazy).
- Sprint→req→chain walk = the EXISTING `/api/trace/children` (FWD_SCAN `Sprint:['tasks','requirements']`, `Requirement:['useCases',...]` → UC→Class→Method→Impl→Test, server.ts:2283). The MODEL rb-trace-tree already lazy-expands via this SAME endpoint.
### DESIGN — MINIMAL DRY WIRING (reuse the source, not the hierarchy)
- **REPLACE** `traceabilityRoots()`'s flat-497 body with the SAME current+sprints-overview node-set, from the SAME sources: `[ CurrentSprint node (reuse the CurrentSprint singleton uuid) , ...Sprint nodes in `number` ASC ]` — i.e. REUSE the `/api/trace/sprints` ordered-Sprint enumeration verbatim (NOT a new sort, NOT flat reqs).
- **TRUEST DRY (recommended):** EXTRACT the ordered-sprint enumeration currently inline in `/api/trace/sprints` (:2090-2101) into ONE shared helper `sprintOverviewNodes()`; BOTH `/api/trace/sprints` AND `traceabilityRoots()` call it = single source of truth for sprint order. (Minimal alt: `traceabilityRoots` inlines the identical `ior:class:Sprint`-by-`number` enumeration — same result, one less extraction.)
- **Sprint→req→chain = ZERO new code:** each Sprint node is a real `ior:class:Sprint` uuid → the model rb-trace-tree lazy-expands it via the EXISTING `/api/trace/children` (FWD_SCAN Sprint→requirements→…→Test), EXACTLY as /trace does. Requirements are the chain roots under each sprint (Task = navigation, per the 6-step chain). NO fork, NO reinvented hierarchy.
### MECHANISM (for req — the explicit ask)
- `traceabilityRoots` is an **impl-EDIT** (rides mofChildren **b6c88d83**; the `rawbin:traceability→traceabilityRoots()` dispatch @:1294 is unchanged) — its BODY swaps flat-req-scan → the shared ordered-sprint enumeration + CurrentSprint node.
- Optionally ONE new small decl `sprintOverviewNodes()` (the extracted shared enumeration) so /api/trace/sprints + traceabilityRoots share it (req IMPL-MINTs if we take the extraction; else the edit rides b6c88d83 alone).
- Sprint→req→chain expansion rides the EXISTING `/api/trace/children` trace-walk impl — NO new decl.
### SHARPENED AC (align + refine req's DRY draft)
"AC-reuse-sprints-overview (DRY): rawbin:traceability resolves to the SAME current+sprints-overview node-set the /trace tree builds from the ONE ordered-Sprint source (`ior:class:Sprint` by `number` ASC = /api/trace/sprints = sprints.overview.md order) — a CurrentSprint node + Sprints in project order S1→S35; each sprint expands (via the EXISTING /api/trace/children walk) to its requirements → each req to its chain (Req→UC→Class→Method→Impl→Test). NOT a reinvented hierarchy, NOT 497 flat roots. GATE @390: expand rawbin:traceability → CurrentSprint + ordered sprint nodes (SAME order as /trace + sprints.overview) → each sprint → its reqs → each req → its chain." (Test 83abce21 flat-gate = SUPERSEDED; re-gate on this reused structure.)
