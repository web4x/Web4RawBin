...
|------|------|--------|----------------|
| SVG viewer fullscreen iframe + native zoom | `bef36fd2` | In Progress 🧪 | **Architect chain wired (`38653299`)**: UC `c27d67d8 svgViewer.pinchZoom` + Class `7dd2f3c3 SvgViewer` (sourceFile=server.ts:843) + Method `4a4591ca SvgViewer.onPinchEnd` (lines 880-881) + Impl `094c18a4` touchend+touchcancel apply() flush. **Tester champagne PASS (`82ddae97`)**: Test `10c2e3ca` 2-touch pinch (touchstart→move→end no pan), 2/2 PASS; chain Impl→Test wired (Impl.tests[]=[Test], Test.verifies[]=[Req042bab1a R18.34]). Impl shipped v0.5.114 → v0.5.121; rule-pair (a)+(b) ✓. R18.34.B DEVICE-ACCEPTED 2026-06-10 (Tron on v0.5.125 — snap-back gone, gate #27 cleared). Expert stripping debug instrumentation → v0.5.126 clean. Tester writing corrected device-representative champagne. R18.34.B joins Tron-QA gate alongside R18.34. **Only Tron final QA + v0.5.126 clean-build acceptance remains.** |
...
| ~~`675cc8e3` Source link~~ | DELETED 2026-06-09 | — | DISPOSED per PO 2026-06-09: source-link scope ALREADY COVERED by R18.26 + R18.27 + R18.28 (renumbered from original R18.13/14/15 in Follow-on D), shipped in `df4e4011` v0.5.99 (Parent link/ownerIor), `c3ba4fd9` (req), `08ae00f8` v0.5.106 (source-link filter). All three reqs owned by **T187**. Disposed: deleted `675cc8e3` task unit + `task-r18.13-15-source-browse-line.md` + 3 stale dup R18.x req units + 2 generator artifacts. UC `725981f9 sourceLink.browse` re-pointed `ownerIor` → T187 and added to T187.useCases[]. S18.tasks[] 14 → 13; S18.requirements[] 21 → 18. Zero residual refs verified. |
| T202 Class.method-per-UC narrowing — shared Class picks wrong method | `8a303a65` | Planned | **req-eng → architect → expert → tester** (CMM4 4-role) | NEW 2026-06-09 (commit `b30f40a2`). Sibling/follow-on to T187. When ≥2 UseCases share a Class, the chain expander uses global `Class.methods[]` fan-out instead of UC's `chainMethod` → wrong method shown. Fix: `/api/trace/children` passes UC `chainMethod` context so expander resolves per-UC. **Placeholder Requirement** `4d525a4d-…` per learning #38 — req-eng to canonicalize (verbatim Tron quote + real R18.x altId). Architect designs after; expert implements with rule-pair (a)+(b); tester verifies AC1-AC6 (incl. T187 baseline preserved + trace:audit:strict clean). |
...
4. **Duplicate R18.13/14/15 requirement scenario units — RESOLVED 2026-06-09.** Three orphan dups (`7d59b620`, `a0abb37d`, `b1299650`) DELETED per PO decision (anomaly #1 dedupe pattern). Before delete: 2 Done tasks (`bda7857d` Forward-only fallback + `10a52f9f` Orphan-method cleanup) had `coveredRequirements[]` pointing at dup R18.13 — re-pointed to canonical R18.13 (`68bdef75`); canonical R18.13.tasks[] reciprocates with both Done tasks added. UC `725981f9 sourceLink.browse` had `ownerIor` pointing at the deleted `675cc8e3` task — re-pointed to T187 (semantically appropriate; T187 owns R18.26/27/28). 2 generator artifact files deleted. Zero residual refs verified via full `grep -rl` of `scenario/`.
