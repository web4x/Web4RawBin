...
`focus`=30.53 (Sprint-30 Done) never advanced into S31; S31.1–31.4 statuses stuck "In Progress"; stale `nextBacklogOverride`→31.3. **The primary desync is the FOCUS flag** — it is the deliberate pin that names the current task, and it is stuck on a past-sprint done task with NO S31 task focused. Truthful DATA (PO-clarified 2026-07-20) = **set FOCUS on the real current S31 task + honest per-task status** — NOT force-Done: R31.3/R31.4 are legitimately **gate/device-pending** (not Done), so several S31 tasks may correctly be non-done at once. That is exactly why the code derivation (row 3) must key CURRENT on a **valid focus first**, and use forward-fall-to-first-not-done only as a LAST-RESORT fallback — "first not-done" cannot disambiguate multiple legit in-flight tasks; the planner's focus does. Planner owns this and is measuring in parallel.
...
- **ROOT-3 = PLANNER (data):** set FOCUS on the real current S31 task + truthful per-task status; clear the stale nextBacklogOverride→31.3. Do NOT force-Done R31.3/R31.4 — they are gate/device-pending (PO-clarified). The code fix keeps the pin correct ONCE focus+status are truthful; valid focus is the primary current signal.
...
Design-only (architect). Expert builds fixes 1-3; planner corrects data (root-3); I backstop the recompute + restart remoteShells:0.2 if server.ts ships.

## ARCHITECT BACKSTOP — v0.7.100 / 85b73ba15 (robbin-architect 2026-07-20): **PASS**
Expert shipped the logic half; I restarted remoteShells:0.2 → served **0.7.100**.
- **STATIC (vs spec):** ROOT-1 — server.ts:1374 `const slots = CurrentSprint.slotsFrom(idx)` (imports CurrentSprint :41; NO `model.slots` read); `static slotsFrom(idx)` (CurrentSprint.ts:87-88) = `new CurrentSprint(index).getThreeSlots()` — throwaway instance on the FRESH per-request index, bypasses the cached singleton. ROOT-2 — current derivation (:213-216): valid focus (`t.focus && !t.done`, in-sprint) wins → else chain-req in-sprint `!done` → else forward-fall first `!done`; chain-only fallback (:217-221) guards `!cuDone`. ROOT-2b — chain-fallback name `cu.model?.name || this.taskName` (unit first, cache last, :221).
- **LIVE (public /api/trace — solo-verifiable, NOT 403-limited):** `GET /api/trace/children/current-sprint-singleton…001?mode=trace` (recomputed) → **📌 Current = Task 31.4 [IN-PROGRESS] with the LIVE title 'default-drawer'** (NOT the frozen 'fullscreen' — proves ROOT-1 recompute + ROOT-2b live-name), **✅ Last Completed = Task 31.2**, **📋 Next Backlog = Task 31.5** — matches PO-expected exactly; three slots DISTINCT (BUG-C). INV-G unchanged (`/api/server-manager/tree`+`/server-manager` → 403); `/trace` → 200.
- **Recompute-on-read = by construction:** `slotsFrom` never touches `model.slots`, so the served pin self-heals on every read regardless of the persisted snapshot (planner's stopgap also refreshed persisted, so both now agree — no live delta available, but the static path proves independence).
- **Remaining:** ROOT-3 truthful (planner e6f107ef2, focus→R31.4); pin now correct-by-construction. Tooling gap #102 unblocks per expert; R31.4 gate GREEN pending Tron device visuals.

## NOT-A-BUG (measured 2026-07-20): `wipStatus=class` is a UNIFORM convention artifact, NOT an R31.4 gap
Planner flagged R31.4's chain "class hop empty (wipStatus=class)". MEASURED: this is how the pin looks for **every** task — the champagne-complete R31.2 chain has the SAME `uc.classes=None` / `class.ownerIor=None` / `method.ownerIor=None`. The repo convention traces the chain via `UC.method`→Method + `Class.methods[]`→Method and never populates `UC.classes[]`. `autoFollow` (CurrentSprint.ts:379) derives the class hop from `ucM.classes[0]`, which is empty for all tasks → `activeHop` always lands on `class` → `wipStatus='class'` uniformly. So it is **cosmetic + uniform**, blocks nothing, and I did NOT wire R31.4's `uc.classes[]` — doing so for one task would fork it out of convention. It does NOT affect the pin's current/last/next slots (the recompute fix targets those). OPTIONAL separate design item (NOT part of the pin-keeping fix): make `wipStatus` meaningful by deriving the class hop from the Method's owning Class (`Class.methods[]` reverse-lookup) instead of `UC.classes[0]` — repo-wide, PO-authorized only. This stays SECONDARY to the R31.4 drawer/badge build (in flight).
