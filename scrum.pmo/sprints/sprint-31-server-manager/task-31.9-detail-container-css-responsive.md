...
ROUND-2 CHAIN-COMPLETE, all 7 ACs MET (verified disk: R31.9 tally 7 met explicit; req 2c9e3d000). Round-2 gate GREEN DET-3x v0.7.134 @390 /trace+SM (tester 0e42753ea: full-width + scroll-past + resize + non-regression held). Architect OWNED the root (their fix-table app.css:398 position:fixed changed bottom drawer static->fixed overlay = lost full-width + scroll-past); 1-line revert to position:static + width:100% (expert 928bdd5d6, impl-edit ff684e10 no-new-node reconcile-only). Round-1 5 ACs + Test 2b20035d preserved + round-2 2 ACs met. -> QA-Review; ONLY Tron device re-verify (the resize/scroll fix @390+intermediate+desktop) remains -> DONE. DRY: same fix /trace+SM (one rb-detail-drawer).
