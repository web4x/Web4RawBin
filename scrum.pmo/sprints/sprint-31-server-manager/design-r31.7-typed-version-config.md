...
## R31.7 FINAL CLOSURE (queued, PO 2026-07-22, non-blocking) — getVersion boot-stamped const, not manifest FILE
My own residual catch (surfaced at the FIX-2 restart): `getVersion()` (server.ts:55-59) reads `build-manifest.json` PER-REQUEST. A build-without-restart updates the manifest FILE → `/api/config` shows the NEW version while the process runs OLD code = the version-lie STILL possible (just a different file than the old package.json read). The restart-verify must fall back to PID/uptime because /api/config can still lie.
FIX (small, correct-by-construction): capture the version ONCE at process boot into a module-level const, and `getVersion()` returns THAT — never re-reading the file per request.
| File | Line | Current | Fix |
|------|------|---------|-----|
| `src/ts/server/server.ts` | 55-59 `getVersion()` | reads `build-manifest.json` per-request | at module load: `const BOOT_VERSION = (read build-manifest.json once) ?? PKG_VERSION;` — `getVersion()` returns `BOOT_VERSION`. Frozen at boot ⇒ `/api/config` reflects the RUNNING process's version; a build-without-restart can NOT change it (only a real restart re-reads). |
INV-V4 (served==booted): `/api/config` version == the version the running process booted with; a rebuild-without-restart leaves `/api/config` unchanged (honest: shows what's running). Closes the last version-lie axis — `/api/config` becomes a valid deploy signal again (no longer needs the PID/uptime workaround). Note: sw.js CACHE_NAME is a static file (build updates it immediately, served fresh) — that's correct for the PWA (client cache-bust); the fix is only for the server's self-reported version. req: fold as an R31.7 closure AC (INV-V4); thin impl-edit on getVersion, no new node. expert builds + real restart; I backstop (rebuild-without-restart → /api/config unchanged; real restart → updates). Non-blocking (P0/FIX-2 already closed).
