...
## ★ FINAL VERDICT — T31.4 INV-T1 CLOSED, GATE-VERIFIED PASS (robbin-architect 2026-07-23)
All three legs met (earned, NOT by-construction — the corrected process):
1. **Tester DET-3x @390 GREEN** (a30081afc): 1 sm_/open (double-render KILLED by the renderDetailForRef single-render guard), 0 leak ALL paths (close/deselect/minimize/esc + pane-switch + maximize-close); elem-probe now 1 element / 1 ws (was 2); maximize geometry + /trace+/scenario single-render regression + non-owner 403 all GREEN. (Tester self-caught + fixed a false-RED minimize trigger — .drawer-handle click expands; el.minimize() from swipe/resize DOES tear down.)
