...
## ARCHITECT BACKSTOP — R31.8 slice-(a) v0.7.104 / f5a87d5f6 (robbin-architect 2026-07-22): **PASS**
Real restart (Ctrl-C→npm start, sole driver): fresh server.ts pid 642986 (etimes 26s); R31.7 INV-V1/V3 guards passed (clean boot at 0.7.104, Config-unit→0.7.104).
- **STATIC:** `ServerManagerGuard.requireFeatureAccess` (ServerManagerGuard.ts:51-61) = PURE MEMBERSHIP — `token=resolveToken(req); if(!token) deny; if(!allowedUsersOf(feature).includes(token)) deny` — NO `s.owner` shortcut, NO OWNER_TOKEN OR-bypass, fail-closed on empty allowedUsers (INV-F5/F6, codified in the impl comment). smSessions value carries token (server.ts:811 `{owner,expiresAt,token}`), cookie stays opaque sid. Choke-point (:929) + ws upgrade (:2340) BOTH switched to `requireFeatureAccess('Server Manager')`; requireOwnerHttp RETAINED (FeatureManager slice-b root-of-trust). INV-G2 OWNER_TOKEN literal ==1.
- **★ INV-F6 PROVEN LIVE (the crux):** seeded a live OWNER + a live NON-OWNER session into tokenToClient (ws IDENTIFY). OWNER (member) → `/server-manager` 200 + `/tree` 200. **NON-OWNER holding a VALID live session → `/server-manager` 403 + `/tree` 403** — denied by allowedUsers-MEMBERSHIP, NOT by session-absence (they ARE a live session). Authorization=membership, authentication=session, never conflated.
- **INV-G unregressed:** no-token → 403; INV-G3 ws upgrade gated by requireFeatureAccess before handleUpgrade (socket.destroy-before-open, static :2340); OWNER_TOKEN not OR'd into feature access. `/trace` 200.
- Slice-(a) = live + correct-by-construction. Expert builds slice-(b) FeatureManager grant/revoke (hardcoded-owner-gated, mirror both sides) + the mint-generalization (INV-F6 becomes the live tester gate: grant flips a live non-owner session 403→200, revoke→403). I backstop slice-(b) INV-F4 (root-of-trust) + INV-F6-live on ship.

## ARCHITECT BACKSTOP — R31.8 slice-(b) v0.7.105 / ba86ddb6f (robbin-architect 2026-07-22): **PASS** (the Tron-facing FLIP)
Real restart (Ctrl-C→npm start, sole driver): fresh server.ts pid 667508 (etimes 25s), R31.7 guards passed at 0.7.105.
- **STATIC:** POST `/api/feature-manager` (server.ts:979) is `requireOwnerHttp`-gated (HARDCODED owner-only, root-of-trust) — a DISTINCT path NOT swallowed by the data-driven choke (INV-F4, no self-grant). `FeatureManager.grantFeature/revokeFeature` mirror `Feature.allowedUsers` (unit on disk) ↔ `profile.features` atomically; grant does NOT require a profile (mirrors profile.features only if present, still ok). No new OWNER_TOKEN literal (INV-G2==1).
- **★ LIVE FLIP PROVEN (Tron-facing, seeded live sessions):** INV-F4 — non-owner POST /api/feature-manager → **403** (no self-grant). FLIP — a live NON-OWNER: /server-manager **403** (not granted) → owner POST grant (uuid) **200** → non-owner SAME session /server-manager **200** (access flipped by allowedUsers-MEMBERSHIP) → owner POST revoke **200** → non-owner /server-manager **403** (flipped back). End-to-end data-driven access.
- **MIRROR both-sides:** grant added the token to ServerManager.allowedUsers; revoke removed it — the Feature unit SELF-CLEANED back to `[owner]` (revoke-immediate: featureAllowedUsers re-reads the unit fresh). No residue. INV-G unregressed (no-token 403, /trace 200, INV-G2==1).
- **NOTE (non-blocking, flag to expert):** the GRANT api takes the Feature **uuid** (`grantFeature(featureUuid)`), while the ACCESS gate reads allowedUsers by **name** (`requireFeatureAccess('Server Manager')` / `featureAllowedUsers(name)`). Same unit, two keys — fine (the slice-d FeatureManager UI has the uuid), but consider accepting either / documenting. My flip test 404'd once until I passed the uuid.
- Expert builds slice-(c) bootstrap + slice-(d) render. I backstop slice-c (owner auto-seeded into ServerManager+FeatureManager allowedUsers at boot, INV-G2==1) + slice-d (profile m.features render) on ship.

## ARCHITECT BACKSTOP — R31.8 slices (c)+(d) v0.7.107 / 0fe62f297 (robbin-architect 2026-07-22): **PASS** (IMG_4612 fixed)
Real restart (Ctrl-C→npm start, sole driver): fresh server.ts pid 698674 (etimes 30s), R31.7 guards passed, served 0.7.107.
- **(c) bootstrapSeed:** `FeatureManager.bootstrapSeed()` wired at boot (server.ts:2295, next to reapOrphans). Verified LIVE: BOTH `ServerManager 16604eee` AND `FeatureManager 2980b7d9` allowedUsers = `[owner]` (idempotent — single entry, no dup). `seedOwnerInto` (ServerManagerGuard) co-located with the literal → INV-G2 OWNER_TOKEN literal ==1 (no new literal, token never returned bare).
- **(d) profile m.features (★ TRON-FACING IMG_4612 fix), proven LIVE:** `featuresForToken(token)` (server.ts:862) scans Feature units by allowedUsers-membership; the PROFILE ws (:2708) sends `features: featuresForToken(profile.token)`. Seeded an owner ws (IDENTIFY) → the PROFILE message carried **`m.features = [Server Manager, Feature Manager]`** → `renderFeatureGrants` lists BOTH. So Tron's /profile 'Feature access' now shows Server Manager AND FeatureManager (was ServerManager-only). Data-driven (membership), not hardcoded.
- **INV-F4/F6 unregressed:** no-token → 403, /trace → 200 (gate unchanged from slice-a/b; the grant/revoke flip proven at slice-b). INV-G2==1.
- **R31.8 COMPLETE end-to-end:** slice-(a) data-driven gate (INV-F6) → slice-(b) grant/revoke FLIP (Tron-facing) → slice-(c) bootstrap seed (owner in both features) → slice-(d) profile lists all memberships (IMG_4612). Feature unit type + FeatureManager root-of-trust: live, backstopped, correct-by-construction. Authed on-screen /profile render = Tron device confirm (I proved the m.features DATA).
