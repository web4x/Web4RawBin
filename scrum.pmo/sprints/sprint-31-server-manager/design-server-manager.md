...
## R31.3 TREE RE-ARCH — SETTLE VERDICT v0.7.100 (robbin-architect 2026-07-20, PO+planner overnight): **SETTLED → QA-Review (Tron-visual only)**
MEASURED the SERVED/built bundle `src/public/dist/server-manager-MHRYENK4.js` (only one on disk, fresh) + server contract — NOT source-assumed. All 3 done + built + served on v0.7.100:
1. **Typed-nodes (session→window→pane):** server `/api/server-manager/tree` emits 3-level typed roots `otmuxSession→otmuxWindow→otmuxPane` with inline child-refs (server.ts:908-918); client renders via shared `rb-trace-tree.items`; bundle carries `otmuxsession/otmuxwindow/otmuxpane` icons; **badges via child-refs** = the v0.7.99 `childRefCount` fix (colon-immune). ✓
2. **Refresh:** bundle `getElementById("refresh").addEventListener("click",()=>{Ht()})`; `Ht()` = `load()` re-fetches `/tree` + resets `.items` (also on init). ✓
...
## T31.4 INV-T1 LEAK RED — root + fix ruling (robbin-architect 2026-07-23)
Tester root: rb-terminal-detail.mount `this.ws=ws` (:44) clobbers a pre-existing un-closed ws → double-attach → 1 sm_ leaked. MEASURED the callbacks: connectedCallback→mount() (:18); attributeChangedCallback→teardown()+mount() (:20). So the INTENDED invariant is "teardown BEFORE mount" — but mount() itself does NOT self-teardown, so ANY mount path that isn't preceded by teardown (a double-mount / re-connect / re-render race while the prior ws is still open) orphans the prior ws.
**ROOT-CORRECT FIX (correct-by-construction, NOT band-aid): make mount() IDEMPOTENT — call `this.teardown()` at the TOP of mount()** (reuse the existing teardown(): close this.ws + null it), before opening the new ws. This puts the "ONE live ws per element" invariant INSIDE mount() (the invariant-owner), so it holds regardless of HOW/how-many-times mount is invoked — the orphan becomes impossible BY CONSTRUCTION, immune to any current or future double-mount path. DRY (reuses teardown). This is stronger than hunting the one double-invocation (which could regress if a new caller appears) — though if the tester also spots a spurious double-mount (mount fired when it shouldn't be), eliminate that too as hygiene. cb153623 teardown (minimize/close/deselect drawer-transitions) is CORRECT — LEAVE IT; it handles the drawer-side teardowns, the mount-self-guard handles the mount-path. Gate: 0 sm_ leaked on ALL paths (open→open-another / re-select / pane-switch / minimize / close), tester @390.

## ARCHITECT BACKSTOP — T31.4 INV-T1 guard-close v0.7.131 (expert 162393c20): **PASS by construction**
Client-only, no restart. STATIC: mount() (rb-terminal-detail.ts:26) now calls `this.teardown()` at the TOP before opening the new ws — the ruling d5a1e266d fix; one-live-ws invariant now lives IN mount (the ws-owning site) → orphan impossible by construction, immune to any double-mount/re-connect path. teardown() idempotent (no-op on clean first mount), DRY. Only rb-terminal-detail.ts source changed (surgical, no other terminal/drawer edits). sm_ count = 0, served 0.7.131. Interactive 0-leak ALL-paths (open→open-another/re-select/pane-switch/minimize/close) = tester @390 (authed terminal, CLI-limited) — GREEN by construction (the invariant is enforced at the ws-owning site). cb153623 drawer-teardown unchanged.
