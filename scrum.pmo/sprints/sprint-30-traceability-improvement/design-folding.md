...
- **Gap affordance — CHEVRON-STYLED to match native folding (Tron ref IMG_4571):** a Monaco VIEW ZONE per collapsed gap rendering an OBVIOUS clickable fold bar `▸  ⋯ N lines` — the `▸` chevron LEFT-aligned in the glyph/line-number gutter column so it reads exactly like Monaco's native collapsed-fold marker; click anywhere on the bar → remove gap from `_collapsedGaps` → applyFold. Expanded gaps show a `▾` re-collapse chevron at their top boundary. Rationale: setHiddenAreas ALREADY produces the native "collapsed line-jump" look (the line ruler skips hidden lines, as in his screenshot) — the ONLY thing native folding adds is the `>`/`v` chevron control, so we reproduce THAT chevron in the view-zone/glyph. So a collapsed region visibly looks foldable/expandable like his reference, NOT silently-hidden lines. Change regions carry NO chevron → visibly non-collapsible (the guard, made visible). CSS: reuse a Monaco-fold-like glyph (`▸`/`▾`, cursor:pointer, muted until hover) so the affordance matches the native visual language. NOTE (design decision): we KEEP `folding:false` (native folding stays off — it can't guard change-regions or sync 3 editors); we match the AFFORDANCE (chevron) not the MECHANISM (setHiddenAreas), which is the robust combination.
