...
- [ ] **R30.14 — Service-Worker auto-update (deploys visible without hard-refresh)**
  ...
  **Acceptance criteria:**
  - [ ] **(invariant)** INVARIANT (Tron: clean releases, NO hard reloads): a version bump MUST auto-propagate to EVERY running client, and a manual HARD-RELOAD (Ctrl+Shift+R / cache-clear / close+reopen) must NEVER be required to receive the new version. A running client that only gets the new version after a manual hard-reload is a HARD FAIL.
  - [ ] **(auto)** Deploying a new version -> each running client picks it up AUTOMATICALLY within the poll interval: either a seamless hot-swap or a one-click 'New version - reload' prompt the user taps (that single tap REPLACES the manual hard-reload). No user action beyond at most one tap.
  - [ ] **(detect)** ServiceWorker.pollForWorkerUpdate: while the app is open, periodically (setInterval ~60s + on visibilitychange/focus) reg.update() re-fetches sw.js and re-runs the /api/config version compare, so a deploy is detected without a navigation/hard-refresh.
  - [ ] **(takeover)** ServiceWorker.claimClients: the sw.js activate handler calls self.clients.claim() (after old-cache cleanup) so the newly-activated SW controls open pages -> controllerchange -> reload runs; skipWaiting + claim = reliable takeover.
  - [ ] **(ux)** Primary UX = the existing one-tap banner ('New version - reload'); NO surprise auto-reload mid-edit (never nuke unsaved merge-editor state). Detection is automatic; the user chooses when to apply the pending version with one tap.
  - [ ] **(gate)** GATE (verified on a REAL long-open running client, NOT a fresh navigation): bump the version + deploy -> WITHOUT any manual hard-reload/cache-clear, the running client auto-updates to the new version (hot-swap or one-click banner) within the poll interval. Confirm the client now reports the new version. Pairs with R30.28 (served==committed==HEAD) so the bumped version IS what the client receives.
