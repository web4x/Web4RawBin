...
- [ ] **R30.35 — Diff coloring by kind (add=green/delete=red/modify=blue/conflict=brown) + per-block IntelliJ merge actions**
  ...
  **Acceptance criteria:**
  ...
  - [ ] **(action-visibility)** The 'x' (remove) action shows ONLY when the center holds BOTH versions (2 lines) - PER SIDE: a left-x on the left version and a right-x on the right version. No 'x' when the center already holds one version.
  - [ ] **(action-visibility)** Pressing left-x or right-x REMOVES that side from the center, leaving ONE version -> the change AUTO-RESOLVES (derived) and the editor JUMPS to the next change (jumpToChange).
  - [ ] **(action-visibility)** The '>>' (left) / '<<' (right) add-side action shows when that side's line is NOT currently in the center.
  - [ ] **(resolution)** The resolved-state DERIVES from the center line-count: 1 line in center = RESOLVED, 2 lines = UNRESOLVED. '>>'/'<<' adds that side (2 lines -> unresolved; 1 line -> resolved); 'x' removes a side (-> 1 line -> resolved). The R30.37 checkmark REFLECTS this derived state and can MANUALLY OVERRIDE it.
...
- [ ] **R30.37 — Per-change RESOLVED-state toggle (green checkmark: outlined=unresolved / solid=resolved)**
  ...
  **Acceptance criteria:**
  ...
  - [ ] **(toggle)** The checkmark REFLECTS the DERIVED resolved-state: SOLID-green when the center holds ONE version (resolved), OUTLINED-green when it holds BOTH (unresolved) - auto-updating from the center line-count. Pixel-distinguishable states.
