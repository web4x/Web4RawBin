...
IN PROGRESS — architect derive-PASS 72cbf9f49, EXPERT BUILDING now (impl-edit to RbDiffEditor: _rightUserPicked guard in setSideRef('right')/right-ref path + serialized promote with generation token; defaultIdx from a this.left.content snapshot taken BEFORE the awaits, not live this.right.content). → expert deploy → QA-Review → tester DET-3x GREEN (repro event-order shows NO post-pick left-reload) → Done. Planner advances status on gate. → DONE 6/6: RED→GREEN DET-3x FIX VERIFIED (c9ebd4dce v0.7.36, no regression). Chain-to-Test CLOSED (verified before Done, not relayed): Test 2b9f6c17 'R30.25 RIGHT-pick preserves LEFT (no post-pick left-reload)' status=pass, wired onto Impl 751934c1 RbDiffEditor.populateLeftHistory (tests[]=[4dca421e R30.17, 2b9f6c17 R30.25]) — impl-edit, no new unit as specified. ★ TESTER HONESTY NUANCE: 2b9f6c17 is the FIRST REAL race-fix test — the prior coverage was 'false-complete-before-fix' (passed without actually exercising the race window); the RED baseline + this test are the genuine gate. ⚠ REOPENED (PO correction): v0.7.36 had a SUBTLE residual RIGHT-pick-LOST bug (expert caught via own repro — the v0.7.36 gate was GREEN but missed the RIGHT-wins-race case; a 2nd false-complete). REAL FIX = v0.7.37 (ec5d62dee: RIGHT pick WINS the promote race — reset in loadSide + early-guard). Status Done→QA-REVIEW, ACs un-checked, version-ref v0.7.36→v0.7.37. HOLDING for tester RE-GATE on v0.7.37 (BOTH race cases + RIGHT-wins); the re-gate should extend/replace Test 2b9f6c17 to cover RIGHT-wins. On tester GREEN v0.7.37 → re-mark 6/6 + Done. → DONE 6/6 (this time for real): RE-GATE GREEN DET-3x on v0.7.37 (57fd48238, gate r3025-right-pick-preserves-left-gate.mjs expanded to 2 race cases + RIGHT-wins NON-VACUOUS assertion; served==gated confirmed by SM). Chain-to-Test closed: Test 2b9f6c17 pass on Impl 751934c1 populateLeftHistory. Applied the served==gated lesson: verified the gate cites v0.7.37 (the served bundle), not just GREEN.
