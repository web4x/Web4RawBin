...
- [ ] **(functional)** EXACTLY ONE Re-Sync button (the R33.6.5 action-bar one); NO pre-action-bar duplicate/old in-diagram toolbar action buttons remain - the old in-diagram toolbar is RETIRED when the actions moved INTO the drawer action-bar. Gate: count Re-Sync buttons = 1. (Device bug, architect diagnosis bc21ca747: the pre-action-bar in-diagram toolbar was not retired -> a duplicate/old Re-Sync remained alongside the action-bar one.)
