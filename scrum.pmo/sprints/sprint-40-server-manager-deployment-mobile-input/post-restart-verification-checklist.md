...
- **A2. Revoked-list — TRON CHOOSES the branch (PO ruling 2026-08-12: state-(C) restart-as-is = FORBIDDEN — armed+absent-list announces armed while the 116 still authenticate; the flag and reality must agree).**
  - **★ PATH-UNIFY PRECONDITION (must hold before Branch A can work — architect catch 2026-08-12):** the tracked+gated list is `revoked-token-hashes.json` (ROOT; `check:revoked-tokens` verifies IT) but the server loads `data/revoked-tokens.json` (DATA_DIR, ABSENT, NO bridge). ⇒ CI-green + list-committed still boots FAIL-OPEN. Branch A as "just materialize" is INSUFFICIENT.
  - **BRANCH A′ (revoke the 116 now — PO recommends, path-unified):** FIRST unify the path — point `REVOKED_TOKENS_PATH` at the tracked `revoked-token-hashes.json`, OR `gen --write --out data/revoked-tokens.json` (into the server's load path). THEN the server loads the SAME list CI certifies. Verify B1 boot log says `loaded 116`. Step-7 (F3) proves no over-reject. **FAIL → ABORT-REVERT.** (Just running `gen --write` to the root default without the path-unify = still fail-open.)
...
- **B1. Boot log `revoked-tokens loaded 116`** (NOT 0; 0 = fail-open — list missing OR the ★ path-unify precondition unmet, i.e. server loaded `data/revoked-tokens.json` while the list sits at `revoked-token-hashes.json`). This line is the runtime truth — it catches the gate-points-at-wrong-artifact bug even if CI was green. **FAIL → ABORT-REVERT.**
