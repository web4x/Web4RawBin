...
**Mechanism — every discovered boot is CLASSIFIED into exactly one of three sets, or it REDs:**
1. `OWNED` = an explicit ownership predicate for our boots (e.g. `session/agents/robbin-*` — a NAMED prefix we own, not a catch-all). Post-cure these MUST be timeless; state ⇒ **RED** at `--strict`.
2. `EXCLUDED` = an **EXPLICIT NAMED list** of the other-team boots (`oosh-expert`, `oosh-po@MacStudio`, `oosh-tester`) — never a `oosh*` wildcard, never a silent skip. Each entry carries a reason (other-PO-owned) + a time-box (closes when oosh-PO coordinates their cure). State in these ⇒ **WARN-loud in our report** (flagged, not RED — we can't fix them), never green-silent.
3. `UNCLASSIFIED` = discovered − (OWNED ∪ EXCLUDED). **A boot in neither set ⇒ RED/triage-flag** — a NEW unowned boot CANNOT silently inherit the exemption; someone must explicitly classify it (ours→cure, theirs→named-exclude-with-expiry). This is the divergence check that keeps the exemption honest.
