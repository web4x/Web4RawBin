...
- **Dead-exemption → replaced by SELF-ANNOUNCING EXPIRY (PO + architect, 2026-08-20).** Under a name-list, dead-exemption guarded "an exclusion outliving its FILES". Under the `/^oosh-/` predicate that risk is moot (a prefix only matches files that exist), but a WORSE risk opens: the exclusion outliving its NEED — if oosh cures their boots, WARN-loud (which only fires on state-bearing files) goes SILENT and the time-boxed coordination debt quietly becomes PERMANENT (same decay class as WARN-forever / UNVERIFIED-as-permanent-park). **Replacement = EXEMPTION-SATISFIED detection:** when every `/^oosh-/`-matched boot is timeless+pointer, print LOUD "EXCLUSION SATISFIED — all N oosh boots timeless; coordination debt CLOSABLE, predicate retireable" (non-enforcing good-news, but unmissable). ★ GENERALIZED (my improvement): **a time-boxed exemption's closure clock must NOT be a remembered DATE — it must be a machine-checkable CLOSURE PREDICATE the guard EVALUATES every run and ANNOUNCES when met** (for `/^oosh-/`: closure-predicate = "all matched timeless"). A date is decoration (someone must remember to check); an evaluated predicate is self-enforcing. **Every time-boxed exemption must be able to announce its own expiry, or the time-box is decoration.** (Rename/vanish of an oosh boot out of the prefix is caught by the owner-manifest diff below, not a separate floor — no redundant MIN_OOSH.)
