...
- [ ] AC-add-folder-verb-appears-in-room-context (@390): Tron @v0.8.165 — Add-folder is TOTALLY MISSING on the room Files folder detail (drawer shows only Scenario + Edit). AC: on 390px the Add-folder verb APPEARS on the room Files folder detail (ROOM context) and works. Root (architect): add-folder was a /MODEL-view verb, never offered in the room = service-built-but-UNWIRED.
