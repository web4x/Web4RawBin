...
- **★ INV-PDG-7 FRESHNESS — a gate result older than the artifact it certifies is NOT-RUN → RED (PO guard 3, 2026-08-08):** every device-gate unit is STAMPED with the exact `version` AND `commit` it was produced against. A GREEN result is valid ONLY for that exact served version+commit; the moment the served artifact differs (version or commit), the prior green is STALE = **NOT-RUN = RED** (per INV-PDG-2). `/api/gate-status` and deploy-done compare the latest device-gate unit's stamp to the LIVE served version+commit — a mismatch reads RED, never "it was green once." This kills the stale-artifact class (a green from v0.8.56 cannot certify a live v0.8.65). Applies to EVERY gate this trigger runs, including R40.19's change-lint and R40.20's device-only ACs.
