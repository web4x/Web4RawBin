...
## §DISCOVERY = ONE utility (req/PO: the recursion one level up)
`AcGuard.assertUniversalEnumerated` discovers the "X universe" by SHAPE (structural/AST), and the live-MVC lint (R37.12 B) discovers `*-detail` async-append sites by SHAPE — SAME operation ("find code entities matching a shape"). Two implementations = the one-implementation-family problem RECURSED. So: **ONE shared `structuralDiscover(shapePredicate)` utility** — an AST/pattern enumerator over the source — that BOTH route through. `assertUniversalEnumerated` calls it with the AC's shape-predicate to get the enrolled-universe (DERIVED, never a hand-list — a declared should-enroll set would inherit the drift = the 6th reminder). The live-MVC lint calls it with the async-append-site predicate. Invariant `discoveryUtilitySingleSource`: no second shape-matcher exists; a raw ad-hoc component-enumeration = lint RED (the discovery utility guards ITS OWN family). This closes the recursion: the mechanism that finds drift is itself single-source.
