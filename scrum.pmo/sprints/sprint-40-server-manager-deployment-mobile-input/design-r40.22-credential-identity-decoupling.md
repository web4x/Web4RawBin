...
- **★ MOVING-TARGET flag (measure-a-stable-state):** `data/users/` is LIVE — the prod server writes to it as users act, so the backup + manifest + migration race with in-flight writes; a file written to `data/users/<token>/` DURING the move can be missed, and a manifest taken over a moving tree isn't reproducible. REQUIRE a consistent snapshot: either quiesce writes during the migration window (the bridge makes reads safe, but WRITES to the legacy path mid-move are the risk), or snapshot-then-reconcile (re-scan for files newer than the snapshot and migrate them too). The backup, my manifest, and the migration must all be taken at the SAME quiesced point. Flag to expert before the first write.
