...
## ★ CONSTRUCTION NOTES — carry these on the chain (they get accidentally undone by "simplifying" later)
1. **`ownerActionFetch` is OWNER-SCOPED, never blanket.** Attach `x-player-token` ONLY to the 4 owner actions (+ any future owner button), via ONE dedicated, obviously-named helper. Do NOT route general reads through it. Reason beyond leak-surface: **a blanket identity header risks ACCIDENTAL OWNER-ELEVATION on reads** — an endpoint that branches when it sees an owner token could silently start returning owner-only data to a page that never asked. That is a quiet failure that looks like a feature. Least-privilege: identity travels only where owner-auth is required.
2. **Header-auth is deliberately MORE CSRF-safe than a cookie.** A cross-site POST cannot set a custom request header, so `x-player-token` cannot be forged by a cross-site form — whereas a cookie-based owner session auto-sends and is CSRF-exposed. So this fix lands on a BETTER security posture than a cookie would. **Do NOT "simplify" this to a cookie-based owner session later** — that would silently regress CSRF safety. This property is why header-over-cookie is the intended design, not an accident.
3. **Single-source against per-call-site regression:** the 4 (and future) owner actions route through the ONE helper, so a 5th owner button cannot re-introduce the bug with a raw fetch. (make-current was exactly a call site the R40.45 work never reached — the single helper is the structural fix for that recurrence.)
