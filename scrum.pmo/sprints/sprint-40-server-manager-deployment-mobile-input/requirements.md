...
- [ ] **R40.39 — Feature is MISSING its type-index link folder — Features must resolve THROUGH the index like the other 18 types, never a corpus scan (structural regression, not performance)**
  [requirement:uuid:7f5ea6f1-a907-4221-bcd6-64b62431fd5b]
  > Tron (2026-08-17): "thats the reason for having a link folder of type features with only the references ln links... like on task, class, and all the other scenario-units... when did you lose that!!!"
  STRUCTURAL DEFECT (Tron 2026-08-17): scenario/sprints.md/ holds a TYPE-INDEX LINK FOLDER (ln reference links) for 18 unit types — bug/changerequest/class/currentsprint/device/file/implementation/message/method/requirement/room/skill/sprint/task/test/tracelink/usecase/user — and **feature is the ONE missing**. That is WHY bootstrapSeed walks all ~5677 units: Features have no index to resolve through, so someone wrote a corpus scan instead. This is a MISSING TYPE-INDEX (wrong shape), NOT a performance problem: measured boot is ~1.3s and a full 5680-unit scan is 0.1s, so speed was never the point. FIX: restore feature's type-index folder (same ln-link convention as the other 18, invent NO new mechanism); Features resolve THROUGH it; DELETE the walk-all-units path because it is the WRONG SHAPE. Still-real hygiene retained: boot read-mostly (seed-if-absent lazily) + non-atomic Feature write -> temp+rename. ★ AUDIT (measured): feature is NOT the only gap — 11 types-in-use lack a folder (see AC-audit).
  **Acceptance criteria:**
  - [ ] **(functional)** `feature` gets its TYPE-INDEX LINK FOLDER at scenario/sprints.md/feature/ — ln reference links to the Feature units, EXACTLY like the other 18 type folders (task, class, requirement, ...). Same convention; invent NO new mechanism. This restores what was lost.
  - [ ] **(functional)** Features resolve THROUGH the feature type-index (ln-link folder), NEVER a corpus scan. DELETE the bootstrapSeed walk-all-units path because it is the WRONG SHAPE (a type with no index got a corpus scan), NOT because it is slow.
  - [ ] **(gate)** A GATE FAILS if ANY unit type is resolved by a corpus scan when a type-index folder exists for it, with STUB-MUST-FAIL: plant a corpus-scan resolution for an indexed type -> assert RED. Catches the missing-index/wrong-shape class by construction, not by the next incident.
  - [ ] **(functional)** Boot is READ-MOSTLY: bootstrapSeed does NOT write per Feature on the boot path — seed-if-absent LAZILY at point-of-use (auth path already loads allowedUsers), OR a read-only ASSERT + explicit repair. (Real residual, survives the correction.)
  ...
  - [ ] **(functional)** AUDIT — is `feature` the only type missing an index? MEASURED (2026-08-17): NO. 18 type folders exist; 11 ior:class types IN USE lack one: **Feature** (confirmed genuine, boot-used) + Company, Config, Email, Gate, ModelElement, Phone, Profile, Relationship, TestCase, WebItem. Each must be CLASSIFIED genuine-lost-index vs intentionally-excluded (runtime-PII types Company/Email/Phone/Profile/WebItem may be intentional; Config/Gate/ModelElement/Relationship/TestCase may be sub-types/newer) — architect/PO triage. 'If we lost one we may have lost more' = confirmed.
