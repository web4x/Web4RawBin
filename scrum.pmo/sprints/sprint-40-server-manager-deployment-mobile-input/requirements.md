...
- [ ] **R40.106 INC-5 — the 3 duplicated create-path children[] RMW bodies DELETED into the ONE FolderService.linkIn (DRY collapse, pure refactor)**
  [requirement:uuid:025ff2e8-34f3-4d04-9193-5bc2b0bdce1c]
  The DRY-collapse increment of R40.106 (One DnD; identity-by-reference). The 3 duplicated create-path children[] read-modify-write bodies (multipart-upload-into-folder, federation-import, add-folder-nest) were DELETED into the ONE FolderService.linkIn — the object owns the edge-write, no per-path duplicate. A PURE REFACTOR: the expected shape was that NOTHING moves, and nothing did. SHIPPED + GATED + architect-backstopped @ served v0.8.221 (568c4fd81, pushed+tagged). Owner = expert (built) + tester (gated) + architect (backstop PASS).
  **Acceptance criteria:**
  - [ ] **()** 
  - [ ] **()** 
  - [ ] **()** 
...
- [ ] **R40.106 INC-6 — NESTED folder pure-edge flip: rendered children AND directChildCount both derive from children[] EDGES (single source); FIX-A location-union retired for nested**
  [requirement:uuid:ad93d384-22a3-49a9-87be-26bfc4296232]
  The pure-edge flip increment of R40.106. A NESTED folder now derives BOTH its rendered children AND its directChildCount from its children[] EDGES = ONE source of truth; the FIX-A location-union is RETIRED for nested via if(!selfFolder); ROOT unchanged (see the remaining-half debt 3b545845). Scope-B ruled on CORRECTNESS (PO): count-on-location while render-on-edges would DISAGREE the first time a true edge-only link exists = a chevron showing N over a folder rendering N+1. SHIPPED + GATED @ served v0.8.222 (8eb775663, pushed+tagged, architect 7-point bar PASS, tester GREEN). Owner = expert (built) + tester (gated) + architect (7-point bar).
  **Acceptance criteria:**
  - [ ] **()** 
  - [ ] **()** 
  - [ ] **()** 
...
- [ ] **R40.106 INC-7 DELETE-PROTECTION (in flight) — a delete really removes (verify-GONE + stays-gone) AND never destroys a protected unit; sanctioned list-driven sweep + orphan-only tripwire**
  [requirement:uuid:740f8996-ee1e-417c-9fbc-c11b81586613]
  The DELETE increment of R40.106 + its protection sub-arc (crossRef R40.107 persist-invariant). Ran as INCIDENT-RESPONSE (protecting Tron's live data while building real delete), now boarded scenario-first so the pin can answer 'what is being driven'. THE ARC (each shipped ALONE + gated + attributable): v0.8.225 deleteRoom COMPOSITE verify-GONE + deleted-flag resurrection fix + 6 keep-marks (architect-signed on the FIRING; first real prod deleteRoom proven GONE + STAYS-GONE) · v0.8.226 PRESERVE-UNKNOWN merge-over-stored (keep-marks were silently wiped on EVERY re-persist -> Tron's ACTIVE rooms lost protection because they are in use; fixed the CLASS not the field; canary bite green) · v0.8.227 sweep REAL-OWNER brace (Tron's 26 committed profiles were UNPROTECTED on the bulk-delete path; protection had lived in gate-CONVENTION not the acting code -> now by construction, RED->GREEN both halves, architect-signed) · v0.8.228 SANCTIONED sweep op (list-not-filter, two-phase fail-closed, MANDATORY setSha; sign pending bites) · v0.8.229 ORPHAN-ONLY tripwire (in flight). Owner = expert (built) + tester (gates) + architect (signs each firing). This is the CURRENT in-flight work (pin points here).
  **Acceptance criteria:**
  - [ ] **()** 
  - [ ] **()** 
  - [ ] **()** 
...
- [ ] **Your rooms and profiles are safe — a bulk delete cannot reach your rooms (all 26 committed accounts protected by construction) and a protection mark survives a save**
  [requirement:uuid:ddc37f95-a868-4948-832b-729cfd8b13a6]
  ONE arc-level ACCEPT row for the R40.106/R40.107 protection work, written in TRON'S TERMS — what he can independently verify, NOT the internals we built (PO ruling 2026-09-12: do not split into per-increment rows; a count is a pointer not a finding). THREE things he can check: (1) ROOM CREATION WORKS again on prod (measured); (2) his ROOMS CANNOT BE REACHED BY A BULK DELETE — all 26 committed accounts protected BY CONSTRUCTION in the acting code, not by gate-convention; (3) a PROTECTION MARK now SURVIVES A SAVE (it was silently wiped on every re-persist, so his ACTIVE rooms had lost protection precisely because they are in use). The internals are the EVIDENCE behind this row, not the row.
  **Acceptance criteria:**
  - [ ] **()** 
  - [ ] **()** 
  - [ ] **()** 
...
- [ ] **R40.107 CREATE-fix — room creation works again: GUARD#6 partial-write false-positive fixed (an ABSENT members field was read as an explicit drop-all, bricking CREATE_ROOM for a distinct-primary-no-redirect creator)**
  [requirement:uuid:7dc025f0-2e5d-42ee-83fd-dde5b322df19]
  The CREATE-fix under R40.107 (a WRITE must not mutate fields it was not asked to change). BUG e73b2af8 (GUARD#6 partial-write false-positive): an ABSENT members field on CREATE_ROOM was read as an EXPLICIT drop-all, so GUARD#6 false-positived and BRICKED room creation for a distinct-primary-no-redirect creator. REPRO-CONFIRMED-PRE-FIX (both cases). FIXED v0.8.224 (38e6fa06b), architect-signed (b267c452a), gate (cb2f93968), chain-to-shipped 4 Test units (commit ebeb8940d). This is the actual 'room creation works again' outcome (Tron-verifiable) — distinct from the delete-protection arc (ddc37f95). Owner = expert (fixed) + tester (4 Test units) + architect (signed).
  **Acceptance criteria:**
  - [ ] **()** 
  - [ ] **()** 
  - [ ] **()** 
...
- [ ] **R40.106 root-edge remaining half (DEBT) — ROOT folder still derives directChildCount by LOCATION, not edges; the 2nd half of the two-source pattern INC-6 fixed only for nested**
  [requirement:uuid:3b545845-b469-40d0-b09d-99347009975c]
  HONEST DEBT (PO 2026-09-12, boarded so it cannot hide) — INC-6 (ad93d384) flipped NESTED folders to single-source-off-edges, but ROOT still derives by LOCATION = the REMAINING HALF of the SAME two-source pattern. This is NOT correct-by-design and must NOT be mistaken for a deliberate asymmetry. The expert ANSWERED this rather than dodged it: design-notes/r40106-root-edge-remaining-half.md carries a drain-check + flags the files[] flat-registry DUAL-ROLE dependency to the architect — root is NOT a 1-line mirror of INC-6 (the flat-registry files[] serves two roles at root, so flipping root needs the architect's ruling first). Boarding it ALSO guards the nested single-source (INC-6) from silently regressing while root stays location-derived. Owner = architect (rule the files[] dual-role) -> expert (build root flip) -> tester (gate).
  **Acceptance criteria:**
  - [ ] **()** 
  - [ ] **()** 
  - [ ] **()** 
...
- [ ] **R40.106 — One DnD: every folder is a drop target, a drop places a LINK to the one stored unit; REMOVE unlinks, DELETE (red) removes the unit + all links**
  ...
  **Acceptance criteria:**
  ...
  - [ ] **(sweep/sanctioned-op)** The ONLY code-checking bulk-delete path (sweepEnumerated, an ORCHESTRATOR over per-unit deleteUnitWithScan) takes an EXPLICIT ENUMERATED uuid LIST, NEVER a filter/predicate — a predicate-driven bulk delete IS the dangerous primitive (once it exists someone calls it with a wider predicate). TWO-PHASE fail-closed: phase-1 dry-run ALL → if ANY would-refuse, ABORT and delete NOTHING (a refusal means the SET is wrong → re-derive, never skip-and-continue = unreviewed partial destruction). SET-SHA MANDATORY: authorize by setSha == sha256(sorted unique list); refuse on missing/empty/≠ (an omittable authorization is not an authorization). Print the set + setSha in BOTH modes (reviewed set == executed set).
  - [ ] **(sweep/real-owner-protection)** A unit owned by a COMMITTED REAL account (e.g. Tron's 26 committed profiles) is REFUSED on the bulk/sweep path — protection lives in the ACTING CODE (evaluated per unit on the !explicitOwner path), NOT in gate-convention that a new caller could bypass. ★ DISCRIMINATION (the half that matters): an EPHEMERAL-owned unit is NOT refused — a brace that refuses EVERYTHING is a FALSE fix a naive AC would accept. The brace must let ephemeral-owned units through AND refuse committed-real-account-owned ones.
  -> folder.dropPlacesLinkRemoveDelete [uc:uuid:4062b06d-2a38-4552-b3a7-5ef9458f50a0]
...
- [ ] **R40.107 — A WRITE must not mutate fields it was not asked to change (migration, handler, re-persist) — before/after differential asserts ONLY declared targets differ, fail-closed**
  ...
  **Acceptance criteria:**
  ...
  - [ ] **(preserve/carry-forward-unknown)** A re-persist PRESERVES every stored field the write does not model — an ARBITRARY (unmodeled) stored field SURVIVES a re-persist, not just model.protected. This is the CONSTRUCTIVE twin of the don't-mutate law: Room.persist() rebuilds the unit by enumerating owned fields and silently DROPS any stored key it doesn't model (Tron's ACTIVE rooms LOST their keep-marks on re-persist). FIX at the ONE chokepoint: carry forward every stored key the write does not set. ★ DISCRIMINATION: fixes the CLASS, not one field — 'enumerate + add protected' is a FALSE fix that leaves the class lossy; the arbitrary-field canary must catch it.
...
- [ ] **R40.109 — Remove the MODEL_STORE_SOURCE write-flag + its MODEL_READ_SOURCE alias — one store, no switch (the 'rollback lever' is a split-brain data-loss footgun, not a rollback)**
  [requirement:uuid:52203a54-a620-42ff-8651-6851e076a748]
  Tester EVIDENCE (isolated rig): with flag=model-store a model write lands ONLY in data/model-store (DRAINED) and is ABSENT from scenario/index where reads resolve -> on any bare restart / lost .env / flip back to default, that write is LOST (data written where nothing reads). The R40.81 close recorded model-store as an 'explicit rollback lever' — PO's wording, WRONG: reads ignore the flag entirely, so it cannot roll anything back; it only MISROUTES writes. It is a SPLIT-BRAIN SWITCH and a documented rollback that cannot roll back is a footgun someone reaches for in an incident. REQUIREMENT: remove the flag + its transitional MODEL_READ_SOURCE alias; one store, no switch.
  **Acceptance criteria:**
  - [ ] **(one-store/write-single-sink)** NO code path can route a model write anywhere but scenario/index — there is exactly ONE write sink. Re-introduce a store-choice (any flag/param selecting a write target) => RED. (Fixes the demonstrated footgun: flag=model-store wrote to the DRAINED data/model-store, absent from scenario/index where reads resolve = silent data loss on restart / lost .env / flip-back.)
  - [ ] **(one-store/no-switch)** The MODEL_STORE_SOURCE flag AND its transitional MODEL_READ_SOURCE alias are REMOVED — 0 references in code, env-readers, config, or scripts. One store, no switch. A residual reference to either => RED.
  - [ ] **(framing/correction)** The R40.81-close framing of model-store as an 'explicit rollback lever' is CORRECTED on record: reads IGNORE the flag entirely, so it could never roll anything back — it only MISROUTED writes (split-brain switch). Removing it loses NO rollback capability; rollback is the git pre-image, not this flag. A doc/req still calling it a rollback lever => RED.
