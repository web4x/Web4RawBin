...
- [ ] **R40.37 — Context-sensitive actions — an action invalid for the unit's TYPE/STATUS is NOT OFFERED (not clickable-then-refused); server guard stays as defense-in-depth**
  ...
  **Acceptance criteria:**
  ...
  - [ ] **(functional)** 'Add folder' IMMEDIATELY creates a real PHYSICAL folder AND its scenario unit, and the itemview appears as that new folder — no separate save step ('should on Add folder immediately add the itemview as a new physical folder with a scenario-unit').
  ...
  -> universalActionBar.applicableActionsFor [uc:uuid:1de961e7-5f64-4768-af31-7e7de8063ed5]
  -> folder.createPhysicalWithUnit [uc:uuid:0c58eb53-73fc-4e4d-97d4-dbe729cc0916]
