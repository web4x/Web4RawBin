...
- [ ] **R40.106 LINK + REMOVE capability — every folder is a drop target, a drop places a LINK to the ONE stored unit; REMOVE unlinks THIS edge only (unit + other links survive) — full UI cycle GREEN**
  [requirement:uuid:6de13bb2-2ee5-4f41-9a1f-a5fd335b3f30]
  The LINK + REMOVE increments of R40.106 (One DnD; identity-by-reference, same law as the one-store root R40.81). SHIPPED + GATED @ served v0.8.220 — the FULL UI CYCLE is GREEN end-to-end: 'link' is OFFERED on a file (move/remove/rename intact) -> real link Command -> picker -> the file RENDERS under the target folder -> REMOVE from that folder through the UI -> GONE from it -> SURVIVES in the source. Owner = expert (built) + tester (gated). MOVE and DELETE are the REMAINING R40.106 increments (DELETE = the red/dangerous one = INC-7 scan-unlink, the next build arc + the guard-first-then-delete convergence).
  **Acceptance criteria:**
  - [ ] **()** 
  - [ ] **()** 
  - [ ] **()** 
  ...
  - [ ] **()** 
  - [ ] **()** 
  -> folder.dropPlacesLinkRemoveDelete [uc:uuid:4062b06d-2a38-4552-b3a7-5ef9458f50a0]
...
- [ ] **R40.68 — deploy.mjs targets the POST-BUILD version — a config-unit-only bump cannot false-abort the served==committed poll or skip the device-gate**
  [requirement:uuid:62a19849-1c9d-4fd8-ac48-91e5dacbb542]
  deploy.mjs must target the POST-BUILD version, not a version read once BEFORE its own build stamps it — else a successful deploy is served-but-UNGATED. DEFECT (verified first-hand): scripts/deploy.mjs reads committedVersion() from package.json ONCE at start (:34) BEFORE the build step (:40) stamps package.json from the Config unit. On a CONFIG-UNIT-ONLY bump (no pre-build before deploy.mjs), the read is STALE. CONSEQUENCES measured live on v0.8.140: (a) the build-artifacts commit MESSAGE stamps the STALE version (098880909 says v0.8.139 while artifacts are v0.8.140); (b) the served==committed POLL targets the stale version -> a FALSE served!=committed abort on a SUCCESSFUL deploy -> the un-skippable device-gate step 5 NEVER RUNS = a served-but-UNGATED deploy (INV-PDG-5 defeated BY CONSTRUCTION). FIX (name-it, not inline): read the target version AFTER the build (or directly from the Config unit), OR enforce build+commit-package.json BEFORE the version read.
  **Acceptance criteria:**
  ...
  -> c7d92925 [uc:uuid:c7d92925-41ad-4c88-bbf6-8f8478293e8b]

- [ ] **R40.69 — Second-store + File-name=uuid: no ModelElement in 2 stores + ensureViewUnit refuses uuid path-ref (Tron prod regression)**
  [requirement:uuid:23e77b77-60f7-413d-9d00-81a83486022b]
  A File unit MUST carry its FILENAME in name (name=filename, location=real path, sourceFile=ior:file:<real path>, ownerIor SET) — never a raw UUID where the name belongs. TRON MAJOR REGRESSION (relayed via PO 2026-08-29, prod.wo-da.de:4444): prod File-unit JSON shows ownerIor=NULL; name=a raw UUID (b9fa43a2-ea98-…) instead of a filename; location=the SAME raw uuid; sourceFile=ior:file:b9fa43a2-… — File units carry UUIDs where the NAME belongs. It RENDERS CORRECTLY on an OLDER TEST server = a two-server bisect (the data/generation changed between test and prod, not the renderer). ROOT is the ARCHITECT's (code/data/generation — diffing the same File unit from prod vs test); THIS req captures the SYMPTOM + the correct shape + failable gates. SYMPTOM-ONLY, do NOT build a fix until the root is known.
  **Acceptance criteria:**
  ...
  -> fcebf924 [uc:uuid:fcebf924-ecf2-4727-bb98-8e0b40781701]
  -> a3712bf2 [uc:uuid:a3712bf2-be96-4520-b84b-e0529fd94ff5]
  -> modelTree.ensureViewUnit [uc:uuid:c3902503-5bdc-479c-8457-51ba4421f98a]

- [ ] **R40.70 — Add-folder/add-diagram is MVC-live in the tree + broadcast cross-client so EVERY client re-renders (a passive 2nd client, not the actor)**
  [requirement:uuid:542946c4-045f-4aab-8313-2f926ceaf530]
  When a user adds a folder or a diagram under a diagram, the create MUST (1) create the unit, (2) MVC-live add it to the TREE VIEW with NO reload, and (3) BROADCAST a cross-client ws event so EVERY connected client re-renders — not just the acting one. Tron's words: 'make sure that when i add folder or add diagram under diagram it not only creates diagram and folder but mvc live adds it to the tree view and casts it as cross client ws event so all clients rerender.' Ranks with the live-MVC failure Tron reported (current-task change: data changed but view only updated after RELOAD, next slot unchanged, gate GREEN / device NO) — same family, likely same root. The ACs are the PROPERTY not the mechanism, and AC-c is the one our gates keep missing: the ACTING client updating ITSELF locally is INADMISSIBLE as proof — the property is a SECOND, PASSIVE client updating from the broadcast ALONE.
  **Acceptance criteria:**
  ...
  -> modelTree.liveInsertChild [uc:uuid:f9572bd8-e895-4a18-9578-ed1a87b39d0d]

- [ ] **R40.71 — Method/Class detail panel shows NO code location — a user clicks a method and gets a bare, useless panel @390**
  [requirement:uuid:b5e4646c-e772-4b18-bf8c-7a5e1e64adc8]
  USER-VISIBLE DEFECT (architect+expert prod sweep, served v0.8.150). A user clicks a Method or Class in /trace or /model and the detail panel opens BARE: the red 'Method' badge + the name + the uuid, but NO source-file link, NO description body, empty sections = nothing to act on. MEASURED (robbin-req reproduced architect exactly): 344/657 Method units (52%) + 60/192 Class units (31%) have no model.sourceFile. Root (expert): sourceFile absent -> renderSourceLink() returns '' in rb-method-detail (fetchDetailData -> /api/trace/children data.sourceFile). ★ DERIVABLE: 328 of the 344 methods HAVE an implementation that carries sourceFile (the FORWARD edge, R37.32) = the fix derives the link from impl.sourceFile.
  **Acceptance criteria:**
...
- [ ] **R40.72 — Dangling tree links — a user clicks a chevron/link and it expands to EMPTY @390**
  [requirement:uuid:0deb7ab5-745b-4f16-9c30-6e62f4f27a42]
  USER-VISIBLE DEFECT (architect prod sweep, served v0.8.150; TYPE-CORRECT re-measure). A forward ref the tree TRAVERSES points to a unit that does not exist, so tapping the chevron/link expands to EMPTY. CORRECTED COUNT (architect owned an over-count): 17 user-facing dangling on the rendered surface = 15 Requirement.useCases -> dead UseCases + 1 Class + 1 Task. The earlier 41 (architect) and 469 (robbin-req working-tree) BOTH over-counted by including refs on keys the tree does NOT traverse (type-incorrect) + working-tree churn; 17 is the type-correct served number a user actually hits. RENDERED PROOF (better than any count): Requirement 1c3b86ad declares ONE useCase, that useCase is DEAD, prod returns ZERO children -> a user sees a Requirement whose ENTIRE chain is invisible. ★ CAVEAT (architect): a chunk of these are CROSS-BRANCH-UNCARRIED (the hotfix-vs-main divergence class) - some may VANISH on a carry, NOT need a code fix; an AC that assumes a code fix would be wrong.
  **Acceptance criteria:**
  ...
  -> traceTree.linksResolveOrHide [uc:uuid:b6d381d8-409b-4048-bb77-d1f44088ba48]

- [ ] **R40.73 — Unreachable methods — 3 methods are in no Class, a user cannot navigate to them at all @390**
  [requirement:uuid:04dc74bd-8447-4a5e-bf6c-fff3659a91fd]
  USER-VISIBLE DEFECT (architect prod sweep, served v0.8.150). 3 Method units are in NO Class.methods[] array, so they are INVISIBLE in the tree — a user cannot navigate to them from their Class at all (no path exists). MEASURED: robbin-req reproduced 3 unreachable exactly. This is the navigation face of the forward-edge model (R37.32): a Method not forward-referenced by any Class has no chain path to render under.
  **Acceptance criteria:**
...
- [ ] **R40.76 — Deploy-integrity: the served==committed / version guard covers only the SERVER half — client dist serves statically and goes live unversioned on build (guard-coverage gap, DEFERRED)**
  [requirement:uuid:e57df4b4-dcd3-4aee-be87-dbc717203922]
  DEPLOY-INTEGRITY FINDING (PO capture 2026-08-31, DEFERRED pending Tron). The version-bump / served==committed / phantom guard protects only the SERVER HALF of a deploy. MECHANISM (PO/expert explanation, almost-certainly): dist is BUILT INTO THE WORKTREE and served STATICALLY, so CLIENT changes go live the instant they are BUILT (no restart needed) while SERVER changes wait for a RESTART. OBSERVED (PO/tester-measured): the 'source not available' CLIENT marker rendered on served 0.8.151 while the expert had that marker in the HELD v0.8.152 = prod was briefly serving 0.8.152 CLIENT code under a 0.8.151 VERSION STRING. So the version string + phantom guard track the SERVER half; a client change can be live-and-unversioned. The restart to v0.8.152 resolved THIS instance (PO-measured). ★ STATE IT AS WHAT IT IS: a GAP IN GUARD COVERAGE (the guard silently covers only half of what it claims), NOT a bug in anyone's work. A guard that covers half of what it claims = phantom-coverage one level up (R37.31 family). DEFERRED; nobody fixes now (functionality first); written down while we understand it.
  **Acceptance criteria:**
  - [ ] **(deploy-integrity)** The served==committed / version guard covers BOTH the server AND the client half of a deploy: a client change live in dist under an old version string is DETECTED, not silently unversioned. Today it covers server-only.
  - [ ] **(deploy-integrity)** The served CLIENT bundle's version is measurable/asserted independently of the server version, so 'client 0.8.152 under server 0.8.151' surfaces rather than reading as 0.8.151 everywhere.
  - [ ] **(honesty)** Recorded as a GUARD-COVERAGE GAP (the guard's claim overstates its coverage), NOT a defect in the expert's deploy or anyone's work. The v0.8.152 restart resolved the instance; the gap is structural.
  - [ ] **(deferred)** DEFERRED pending Tron scheduling; no work starts until scheduled (functionality-first).
  - [ ] **(discovered-not-hand-listed)** The gap is evidenced by measuring served-client-marker vs served-server-version (as PO/tester did), not asserted; a client/server version skew is measurable.
  -> deployGuard.coversClientAndServerHalves [uc:uuid:424d6aac-1fc9-45d3-91be-d8df5e78552e]

- [ ] **R40.77 — Virtual puml collection ALSO surfaces the real on-disk physical-folder tree beneath it (physical folders as scenario-units) — T37.21 part 5**
  [requirement:uuid:70c6b806-d4be-4dc9-b23f-cfd61c8c7eea]
  T37.21 part 5 (Tron verbatim): 'we see the puml files under puml but not in the physical folders as they are. puml is a virtual collection with no physical folder - below I want the folder paths where the real puml files are, as a tree of physical folders as scenario-first units.' The puml collection is VIRTUAL (no physical dir). MEASURED (robbin-req re-derived vs disk, not the transcription): class-diagram.puml appears 4x across 4 DISTINCT physical folders (sprint-02/08/09/31 .../diagrams), use-cases.puml 3x - the SAME name in DIFFERENT physical folders = exactly the information the physical tree must reveal (the dup names Tron saw). KEEP the virtual collection; BENEATH it surface the actual on-disk folder PATHS as a tree of PHYSICAL folder scenario-units (rides R40.16 folder-as-unit, NO duplicate folder model).
  **Acceptance criteria:**
  ...
  -> pumlCollection.surfacePhysicalFolderTree [uc:uuid:c0717060-ed50-443a-90fe-32867761779e]

- [ ] **R40.78 — Add-folder ALSO inside a room Files collection (nested room folders) — needs a defined per-room on-disk storage target (OPEN design question)**
  [requirement:uuid:c638d72c-17ce-4863-b939-a00aa09e6c30]
  Tron chose the WIDER Add-folder scope (PO asked directly 2026-09-01): the Add-folder action appears NOT ONLY on real on-disk folders but ALSO INSIDE a room Files collection, creating NESTED room folders. This is genuinely NEW capability beyond T37.21 part-2 (mkdir on the /model side). ★ OPEN DESIGN QUESTION, stated plainly (NOT hidden): a room Files collection is VIRTUAL today with NO location on disk to mkdir into — so WHERE ON DISK does a room nested folder live? The architect must define a PER-ROOM STORAGE TARGET before this is buildable. Once the target is defined, Add-folder-in-room follows the SAME Tron BOTH ruling as part 2 (mint the unit AND mkdir the real directory at that target) + the same live-MVC + WS-fan-out.
  **Acceptance criteria:**
  ...
  -> roomFiles.addNestedFolder [uc:uuid:65b07a49-a33f-4888-9ca3-d8464ddd78fb]

- [ ] **R40.81 — ONE physical unit store; every other index/view is a symlink tree into it (a duplicate physical store is forbidden)**
  [requirement:uuid:be8ec6b6-da9c-41ae-8a45-bdc865baa5d0]
  Tron architectural directive verbatim (relayed via PO 2026-09-05): he does NOT want a data model-store; units can have ln links in redundant index trees, so we can have a model-tree FOLDER but NOT a duplicate index. THE PRINCIPLE: ONE physical store of unit files (scenario/index), every other index or view is a SYMLINK tree into it, a duplicate PHYSICAL store is forbidden. MEASURED current state: scenario/index = 5947 real files / 0 symlink (the one store); data/model-store = 777 real files / 0 symlink (a 2nd physical store = the violation); 33 UUIDs are real files in BOTH (byte-identical now, no divergence yet, NO rule for which wins if they do); 744 model-store units are model-store-ONLY (so 'delete the dupes' alone leaves 744 = rule half-applied). Correct shape already exists elsewhere: data/users room-file dirs are 53/53 symlinks into scenario/index (+ R17.5 speaking-name tree, sprints.json). The gate that FAILS on a duplicate real file for an existing UUID is the by-construction guarantee.
  **Acceptance criteria:**
  ...
  -> unitStore.assertSinglePhysicalStore [uc:uuid:ea8c3cf8-8d96-429d-9eba-e9f8f5a866b4]

- [ ] **R40.82 — Folder owns its children through a per-type parent-children interface (ONE canonical derivation, tree+detail same instance)**
  [requirement:uuid:eb7b086d-dee4-4a7a-ba97-a83ad372836a]
  Tron root lens 2026-09-05 (via PO): what belongs to the object lives ON the object, data AND behaviour. Children are answered BY the object through a per-type parent-children interface (node.children()). Folder.children() is the SINGLE owner of its answer; BOTH the tree and the detail render from node.children() on the SAME instance; NO surface derives children by any other means. The measured defect this fixes: the tree and the detail each derived children THEIR OWN WAY = a duplicate derivation = the exact duplication the traceability graph exists to make impossible/visible. This is the fix for the defect Tron can SEE (his folder is still invisible) and everything else waits behind it. Same law as R40.81 one-physical-store: data (store) and behaviour (children) both belong ON the object.
  **Acceptance criteria:**
  - [ ] **(by-construction)** EVERY node type implements the per-type parent-children interface (node.children()). A type that can contain children but derives them off-interface => RED. This is the OOP shape: children are answered BY the object, per type. ★ DECISIVE add-a-new-type test (LAW-9 case-3): adding a new type must NOT require editing a resolver/central switch — a new type-CLASS owns its own children() method. If adding a type edits a resolver => VIOLATION (resolver owns what the type should). PO shape ruling: per-type form, tree asks the OBJECT, no resolver switch.
  - [ ] **(by-construction/ONE-canonical)** Folder.children() is the SINGLE owner of its answer — the canonical derivation of a Folder's children lives ON the Folder, exactly once. A SECOND derivation of a Folder's children anywhere in the codebase => RED (the duplication the traceability graph exists to stop; per the root lens, a 2nd derivation is a TRACEABILITY DEFECT not paperwork).
  - [ ] **(render/ONE-derivation/DEVICE-390)** BOTH the tree AND the detail render from node.children() on the SAME instance — one derivation, not two parallel ones (the measured duplicate-derivation bug: tree and detail each deriving children their own way). Screenshot-checkable @390: the children shown in the tree == the children shown in the detail, sourced from ONE node.children() call on ONE node instance. Divergence => RED.
  - [ ] **(by-construction/THE-GUARD/traceability-query)** THE failable guard (a TRACEABILITY QUERY, per the root lens = the system USED as DRY enforcement, not a ledger): NO surface derives children by ANY means other than node.children(). A surface that walks/queries/computes children off-interface => RED. This is the by-construction guarantee that one-derivation cannot be bypassed. ★ AUTOMATIC-FAIL RULING (Tron 2026-09-05): the per-site test is NOT did-the-raw-fetch-disappear but DOES THIS SURFACE ASK AN OBJECT FOR ITS OWN CHILDREN (node.children()). Routing surfaces through a SHARED FETCH/CHILDREN HELPER (import a function, pass it a ref) is a DEFECT BY DEFINITION (a functional construct owning what the object should own = a 2nd place the truth lives) and is an AUTOMATIC FAIL even if the raw-fetch guard goes GREEN. The helper-escape does NOT satisfy this AC.
  - [ ] **(acceptance/DEVICE-390)** CUSTOMER EXPERIENCE / ACCEPTANCE (Tron @390), NOT verification: after the fix, Tron EXPERIENCES his folder VISIBLE in tree + detail. This is his ACCEPTANCE (sign-off on delivered verified work), NOT a test he is asked to run and NOT our proof. STANDING LAW (Tron 2026-09-05): verification of the real user path is COMPLETE by a FIXED HARNESS BEFORE he sees anything; cannot-verify = DO-NOT-SHIP, never ship-and-ask-him. If the real-path harness gap blocks verification, ESCALATE THE GAP with a fix proposal (tester real-room-reach), never as a request for Tron to confirm/re-try/check. ★ AND render-green != ownership: his seeing the folder (behavioural) is a SEPARATE signal from structural ownership (12->0); neither substitutes.
  - [ ] **(by-construction/tier-split)** The client tree ONLY FETCHES the one Model-tier children derivation (RoomFolder.childrenByLocation / roomFilesChildren endpoint) and reconciles the DOM incrementally — it NEVER computes room children LOCALLY (no client-side files[]/location logic). A client-side room-children derivation => RED (that WOULD re-introduce the R40.83 dual-face divergence: two derivations of the same children). ONE source (Model/server), the View reflects, never recomputes. Architect-ruled legit-by-construction 2026-09-05.
  -> node.childrenThroughInterface [uc:uuid:58fb8080-c6ef-423c-bbce-8173dcf99b62]

- [ ] **R40.83 — Room folder dual-identity resolves to BOTH faces (contents + verbs) from one selection — no split-identity broken half**
  [requirement:uuid:107b28dc-41e2-4a51-9f36-74b658422a2e]
  DEFECT FAMILY (tester-measured root, Tron saw the prod face): a room folder has TWO identities — a real folder UUID (resolved by /api/ior) and a roomcoll LOCATION REF (resolved by /api/trace/children) — and NEITHER api resolves BOTH. So whichever ref the tree uses, one half of the detail breaks. TWO FACES of ONE root: prod 0.8.174 shows the verbs with an EMPTY BODY (children unresolved by real uuid; the face Tron has SEEN); scratch-with-fix shows the nesting with NO VERBS (unit unresolved by roomcoll ref). FIX: one selection resolves BOTH faces (unit->verbs incl add-folder AND children->contents) via ONE canonical folder-identity resolution. User terms (Tron): selecting a folder shows its contents and lets you add a folder inside it.
  **Acceptance criteria:**
  - [ ] **(by-construction/ROOT)** ROOT FIX: selecting a room folder resolves BOTH faces from ONE selection — the real folder UUID (its unit -> verbs incl add-folder) AND its children (its contents). Neither /api/ior-by-real-uuid ALONE nor /api/trace/children-by-roomcoll-ref ALONE is sufficient. A resolution that yields only ONE face => RED.
  - [ ] **(render/prod-face)** The face Tron SAW on prod 0.8.174: selecting a folder shows verbs with an EMPTY body (children unresolved by real uuid). AC: a folder with children shows its CONTENTS, not an empty body. Empty-body-on-a-folder-that-has-children => RED.
  - [ ] **(render/scratch-face)** The other face (scratch-with-fix): nesting shows but NO verbs (unit unresolved by roomcoll ref). AC: a selected folder shows its verbs (incl add-folder). No-verbs-on-a-resolved-folder => RED.
  - [ ] **(acceptance/USER-TERMS)** Tron in user terms: selecting a folder SHOWS ITS CONTENTS and LETS YOU ADD A FOLDER INSIDE IT — both together, from one selection. This is the user-facing acceptance; both halves work at once, not one-or-the-other.
  - [ ] **(by-construction/root-lens)** ROOT LENS (DRY): a room folder has ONE resolution serving BOTH faces (contents + verbs), NOT two APIs each keyed to a different ref that can diverge. The dual identity (real uuid + roomcoll location ref) is UNIFIED into one canonical resolution; the tree does not pick a ref that breaks one detail half. A second/divergent identity resolution => RED. The split identity IS the defect (two places the folder identity lives, they diverge into two broken faces).
  - [ ] **(stub-must-fail)** The exact two-face split must trip RED BOTH ways: (a) a folder resolvable by real uuid (unit/verbs) but NOT by roomcoll ref (children) => RED (prod face); (b) a folder resolvable by roomcoll ref (children) but NOT real uuid (unit/verbs) => RED (scratch face). A suite green on either split is inadmissible.
  - [ ] **(verify/customer-not-tester)** WE verify @390 real-WebKit in a live TEST-MEMBER room (member-gated, tester gate r4022, no owner rights): selecting a folder shows contents AND add-folder works. Tron ACCEPTS delivered verified work — he SAW the prod empty-body face, which is OUR verification failure, not a step we ask him to run. NOT Tron-verifies.
  -> roomFolder.resolveUnitAndChildren [uc:uuid:87be8da5-823c-4195-b30b-c4273c7fa4c0]

- [ ] **R40.84 — Adding a child updates that node in place — the tree must not collapse or rebuild wholesale (MVC, expanded state survives)**
  [requirement:uuid:6149c2d3-87d0-41d6-a67f-175e2bab5ad8]
  Tron defect on 0.8.175 (via PO): adding a child to a folder causes a FULL TREE COLLAPSE + wholesale RERENDER instead of updating just that node in place — his words, far from MVC. Architecture violation, NOT a UX-polish item: the unit IS the MVC model, so adding a child should update THAT node children IN PLACE (controller mutates the model node, the view re-renders just that node via the ONE VIEW BUS); the tree must NOT collapse and must NOT rebuild wholesale; any expanded state must survive. User terms: when I add a folder the tree stays exactly where it was and only the folder I added into changes.
  **Acceptance criteria:**
  ...
  -> folderTree.addChildInPlace [uc:uuid:973f66af-e615-43dd-ab5e-a44786298ded]

- [ ] **R40.85 — Plain file upload via the drop area succeeds (0.8.175 REGRESSION: recognition works then upload dies) — all uploads**
  [requirement:uuid:deeeec90-568c-4019-8163-0c2dfd460741]
  Tron 0.8.175 (via PO): dropping a file on the DROP AREA — dnd debug shows the png recognized as image/png then Upload FAILED, while folder-add in the same room works. Scope = ALL uploads (not a folder case). A REGRESSION from our deploy = US breaking something, NOT a missing capability. User terms: dropping a file on the drop area uploads it and the file lands in the room. NOTE (PO): distinct from R40.86 (folders-as-drop-targets feature) — do NOT conflate; this is the regression, that is the capability.
  **Acceptance criteria:**
  ...
  -> dropArea.uploadFile [uc:uuid:2640f986-ec15-4579-89f3-807158239a5a]

- [ ] **R40.86 — Folders are drop targets — dropping content onto a folder places it inside that folder**
  [requirement:uuid:a7bd184b-af45-47c1-a8f7-2488bde5f71d]
  Tron INTENT (capability, does NOT exist today): folders SHOULD BE drop targets; dropping content onto a folder places it INSIDE that folder. Currently folders are NOT drop targets (Tron dropped on the drop area, not a folder — see R40.85 regression). Frame as the BEHAVIOUR (folders accept drops + the dropped content lands in that folder), NOT merely fix-the-upload. Same CONTAINMENT theme as R40.83 (room-folder identity) + R40.84 (in-place add). User terms: when I drop content onto a folder, it lands inside that folder.
  **Acceptance criteria:**
  ...
  -> folder.acceptDrop [uc:uuid:af1bf20b-9000-45db-9c55-9d93f4822a7c]

- [ ] **R40.87 — Add folder SUCCEEDS on a model collection (route to model-store when no physical dir) — truthful applicability, offered implies succeeds**
  [requirement:uuid:2afa97a9-245e-4384-a510-7311bbd4de2d]
  Tron ruling (reported MULTIPLE TIMES; architect design 5cfa4f9dd): Add folder must SUCCEED on a model collection like diagrams, NOT be suppressed — suppressing recreates the sometimes-a-button behaviour he explicitly rejected. ROOT: the model endpoint routes EVERY add-folder through the PHYSICAL create path, which returns the error bad-parent-loc for a parent with no real directory (the uncovered THIRD bad-parent-loc cause = a virtual Folder with no physical dir). FIX is ROUTING: branch to the MODEL-STORE create when the parent has no physical dir (both paths already exist). DOCTRINE (the valuable part): a Folder unit is a MODEL OBJECT, not inherently a directory; we assumed folder == directory and THAT assumption is the defect. TRUTHFUL APPLICABILITY: if we offer a verb it must succeed — offered implies succeeds by construction. The literal error Tron sees: bad-parent-loc.
  **Acceptance criteria:**
  ...
  -> addFolder.routeByParentPhysicality [uc:uuid:abac573a-9525-4d73-8273-a0765058d4bf]

- [ ] **R40.88 — Standing rulings are FAILABLE INVARIANTS — every ruling carries a guard that trips RED when a later feature contradicts it (extends R40.54 to the ruling level)**
  [requirement:uuid:b118f2c1-7ff5-4adc-a9a7-77e25a192ec2]
  PO ruling 2026-09-05 (the most valuable thing today; stops the WHOLE CLASS, not one instance): a standing ruling that lives only as PROSE is a REMINDER, and a reminder can be silently CONTRADICTED by a later feature with nothing to trip. That is the reminder-not-mechanism / false-satisfaction disease ONE LEVEL UP — at the RULING level (same family as knowing-a-rule-is-not-the-guard, the mechanical step is). ROOT (worked, on disk): R40.37 RULED folder!=directory on 2026-08-12; R40.78 (room physical folders) RE-INTRODUCED mkdir/physical-dir; NO guard caught the contradiction -> the recurring bad-parent-loc Tron reported multiple times. CURE: every standing ruling carries a FAILABLE INVARIANT (a CI gate that goes RED on contradiction), the guard itself is stub-must-fail (cannot certify nothing), and it is ONE mechanism shared with the trainer/ARON doctrine (not two). Extends R40.54 (failable ACs) to failable RULINGS.
  **Acceptance criteria:**
  ...
  -> ruling.enforceFailableInvariant [uc:uuid:669cd55d-dbb3-4ee2-a1ff-a585f8ff58d1]

- [ ] **R40.89 — No real iOS Safari test capability — acquire a real iOS Safari path (device or cloud service); Playwright WebKit (Linux) misses iOS-Safari-version-specific bugs**
  [requirement:uuid:893e78df-5cf6-4d1d-baec-f13fd501585d]
  CAPABILITY GAP (tester-named, PO 2026-09-05; a real weakness, not a one-off): WE HAVE NO REAL iOS SAFARI TEST CAPABILITY. Playwright WebKit IS Linux WebKit and does NOT carry iOS-Safari-version-specific bugs — this exact service-worker STREAMED-BODY DROP was INVISIBLE to it. CONSEQUENCE (stated honestly): iOS-only defects will keep reaching us through TRON DEVICE rather than through our gates, and any iOS-specific AC we write can only ever be CODE-PROOF + SIGNATURE-MATCH, NEVER device-verified BY US. That is a GAP IN WHAT OUR GREENS CAN MEAN, so it belongs on the RECORD, not in someone memory. FRAME: acquire a real iOS Safari path (a physical device OR a cloud device service). This is WHY Tron keeps finding what we cannot.
  **Acceptance criteria:**
  - [ ] **(capability/target)** A real iOS Safari test path EXISTS — a physical iOS device OR a cloud device service — that runs our gates against ACTUAL iOS Safari (version-specific), NOT Linux/Playwright WebKit. This is the standing acquisition TARGET; until it lands, the honesty guards below hold.
  - [ ] **(failable/false-green)** Playwright WebKit == LINUX WebKit and does NOT carry iOS-Safari-version-specific bugs. A gate or AC claiming iOS-Safari coverage VIA Playwright WebKit => RED (false-green). WORKED EXAMPLE: the service-worker streamed-body drop was INVISIBLE to Playwright WebKit and reached us only through Tron device.
  - [ ] **(failable/third-state/R40.88-instance)** UNTIL the capability is acquired, every iOS-SPECIFIC AC is labeled HONESTLY = CODE-PROOF + SIGNATURE-MATCH ONLY, NEVER device-verified-BY-US (deferred-with-the-gap-named, the third honest state). An iOS-specific AC claiming device-verification-by-us WITHOUT a real-iOS-path => RED (false-claim). This is an R40.88 instance: a ruling about what our iOS greens CAN MEAN, carrying its own failable guard.
  - [ ] **(process/one-home)** iOS-only defects reaching us THROUGH TRON DEVICE (not our gates) are ROUTED to this standing item (tracked), NOT treated as one-offs. This is the recurring-report-gets-ONE-home rule applied to the iOS capability gap: it is WHY Tron keeps finding what we cannot.
  -> iosSafari.realTestPath [uc:uuid:35031bf7-1a71-4533-b84d-bda828866e59]

- [ ] **R40.90 — expandPath waitForNode TIMEOUT on the room ROOT node (tester-observed, un-owned symptom — tracked defect)**
  [requirement:uuid:393e297a-238e-4d89-85f3-d8afa5e43ce3]
  TRACKED DEFECT (PO 2026-09-05, do-not-lose): the tester observed REPEATEDLY during R40.84-B that expandPath waitForNode TIMES OUT on the room ROOT node. It did NOT affect the R40.84 verdict (folders render via the reDerive IN-PLACE path, Impl 8693dc2b, NOT expandPath) but it is a REAL UN-OWNED SYMPTOM and must not be lost. SYMPTOM-ONLY: the root is not yet owned; captured scenario-first as its OWN req with the literal symptom text (intake-search finds it). Do NOT attach to R40.84 (distinct code path). Literal symptom: expandPath waitForNode TIMEOUT on the room ROOT node.
  **Acceptance criteria:**
  ...
  -> expandPath.resolveRoomRootNode [uc:uuid:bf89dd5c-4835-4066-a2e2-9531e21d6cc4]

- [ ] **R40.91 — Exactly ONE unit-changed->notify translator — a 2nd drifted copy is the R40.84-B 4-round defect**
  [requirement:uuid:929a5117-b30b-4aec-9f50-d9de74e61cbd]
  Architect guard design (d5228cf40, PO-queued after R40.84-B): there must be EXACTLY ONE unit-changed -> notify TRANSLATOR (the owner = notifyUnitChanged). A 2nd site that builds the notify key INLINE on a msg.type==="unit-changed" match (instead of delegating to notifyUnitChanged) is a DRIFTED COPY = the R40.84-B 4-round defect (the second translator diverged, causing the 4-round re-render). The guard = check:one-unit-changed-translator: scan the NARROW hazard (inline-key-build on unit-changed), legit ViewBus.notify(viewBusKey) emits NOT flagged (0-noise). Single-owner by construction, same family as R40.82 (children-single-owner) + R40.88 (no-mkdir); an R40.88 instance (a ruling enforced by a failable guard with a per-run selftest).
  **Acceptance criteria:**
  ...
  -> notifyTranslator.singleOwner [uc:uuid:089da882-48b1-4a10-8442-683d6e1f9276]

- [ ] **R40.92 — Add folder to a model collection succeeds but the folder never appears**
  [requirement:uuid:6009a5ad-a39a-4b77-932e-0d597c897bdf]
  Tron adds a folder to a MODEL COLLECTION (e.g. diagrams) — it mints successfully but appears NOWHERE. ROOT (measured in code): the collection children derivation at server.ts:1789 (rawbin:diagram) returns ONLY ior:class:Diagram units and NEVER Folder units; and a store-only model Folder (mintRealUnit) carries parent=rawbin:diagram but NO location, so the location-based dir: merge at server.ts:1780 cannot find it either. Both paths exclude a store-only Folder parented under a model collection. SAME SHAPE AS R40.84-B / R40.87: succeeds INVISIBLY — offered<=>succeeds<=>visible is violated. c83c02f2 is only HALF fixed from Tron's point of view: the error stopped, but he still SEES NOTHING when he adds a folder to diagrams. FIX: the derivation includes parent-linked (parent=rawbin:<collection>) store-only Folder units so the folder RENDERS in the collection after add + persists.
  **Acceptance criteria:**
  ...
  -> collectionChildren.includeParentLinkedFolders [uc:uuid:fe9a2e26-3809-4155-ba2a-8adb43138f62]

- [ ] **R40.93 — Room-folder physical create routes through the ONE owner (createPhysicalFolder), not a raw inline mkdir**
  [requirement:uuid:e0c95904-80b9-481f-bb0a-38d4bc425623]
  server.ts:2565 creates a ROOM folder via a RAW mkdir ('mkdirSync(filesBase,{recursive:true}); mkdirSync(target)') instead of going through the ONE physical-folder owner FolderService.createPhysicalFolder (RoomFilesService already gates room-physical at server.ts:47). It is LEGIT room-physical-by-construction (NOT a model-folder hazard), currently INFRA_ALLOW-listed so the guard is GREEN — so NOT blocking. But it is a SINGLE-OWNER violation of the shape Tron cares most about: ONE canonical owner for physical-folder creation; duplicate physical-create paths ARE the defect (same family as R40.81 one-store / R40.91 one-translator two-source). FIX: the room-folder create routes through the owner; on fix the INFRA_ALLOW entry for server.ts:2565 COMES OUT — the guard stays GREEN by ROUTING, not by LISTING. Was HIDDEN until R40.88 e4 close (3f9a2d309) removed the recursive:true that masked it.
  **Acceptance criteria:**
  ...
  -> roomFolder.createThroughSingleOwner [uc:uuid:a638e7fb-7590-4a13-a313-e9fb738550d4]

- [ ] **R40.94 — scoreboard() throws on undefined.length — the chain-scoreboard roll-up cannot run**
  [requirement:uuid:f615ac30-479a-4401-a62e-5860ad0e5d13]
  The chain-scoreboard summary function scoreboard() THROWS on undefined.length when invoked, so it cannot produce the N/M roll-up. Surfaced by the skill-expert while delivering the R40.84 measurement. This is the summary Tron would read to see his OWN totals: the per-chain credit is now CORRECT (label false-open fixed 8693dc2b, isolated +1), but the ROLL-UP that aggregates it cannot run. Root is UNDIAGNOSED/UNOWNED (captured symptom-first, do-not-diagnose). DISTINCT from the R40.84 label false-open (strict-AST reading the requirement-id as the method-token, fixed) and from the false-open scorer bug. Tooling = scripts/campaign-scoreboard.mjs / campaign-scoreboard-region.ts family (scoreboard requirement R30.11 measures the walked chain; R24.3 chain-scoring skill).
  **Acceptance criteria:**
  - [ ] **(symptom/by-construction)** Invoking scoreboard() to produce the N/M chain-scoreboard ROLL-UP summary COMPLETES WITHOUT THROWING — no 'cannot read .length of undefined'. The roll-up runs to aggregate the (already-correct) per-chain credit into totals.
  - [ ] **(impact/why-it-matters)** The chain-scoreboard roll-up is the summary Tron would read to see his OWN totals (N/M hops across chains). Per-chain credit is now correct (label false-open fixed, 8693dc2b, isolated +1), but the aggregate cannot be produced until scoreboard() runs — so Tron cannot see the totals.
  - [ ] **(self-failability)** The fix is proven by a gate that FEEDS the input shape which currently throws (a unit/collection whose scanned field is undefined) and asserts scoreboard() returns a roll-up instead of throwing; run against the pre-fix code it reproduces the throw (RED). A gate that cannot reproduce the throw is not proving the fix.
  - [ ] **(provenance/honest)** Captured SYMPTOM-FIRST with the literal error (scoreboard() throws on undefined.length when invoked); root is UNDIAGNOSED/UNOWNED (do-not-diagnose per PO). DISTINCT from: (a) the R40.84 label false-open (strict-AST read the requirement-id as the method-token; FIXED 8693dc2b, now credits +1 isolated) and (b) the false-open scorer bug. This is the ROLL-UP invocation throwing, a separate tooling defect. NOT attached to R40.84.

- [ ] **R40.96 — REST transport is the scenario unit JSON — internal content moves as an idempotent unit-JSON PUT by uuid, never multipart**
  [requirement:uuid:d7deea49-398e-4ed4-9602-c45fccce172e]
  THINK WHAT REST MEANS: REpresentational State Transfer — the representation of state IS the scenario unit JSON, so every internal transfer carries unit JSON and NOTHING ELSE. In-app content transfers as an IDEMPOTENT unit-JSON PUT by uuid (same uuid + same content = no-op), NOT multipart, NOT a bespoke payload. Deletes the hand-rolled transport layer that caused the week-long outage.
  **Acceptance criteria:**
  - [ ] **(cross-cutting/transport)** CROSS-CUTTING (planner constraint on every req in this set): the scenario-unit JSON is the model AND the only payload over REST or any transport (except the single NativeFileIngress edge, R40.97). No bespoke/per-target payload.
  - [ ] **(by-construction)** In-app content transfer = a unit-JSON PUT keyed by uuid; re-PUT of the same uuid+content is a no-op (idempotent), not a duplicate.
  - [ ] **(gate)** No internal transport (REST route, ws, bus) carries a non-unit-JSON payload.
  - [ ] **(self-failability)** Seed a bespoke/multipart internal payload -> the scan goes RED (own stub, isolated R40.31).
  -> unitTransport.putByUuid [uc:uuid:06444973-7f6f-4bb4-81c0-2b23715f8d79]

- [ ] **R40.97 — NativeFileIngress is the ONE edge that owns native-file multipart entry + content-type param parse, converting to a unit immediately**
  [requirement:uuid:4d6e701b-4750-4e51-8992-9790e55ce817]
  Tron's exemption, encoded exactly: native files DO come in as multipart binaries (Finder/picker) — but ONLY at ONE ingress boundary, converted to a scenario unit IMMEDIATELY, never propagating inward. That single owner (NativeFileIngress) ALSO owns the content-type parameter parse (dequote boundary / strip params) ONCE — which makes the boundary-outage fix PERMANENT BY CONSTRUCTION (no second place parses content-type, so no second place can re-break it).
  **Acceptance criteria:**
  - [ ] **(by-construction)** Exactly ONE boundary (NativeFileIngress) accepts native-file multipart entry and emits a scenario unit; the multipart representation never propagates past it.
  - [ ] **(by-construction)** The content-type parameter parse (dequote boundary / strip params) lives ONLY inside NativeFileIngress — the outage fix is permanent because no second parser exists to re-break.
  - [ ] **(gate/scan-hazard)** Any multipart / boundary / content-type-string operation OUTSIDE NativeFileIngress == 0.
  - [ ] **(self-failability)** Seed a multipart parse outside NativeFileIngress -> RED (own stub, isolated).
  -> nativeFileIngress.toUnit [uc:uuid:15f3d041-cad3-4e2c-a087-77b52a852834]

- [ ] **R40.98 — Binary content is represented WITHIN the unit — base64 inline on the wire, .content sidecar + sha256 hash-dedup at rest**
  [requirement:uuid:97ce5ac6-3c7c-408b-81fe-1081c3370c48]
  Binary is carried WITHIN the scenario unit, not as a separate payload: base64 inline on the wire; at rest, the existing .content sidecar + a sha256 content-hash that DEDUPS identical bytes (same hash = one stored blob, no-op re-store). The natural class owns the toUnit/fromUnit conversion (R40.99 interface).
  **Acceptance criteria:**
  - [ ] **(cross-cutting/transport)** CROSS-CUTTING (planner constraint on every req in this set): the scenario-unit JSON is the model AND the only payload over REST or any transport (except the single NativeFileIngress edge, R40.97). No bespoke/per-target payload.
  - [ ] **(by-construction)** On the wire, binary is base64 inline WITHIN the unit JSON — never a separate multipart part.
  - [ ] **(by-construction)** At rest, identical bytes dedup by sha256 (one stored blob, re-store of same hash is a no-op) via the existing .content sidecar.
  - [ ] **(by-construction/OOP)** The natural class owns toUnit/fromUnit — no free binary-transport function (functional-construct = defect).
  - [ ] **(self-failability)** Seed two identical-byte stores -> exactly one blob (else RED).
  -> binaryUnit.base64AndHashDedup [uc:uuid:0e4944d6-de41-46b1-97e1-7e35b6ea668a]

- [ ] **R40.99 — Natural domain classes implement UnitConvertible {isBinary/load/toUnit/fromUnit} — ask the object, no free transport function**
  [requirement:uuid:92a5d0d4-0e4a-4a0c-bd4b-6e97b9717fa5]
  > Tron 2026-09-06 verbatim: "there is no fucking shapes... only in diagrams... all other are scenario-units as models in class instances which are mvc objects".
  Every natural domain class (WebItem, Email, Contact, Image, CalendarEntry) implements ONE interface UnitConvertible {isBinary, load, toUnit, fromUnit} and OWNS its own conversion — ask-the-object, never a free function/service (a functional construct is a defect by definition). Dispositions: REUSE WebItem 7c486fcb + Email 3bb26ebe (complete the 4 methods + Impls); RECONCILE Contact onto bf440a63 (Contact is the natural class, VCard is the format-lens — rename display-only uuid-stable, blast-radius attached); MINT Image + CalendarEntry (absent, verified). 0 duplicates.
  **Acceptance criteria:**
  - [ ] **(cross-cutting/transport)** CROSS-CUTTING (planner constraint on every req in this set): the scenario-unit JSON is the model AND the only payload over REST or any transport (except the single NativeFileIngress edge, R40.97). No bespoke/per-target payload.
  - [ ] **(by-construction/OOP)** Each natural class implements UnitConvertible {isBinary, load, toUnit, fromUnit} and owns the conversion; a free transport function for a type => RED (ask-the-object).
  - [ ] **(reuse/check-before-create)** WebItem 7c486fcb + Email 3bb26ebe are REUSED (complete their 4 methods + Impls) — NOT re-created (a 2nd class = the dup-defect).
  - [ ] **(reconcile/blast-radius)** Contact is the natural class; VCard is the FORMAT-LENS. Reconcile onto bf440a63 (uuid-stable), RENAME display-only 'VCard'->'Contact', keep VCard as a format facet. BLAST-RADIUS measured + attached (blastRadius field): rename class-identity refs, KEEP format-lens refs; flag any class-by-name resolution BEFORE the flip.
  - [ ] **(mint/check-before-create)** Image + CalendarEntry are MINTED as natural classes (verified absent) with the full UnitConvertible chain; re-check-before-create at build (Image!=File-facet, CalendarEntry!=ical-handler already covering).
  - [ ] **(by-construction)** 0 duplicates among the 5 natural classes (a dup = the traceability defect).
  - [ ] **(radical-oop/mvc)** Each natural class (WebItem/Email/Contact/Image/CalendarEntry) is a CLASS that OWNS its data AND behaviour = model+view+controller IN the object: it LOADS itself, STORES itself, and RENDERS ITSELF. There is no shape/handler outside it that renders it (shapes exist only in diagrams).
  - [ ] **(radical-oop/no-switch)** A handler/function that SWITCHES on a mime/format string to decide behaviour IS THE DEFECT (not an implementation detail). Behaviour is dispatched by asking the OBJECT (MimeType.from -> the natural class), never a switch on the format string.
  - [ ] **(radical-oop/dod)** DEFINITION OF DONE for each: the RIGHT CLASS is instantiated AND rendering itself. "bytes stored" (a payload persisted without the natural class instantiated + rendering itself) is a FAIL, not partial progress.
  - [ ] **(radical-oop/layering)** The dispatch LAYERS: DndContract -> MultipartMime -> MimeType.from -> natural class. Each layer hands to the next; the natural class is the terminus that owns load/store/render. No layer switches on format to render.
  -> naturalClass.unitConvertible [uc:uuid:f4b10768-caa2-4065-82d6-cd9e8d103ad6]

- [ ] **R40.100 — Self-heal emerges BY CONSTRUCTION from the OOP model — retire the functional 419 handshake**
  [requirement:uuid:3b205cfb-e538-4692-9999-8fe0d9ca2b1d]
  Tron: 'we screwed it functional; reevaluate it OOP, then it CANNOT NOT self heal.' Self-healing is NOT a handshake — it EMERGES from the OOP model: an object owns its state + an idempotent unit-JSON PUT-by-uuid (R40.96) + hash-dedup no-op (R40.98) means a re-sync is a safe no-op by construction, so drift self-corrects with no protocol. RETIRE the functional 419 self-heal handshake (Tron killed it; superseded in a85ea536a via architect REV2 1bd093d53). Do NOT mint the handshake. (DISTINCT from the MODEL-object self-heal of the Task 37.4 family, which STAYS.)
  **Acceptance criteria:**
  - [ ] **(by-construction)** Self-heal is a PROPERTY that emerges from idempotent PUT-by-uuid (R40.96) + hash-dedup (R40.98) + object-owns-state — NOT a functional handshake. A re-sync of the same uuid/content is a no-op.
  - [ ] **(retire/resolved)** The functional 419 self-heal handshake is RETIRED (Tron-killed). MEASURED (architect 2026-09-06, scanned all Requirement/UseCase units): NO standalone 419-handshake req/UC unit exists; a85ea536a is a DESIGN DOC ONLY (design-upload-stranded-sw-selfheal.md), superseded via banner = the FULL retirement. R40.100 is the by-construction REPLACEMENT. Nothing to mark superseded (no unit to touch). No handshake requirement is minted; a re-introduced handshake req => flag.
  - [ ] **(boundary)** This is DISTINCT from the MODEL-object self-heal (Task 37.4 family), which STAYS. Retiring the 419 handshake must not touch the 37.4 model self-heal.
  - [ ] **(self-failability)** Seed a divergent re-PUT -> converges to one unit (else RED); seed a re-introduced handshake -> flag.
  -> selfHeal.byConstruction [uc:uuid:dcafb763-80b0-402c-80ec-41ad499017ed]

- [ ] **R40.101 — iOS-shaped upload correctness — content-type boundary parameter handling (quoted boundary + charset params)**
  [requirement:uuid:6dcdcdf6-fbff-4bfc-8b2f-ab6cf624af6a]
  #126 BACKFILL, named as such: the single most critical fix this week — the iOS-shaped upload outage fix — shipped CHAIN-LESS because it arose as a Tron priority-override during a live outage instead of scenario-first, so it has NO covering requirement/task and the permanent gate r4090 (c9c6acb8a) is referenced by NO Test/Impl. THE REQUIREMENT: iOS-shaped upload correctness = content-type BOUNDARY parameter handling (a QUOTED boundary + trailing ;charset params) so a native-file upload parses on EVERY client. Root of the outage: content-type.split('boundary=') kept the iOS QUOTED boundary -> 0 parts -> 401/419. Re-point to BUILT REALITY (R30.11 pattern, NOT design-ahead): the shipped fix = server.ts boundary extraction, v0.8.190, commit 7cc4717fb (surgical dequote, body byte-unchanged). We close the rule-break rather than argue it.
  **Acceptance criteria:**
  - [ ] **(matrix/prod)** r4090's matrix all GREEN with byte round-trip, ON THE PROD SURFACE (not a worktree): V0 control / V1 file-first / V2 token-first / V3 quoted boundary / V4 quoted+charset params.
  - [ ] **(correctness)** A QUOTED content-type boundary (iOS) is dequoted and parsed to >0 parts (the exact outage: quoted boundary -> 0 parts -> 401/419 is fixed).
  - [ ] **(correctness)** Trailing ;charset (and other) parameters after boundary are stripped, not folded into the boundary value.
...
- [ ] **R40.104 — File/folder RENAME is the object's own behaviour — unit.rename(newName) overriding the derived name (displayName wins)**
  [requirement:uuid:069b1f8d-ec87-475c-8dee-a15cd0dcfd40]
  > Tron 2026-09-06: 'we need a file/folder RENAME action' + standing law 'the solution MUST BE OOP DRY MVC'. Rename = the user overriding the derived name ON the object.
  Tron directive 2026-09-06 (verbatim: 'we need a file/folder RENAME action'), framed by his standing law 'the solution MUST BE OOP DRY MVC'. RENAMING IS THE OBJECT'S OWN BEHAVIOUR: unit.rename(newName) — NOT a dialog with rename logic in it. The class already OWNS its name (Image names itself from capture time, Email from subject, Contact from full name, CalendarEntry from title+date, File from a clean basename); RENAME is the USER OVERRIDING the derived name ON the object. THREE distinct names: originalName (the raw input string we were wrongly displaying), derivedName (what the class computes for itself), displayName (what the user set — WINS when present). Nothing is lost; a rename never destroys the original. The user-override half of the naming design (R40.99 = the class owns its name; this = the user can override it).
  **Acceptance criteria:**
  ...
  -> unit.renameOverridesDerivedName [uc:uuid:168c7257-bd0c-49c1-8baf-113f132e27eb]

- [ ] **R40.105 — Each natural class OWNS its name — derives its own display name from its content (derivedName; displayName overrides, originalName preserved)**
  [requirement:uuid:fc201015-2fd8-4a05-b9b1-32e0355a130d]
  The DERIVATION half of the naming design (pairs with R40.104 rename = the OVERRIDE half). Each natural class COMPUTES ITS OWN display name from its content — Image from its capture datetime, Email from its subject, Contact from its full name, CalendarEntry from its title+date, File from a clean basename — the class owns its name, no external namer. THREE names: derivedName (what the CLASS computes), displayName (what the USER sets — WINS when present, R40.104), originalName (the raw input string — PRESERVED but NEVER shown as the name). Display resolves displayName ?? derivedName; originalName is never the displayed name. Scenario-first-retroactive: the expert is already building this (increment #3) — this req routes the in-flight work so it is traceable, not orphaned in code.
  **Acceptance criteria:**
  ...
  -> naturalClass.deriveOwnName [uc:uuid:e72ca5b1-1a91-4205-80de-e4b61054dbd3]

- [ ] **R40.106 — One DnD: every folder is a drop target, a drop places a LINK to the one stored unit; REMOVE unlinks, DELETE (red) removes the unit + all links**
  [requirement:uuid:ff9ed585-a928-46aa-b9ea-3f68c899f208]
  > Tron 2026-09-06: 'one DND for all folders as dropTargets everywhere. dropping something in either places a link to the centralized storage scenario unit always... no matter if file, image, task, modelElement or whatever oop object. we also need a remove button on every file in the action bar and a delete button that is red and really removes from the index and removes all links... (dangerous)'.
  Tron design statement 2026-09-06 (verbatim): 'one DND for all folders as dropTargets everywhere. dropping something in either places a link to the centralized storage scenario unit always... no matter if file, image, task, modelElement or whatever oop object. we also need a remove button on every file in the action bar and a delete button that is red and really removes from the index and removes all links... (dangerous)'. THE MODEL: (1) EVERY FOLDER EVERYWHERE is a drop target via ONE contract, no per-surface variants (room files / model tree / diagrams / traceability). (2) A DROP places a LINK to the ONE centralized storage unit — ALWAYS, never a copy — for ANY OOP object (file/image/task/modelElement/requirement/whatever). One unit, many links = identity by reference (same law as the one-store root R40.81). (3) REMOVE (on every file in the action bar) unlinks THIS link only; the unit + its other links survive. (4) DELETE (RED, DANGEROUS) really removes the unit FROM THE INDEX and removes ALL its links everywhere.
  **Acceptance criteria:**
  - [ ] **(drop/link)** LINK is the PRIMITIVE (=drop, Tron 'places a link ALWAYS'): a drop ADDS ONE EDGE (a container children[] ref) — never a copy, and it NEVER mutates the unit body. A unit in N containers = N edges. ASSERT: after 2 drops of the same unit into 2 folders, the same uuid appears in both + the unit body is BYTE-IDENTICAL (0 new stored units).
  - [ ] **(identity/by-reference)** The SAME unit linked in N folders is ONE unit, N EDGES: uuid identical, content stored ONCE, unit body byte-identical across all containers (containment is a many-to-many edge set, identity by reference = R40.81 law).
  - [ ] **(remove/action-bar)** REMOVE (on every file in the action bar) unlinks HERE only: this link is removed, the unit and its OTHER links survive and still resolve elsewhere.
  - [ ] **(delete/dangerous)** DELETE (RED, dangerous) destroys the unit from the index AND scan-unlinks ALL its edges everywhere. ASSERT via the graph-integrity walker: ZERO dangling refs after (no edge/ref anywhere points at the dead uuid). RECOVERABLE: the pre-image is committed to git before the destroy (his data is never unrecoverably lost).
  - [ ] **(delete/safety)** DELETE is visually RED and requires an EXPLICIT confirm (it is destructive on the owner's real data). REMOVE (unlink) is NOT red and needs no confirm.
  - [ ] **(fleet/one-contract)** EVERY folder EVERYWHERE is a drop target via the ONE shared drop contract (R37.20) — NO per-surface variants: room files, /model tree, diagrams, traceability all use the same contract for drop/link/remove/delete.
  - [ ] **(dry/open-closed)** Drop-as-link + remove + delete are registered PER CLASS, so a NEW object kind gains ALL of it with ZERO central-switch edits — both open-set and closed-set lints stay 0 (no if/switch enumerating object kinds for drop/link/remove/delete).
  - [ ] **(containment/edges)** Containment is the many-to-many EDGE set (container children[] refs) — the SINGLE truth. parentFolder (singular) is RETIRED as a containment source; containment DERIVES from edges, never from a singular parentFolder field.
  - [ ] **(move/derived)** MOVE (re-parent, R40.86) is DERIVED = unlink(source) + link(target); it performs NO containment mutation of its own. The R40.86 bespoke reparent body is RETIRED and re-expressed as this composition. ASSERT: 0 parentFolder/containment mutations outside link/unlink; reparentUnitsIntoContainer's body is deleted/reduced to link+unlink.
  - [ ] **(migration/two-differentials)** SAFE TRANSITION, TWO DIFFERENTIALS (architect CORRECTED render-measurement 9a0be1577 — all 30 parentFolder targets are ROOMS, membership markers, not sub-folder pointers): (a) TRANSPARENCY 0-delta on the ~6893 edge-based units AND the 25 redundant-parentFolder units (parentFolder==own room + already in files[] -> render at root CORRECTLY; retire changes NOTHING); any delta on these => RED. (b) FIX differential on the 5 units (parentFolder=room 3231db71 but NOT in its files[] = INVISIBLE): 4 restored to room 3231db71 fileUnits (invisible->visible) + 1 junk (about:blank#blocked) DELETED; per-uuid; a 0-delta on a real invisible OR a change to any of the 25 OR a 6th unit => RED. parentFolder retired only after both pass; git pre-image first. (CORRECTED from the earlier 25+5 assumption — the 25 are NOT a defect.)
  -> folder.dropPlacesLinkRemoveDelete [uc:uuid:4062b06d-2a38-4552-b3a7-5ef9458f50a0]

- [ ] **R40.107 — A WRITE must not mutate fields it was not asked to change (migration, handler, re-persist) — before/after differential asserts ONLY declared targets differ, fail-closed**
  [requirement:uuid:fe7788d7-4bec-4231-bc2a-3dc3542c88a2]
  STANDING LAW: A WRITE (migration, dedup, handler, re-persist — ANY code that persists) MUST NOT MUTATE FIELDS IT WAS NOT ASKED TO CHANGE. CORRECTED live instance (2026-09-06, architect-measured — the earlier attribution to migrate-one-store.ts was WRONG): migrate-one-store.ts is EXONERATED (byte-identical copy, writeFileSync + skip-if-exists, NEVER overwrites — cannot blank a name or reset a timestamp). The REAL chain: Room.persistMembers (Room.ts:401) writes name:m.name, so a member whose PROFILE was DELETED persists with an EMPTY name; plus joinedAt:Date.now() churn + createdAt reconstruction; and the LOAD path additionally DROPS profile-less members. TRIGGER: the PO test-room/profile cleanup ORDER deleted profiles and caused a bystander room to re-persist. The room re-persist was NOT a migration at all, yet mutated fields nobody asked it to -> the law covers ANY WRITE. One step MORE GENERAL than member-name-must-survive; the cure is the PROVEN differential (assert ONLY the intended change), now a precondition on every write.
  **Acceptance criteria:**
  - [ ] **(migration/differential)** A WRITE (migration/dedup/handler/re-persist) asserts a BEFORE/AFTER DIFFERENTIAL where ONLY its DECLARED target fields differ. Any field NOT in the declared scope that changes => RED.
  - [ ] **(migration/identity)** IDENTITY fields (member name, uuid) and PROVENANCE timestamps (joinedAt, createdAt) are BYTE-IDENTICAL through the migration UNLESS explicitly targeted. A member name blanked, a uuid changed, or joinedAt/createdAt rewritten without explicit targeting => RED (the exact live instance: Marcel Donges -> '' + timestamps rewritten on room 3231db71).
