...
## ★★ RETRACTED (Tron no-2nd-store, 2026-09-06): the "MODEL_STORE blob-create path" below ENTRENCHED a second store — a standing violation (my error, PO's build-go compounded). CORRECT: Folder.acceptDrop mints a SCENARIO UNIT IN scenario/index parented by ref — ONE store. MODEL_STORE elimination is its own full migration (measurement-model-store-elimination.md: 784 units live only there; no-data-loss/no-regression; designed FRESH post-cut). The section below is kept only to mark what was wrong.

## ~~BUILD the missing capability: model-store blob-create path~~ (RETRACTED — see banner above)
