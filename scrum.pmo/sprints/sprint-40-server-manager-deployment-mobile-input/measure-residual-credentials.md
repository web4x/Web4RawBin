...
## Room data-consequence of revoking the 79 (PO's pre-action measurement)
Corrected mid-measure: messages are separate units, not inline — re-measured content via Message/File units per room.
- **Revoke triggers NO room mutation** (only the health field references `revoked`; no delete/orphan/cleanup) → room UNITS persist on disk, fully RECOVERABLE. Revoke is a pure IDENTIFY-time rejection.
- **But access is by membership, and all 12 affected rooms have 0 OTHER-live-members** → after bulk-revoke, no one can authenticate into them → they become AUTH-INACCESSIBLE (not destroyed; recoverable by re-owning).
- It is **12 rooms** (the histogram's 6 owner-tokens own 12), **7 hold content = 28 files/webitems** (Royal Jungle 10, md-safari-pwa 8, Project Sanctuary 3, Toronto Trinity 2, Jesus Munich 2, HeartSpace 2, test-merge 1; 0 messages); several named "Marcel Donges" (Tron's).

**Outcome = (b), not (a):** revoke does not destroy the data but DOES auth-orphan the rooms, and 7 hold Tron's files. **RECOMMEND (b): before the bulk-revoke, re-point the 12 rooms' ownership (at minimum the 7 content-bearing + the Marcel-Donges rooms) to Tron's LIVE identity — as his OPAQUE storageId, NEVER a raw token (secret-hygiene / the writer-chokepoint) — then one bulk-revoke of all 79.** Never orphan his data to close the hole. If you'd rather not re-point junk, (c): the 12-room list is his data — ask Tron which to keep, re-point those, revoke the rest. Either way: measure/decide → ONE bulk-revoke + retire-28 + owner-literal-rotation, one restart.
