...
2. **Blast-radius fail-safe** — a shared bootstrap that THROWS would take out ALL surfaces at once (bigger blast radius than the bug). Each page must still boot + render; the bootstrap's transport-open is try/caught so a ws failure DEGRADES that page (no live-update) — it does NOT crash the shell. Assert every page boots even if the transport can't connect.
