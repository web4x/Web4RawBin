...
- **By-construction criterion + gate:** every LIVE surface (any page that renders units expected to update live) MUST open a transport connection (construct RawBinClient / the live-bridge + connect). A live page with NO ws = dead-by-construction. **Gate:** grep-lint — a page-entry that mounts live unit-views without constructing the transport bridge → RED; plus the page-client acceptance above. This makes a socket-less live page IMPOSSIBLE, not merely fixed for these 3 (family-not-instance — a NEW live page can't ship dead).
