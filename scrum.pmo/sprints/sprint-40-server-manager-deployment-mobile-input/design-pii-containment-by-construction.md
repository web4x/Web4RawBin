...
- **★ R40.22 (credential/identity decoupling — Tron has NOT approved it) IS the mechanism that makes rotation safe:** a stable user-id + a rotatable credential with **dual-validity** so Tron cannot be locked out mid-rotation. **This exposure is the live justification for that design** — the token leak cannot be safely remediated without it. Say so to Tron plainly. (Ties R40.21/R40.22.)
...
## History question — for TRON's decision, not ours (ask #3) — NOW GATED BEHIND ROTATION
**★ Reordered by the credential finding:** the FIRST remediation is **ROTATION** of the 240 `ownerToken`s server-side (see the CREDENTIAL section) — that is what actually neutralizes the leak, because history/private cannot reach already-cloned tokens. Rotation needs R40.22 (credential/identity decoupling, dual-validity) — the live justification for approving it. AFTER rotation, the leaked tokens are worthless and the history question reduces to ORDINARY PII:
