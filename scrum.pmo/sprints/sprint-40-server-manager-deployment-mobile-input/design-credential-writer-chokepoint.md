...
1. **Enumerated credential field-set** (the inventory above): the guard scans ONLY `ownerToken`/`creatorToken`/`uploaderToken`/`token`/`allowedUsers[]` — never the ref fields — so legit uuid refs are never touched (no false-positive).
   - ★ **ALSO PATH STRINGS (expert catch, B1 co-execution):** tokens hide in PATH STRINGS too — the 84 `unitLinks` embed the token as a path SEGMENT (`data/users/<token>/…`), not a field. So the guard must ALSO reject a credential-shaped value inside any `unitLinks` entry / path-valued field, not just the scalar credential fields. Rule: a bare-uuid appearing as a path segment in `unitLinks` (or any `*Path`/link field) → refuse, same fail-closed. The safe form there is the `sid:`/storageId segment. Field-only scanning would miss the exact vector that produced 167 of the leaked path-embeds.
