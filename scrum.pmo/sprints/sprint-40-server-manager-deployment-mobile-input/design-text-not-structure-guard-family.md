...
5. **★ guard-BLIND-BY-INHERITANCE (2026-08-30, PO: "the best specimen — GREEN the whole time it was blind")** — the RbDetailBase hazard-gate ("0 detail elements implement their own funnel outside the primitive") scans RbDetailBase SUBCLASSES; `RbFileDetail`/`RbWebItemDetail` `extend HTMLElement`, so they were **invisible to it BY CONSTRUCTION** — the gate read GREEN with two live violators (own funnel + own /api/ior fetch + own thin-fail) on both /trace and in-room, the entire time. Root: the guard keyed its SCOPE to the ACTORS' membership (inheritance) instead of the HAZARD's presence — evadable by NOT inheriting, exactly as the name-keyed lint (#4) was evadable by renaming. **"A guard keyed to inheritance is a guard keyed to the actors."** This is the family's purest exemplar: a guard giving FALSE confidence while structurally unable to see its own escapees. CURE: scan the HAZARD SHAPE (detail-ish + own fetch/funnel + NOT routed through the primitive → RED, inheritance-AGNOSTIC), self-biting — see design-dry-file-detail-convergence.md (19c340d4f), Phase A makes the gate's "0" true-by-migration not true-by-blindness.
