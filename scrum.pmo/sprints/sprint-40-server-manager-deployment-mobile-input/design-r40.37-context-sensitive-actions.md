...
## ★ AC5 FINAL (Tron answered 2026-08-12): 'physical' = a REAL PERSISTED unit, NOT a filesystem dir
Tron's ruling: Add-folder mints a REAL PERSISTED Folder scenario-unit (vs a synthetic/virtual node); the itemview becomes it IMMEDIATELY; **NO mkdir**. So `FolderService.createPhysicalWithUnit` = **mint + persist + return** (write the Folder unit to the store, return it, itemview renders in one step). The 'atomic mkdir + rmdir-rollback fail-closed-BOTH-ways' clause **DISSOLVES** — there is no directory to orphan. What SURVIVES as the fail-closed rule: **unit-write-fails → nothing changes, NO phantom itemview** (never render a folder whose unit didn't persist). req renames the AC away from 'PHYSICAL-FOLDER' (reads as filesystem). Expert was RIGHT to refuse to guess a path — there was no path. (Measured basis: createFolder 28000b00 = unit-only into data/model-store; Folder fields uuid/name/parent/children, no physical-path.)

## (SUPERSEDED by AC5 FINAL — kept for history) model folders are VIRTUAL store-units — no filesystem dir
