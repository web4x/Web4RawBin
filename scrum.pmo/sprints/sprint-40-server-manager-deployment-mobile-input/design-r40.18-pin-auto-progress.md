...
- **★ OBSERVABLE stale-steer transition (PO addition, R-C6 principle — never silent). MECHANISM (impl-shape, resolves the tester's BITE-6b "where"):** emit it as a **LOG-ONLY, EVENT-DRIVEN** line — at the R40.10 QA-transition, IF the transitioning task IS the current explicit steer, `addLog("explicit current-task steer for <task> expired (reached <status>) → auto-progress resumed")`. Two constraints that keep it clean: (i) **event-driven, ONCE** — fired at the transition, NOT inside the pure derivation (a derive-time log would SPAM on every render while the steer is stale, and would fight idempotency BITE-1); (ii) **LOG-ONLY, never a pin WRITE** — it observes, it does not mutate the pin (a pin-write here would be the two-source hook we banned; the pin stays derived). Optional secondary: a DERIVED one-line UI notice on the @390 pin view while the stale-steer condition holds (pure, no side-effect). **The addLog line is the deterministic assertable for BITE-6b.** Likewise the fail-loud-UNRESOLVED path states WHY (named reason), not merely that it didn't.
