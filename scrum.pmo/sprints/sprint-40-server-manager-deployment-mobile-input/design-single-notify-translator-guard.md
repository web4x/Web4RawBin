...
PO-queued after 0.8.178. Converts the R40.84-B 4-round hunt into a defect that **cannot recur via idiomatic drift** (the way it actually bit us). Scenario-first: req mints the req+UC; I design the guard; tester gates + self-stubs; trainer canonizes. Same shape as the R31.7 no-flush guard + my children-single-owner guard (`9ef91a551`) / R40.88.

> **HARDENED (R40.91 tester c2/c3 + PO ruling, 2026-09-05):** the guard was NOT "unevadable" — the tester proved two evasions. FIXED: the frame-check now resolves the hazard however the type-check is idiomatically SPELLED — string literal, a local `const = 'unit-changed'`, or a message-type constant (`MSG.UNIT_CHANGED`, the codebase's own idiom = the most likely next-translator spelling); and an inline emit via `ViewBus.notify(` OR a local alias of it. The self-bite carries the c2 (alias) + c3 (MSG-const) + c3b (local-const) fixtures PERMANENTLY. **ACCEPTED RESIDUAL (surfaced, never silent):** deliberate obfuscation — computed member access `ViewBus['notify']`, notify passed as a higher-order callback, or the type-string assembled at runtime — is NOT caught (a sabotage threat model, not idiomatic drift). The false "unevadable" claim was DELETED from the guard header + the live-bridge marker (a false property-claim manufactures confidence that stops the next reader looking).
