...
| **room membership** | Room | **~58 non-empty** (61 total; my "13" was WRONG) | **identity fields INSIDE each `members[]` element** (name/token/phone) — NOT a top-level Room.senderName (that field is absent; my grep matched senderName inside members[]) | who was in which room |
★ **COUNT NUMBERS ARE MEASURED FRESH, NOT HARDCODED (expert caught my stale/mis-classified literals):** my 20/107/13 was a RELAYED origin/main FIELD-KEY-presence measure + a Room mis-classification (senderName-anywhere matched inside members[]). The count-assert population = a FRESH field-shape inventory across `git rev-list --all` (all history, both branches), counting **NON-EMPTY PII-value** units per shape (an empty `phones:[]` is a no-op, not counted), Room member-aware. Message==107 held; Profile/Room re-measured. The assert compares THREE numbers by the SAME method: pre-inventory N == redacted-count == (post-scrub 0). Re-derive at point-of-use — the design literal was a stale relayed measurement (the #7/#8 lesson, on the author).
...
- **`git filter-repo --blob-callback`** — NOT `--replace-text` value-regex. Why: a Message `text` body is free-form and has NO distinguishing value-shape (can't regex "a chat body" vs "an AC description"). So the redactor must be UNIT-AWARE: parse each `scenario/index/**/*.scenario.json` blob, and redact BY FIELD NAME within the PII-bearing shapes:
  - `Profile` → redact `phones`/`emails`/`addresses` + **`name`** (real names) + `companies`-if-raw-value → `[REDACTED]`
  - `Message` → redact `senderName`/`text`(body) + **`name`** (a label EMBEDDING senderName+text = full content, expert-found, design missed) → `[REDACTED]`. KEEP timestamp/kind/refs.
  - `Room` → redact **`chatHistory[].senderName`+`.text`** (EMBEDDED CHAT BODIES — chat content lives in Rooms too, expert-found) + `members[].name` + **`name`** (unclear=real: a room name may be a person) → `[REDACTED]`. KEEP chatHistory[].senderId-if-ref, members[].ior/joinedAt/role/status.
  - **KEEP any ref-shaped value (`ior:`/uuid): senderIor/roomIor/members[].ior/chatHistory.senderId/company-ref — R37.29 needs them.** REDACT only raw HUMAN values. Discriminate by `ior:class` + field-shape → the ~293 legit `text` units (AC/checklist) are UNTOUCHED. Keep uuids + all structural/ref fields → refs resolve → NO dangling cascade.
  - **★★ FIELD-LEVEL FAIL-CLOSED (meta-cure for the partial list the expert caught — a hand-list under-covers, the recurring lesson):** the callback classifies EVERY field of a PII-bearing unit as KEEP (ref/structural/timestamp) or REDACT (human value); an UNCLASSIFIED field → **HALT** (same as halt-on-unknown-ior). This makes the field-list DISCOVERED-COMPLETE by construction — no field silently passes unclassified, so a missed-PII-field cannot recur. Field lists are built per-ior from the EXPERT'S measured shapes, not my design literals.
- **★ SECRETS (a SECOND class, expert-flagged):** Room `ownerToken`(61)/`roomKey`(3) are exposed on PUBLIC github → REDACT their values IN THIS PASS (one force-push). `sshPublicKey` = KEEP (a public key is public by design). ★★ **redaction ≠ ROTATION — flag to PO/Tron:** a leaked LIVE token/roomKey stays COMPROMISED after a history-scrub (it was public); they MUST be ROTATED — a security incident-response decision, SEPARATE from + after the scrub. (Architect flags the need; does not implement the rotation.)
...
- **FIELD-SHAPE pre-commit + ci:gate (all shapes) — INHERITS the field-level fail-closed (PO: mechanism of record):** the guard classifies EVERY field of a PII-bearing unit-type (Profile/Message/Room) as keep (ref/structural) or PII-value; a PII-value present with a raw human value = RED; **an UNCLASSIFIED field on a PII-bearing type = RED** (halt-on-unclassified, same as the scrub callback) → discovered-complete, so a NEW PII field-shape added later cannot slip in unclassified (the hand-list-under-covers failure caught 3× today cannot recur). field-shape NOT className, prose-aware. Stub-must-fail (proven able to RED): stage a Message with a `text` body → RED; add an unclassified field to a Profile → RED. A PII value can never be committed again.
