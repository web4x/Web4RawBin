...
**Net model (three transports, one internal representation):** (1) native-file INGRESS edge = multipart, owned by `NativeFileIngress`, converts to unit immediately; (2) in-app drag buffer = unit JSON (T37.20); (3) REST / federation = unit JSON. Internally: unit JSON only, always. Self-heal-by-construction + binary-in-unit (REVISION 2) unchanged. PRIO 1 remains Slice-1 OOP + R40.81; this natural-class/ingress design sits behind it.
