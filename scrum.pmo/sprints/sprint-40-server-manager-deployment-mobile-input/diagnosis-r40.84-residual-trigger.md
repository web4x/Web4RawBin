...
3. **Commit the dist ATOMICALLY** with the version bump (served==committed==source, R31.7). Revert the 3 temp probes in the SAME commit. ★ **DEPLOY HOLD (PO 2026-09-05): build + gate ONLY — prod deploy WAITS for Tron's explicit deploy-go (build-go ≠ deploy-go).** Boot-check applies at deploy time, not now.
