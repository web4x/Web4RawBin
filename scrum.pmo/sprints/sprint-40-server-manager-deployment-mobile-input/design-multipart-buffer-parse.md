...
> ★★★ NARROWED BY TESTER MEASUREMENT (2026-09-06, r4090 node-native raw multipart, real 473521b PNG, HEAD 45f8ad5b1; RED baseline c9c6acb8a). The BODY parser is EXONERATED: unquoted file-FIRST AND token-first BINARY both parse to the exact 473521b — so it is NOT binary/order-dependence in the body (my file-first-binary theory was WRONG; tester measured it, I accept). The SOLE root is the HEADER-side boundary extraction (server.ts:2643): `split('boundary=')[1]` KEEPS a quoted `boundary="----X"` or a trailing `;charset` param → the `--boundary` split never matches → no parts → token MISSING → 401 → "Upload failed". iOS Safari quotes the boundary = the iOS-specific + worked-in-0.6.x fit. ★ THE FIX = the boundary-extraction hardening ONLY (see ## ADDENDUM: dequote + strip params + trim). The buffer-native BODY rewrite below is DE-SCOPED from the outage fix — the body parse is proven fine; do NOT rewrite a working parser mid-outage (the original HOLD's own warning). Keep the body rewrite on file as a SEPARATE optional by-construction improvement, shipped later on its own merit, NOT as Tron's fix. r4090 flips all-GREEN on the boundary hardening alone.
