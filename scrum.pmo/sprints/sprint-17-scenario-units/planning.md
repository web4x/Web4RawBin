...
- [ ] ✅ [T179: SW reliability — auto-activation + STATIC_SHELL build-manifest derivation](./task-179-sw-auto-activation-stale-cache-fix.md)
  **Status:** ✅ impl-shipped — expert `80ed9911` v0.5.78 (planner-first inverted but converged). sw.js: SKIP_WAITING postMessage handler (L27-29) + activate-time old-cache purge (L46-47) + `clients.claim` (L50). build.mjs auto-injects ALL hashed bundle names into STATIC_SHELL — killed the `cache.add('/dist/app.js').catch(() => {})` 404-cache bug. Rule-pair (a)+(b)+(c) ✓; 836/836. Tester SW-ACTIVE verify per strict-bar (2b) pending: AC11-13 + Tron-repro returns ZERO 404s for app.js with SW active. ⚠️ **One-time transition caveat:** Tron's pre-v0.5.78 SW lacks SKIP_WAITING handler → ONE LAST manual clear required to get onto v0.5.78; thereafter all updates auto-activate.
