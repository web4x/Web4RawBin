...
- [ ] 📝 [T172: Strict-direction audit + massive orphan fix (R-H) + R-H.2 atomic-req-split + R-J test-reachability](./task-172-strict-direction-audit-massive-orphans.md)
  **Status:** REFINEMENT DONE (architect concurrent file reconciled per learning #12; planner adopted architect's content + fixed uuid + added Subtasks/QA Audit; deleted planner scaffold). Architect diagnosis: direction is CLEAN; real issue is **239/296 orphans (81%)** — forward-walk reachability fails at the FIRST hop (Requirement→Task: only 2/100 Tasks linked). T171's "50" was wrong metric. Fix: 5-step forward-ref population (req→task, task→uc, uc→class, class→method, method→impl→test) + Sprint allowlist + strict-direction enhancement. **R-H.2 atomic-split + R-J per-test reachability** folded.
