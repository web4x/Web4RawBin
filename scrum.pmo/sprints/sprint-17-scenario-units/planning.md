...
- [ ] ⏳ [T151: Migrate MD traceability bullets → JSON model arrays (no info loss)](./task-151-md-traceability-to-json-arrays-migration.md)
  **Status:** PLANNED — BIG diligent task. Migrate every MD `## Traceability` chain bullet (`up`, `down`, `follows`, `chain.requirement / use case / puml / class/method`, `changes`) into the scenario JSON `model.links.*` / `model.chain.*` arrays. **Per-task before/after item count audit is mandatory loss-detection evidence** (MD-bullets BEFORE == JSON-entries AFTER, exactly, per task). Closes the #19 loop (JSON canonical, MD generated by T126).
