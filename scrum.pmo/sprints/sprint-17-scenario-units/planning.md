...
- [ ] 📝 [T184: Forward-only API emit — strip backward keys at server (R-U umbrella with T181)](./task-184-forward-only-api-emit.md)
  **Status:** 📝 refinement done — architect `e05ddd6f` shipped per-file fix table + FORWARD_KEYS-at-emit design (extract FORWARD_KEYS as module-level export from TraceModel.ts; server.ts:481 wraps `graph.toJSON()` with `forwardOnlyGraph()` filter; internal TraceGraph stays bidirectional for `parent` getter; T181 client filter stays as defense-in-depth). Awaiting expert impl. **Reconcile log (learning #20):** my placeholder scaffold (R-X1→Y1→Z1 rename chain) DELETED; adopted architect's filename + content; replaced architect's fake-suffix uuid with my real v4; adopted architect's R-U umbrella mapping (same requirement as T181 — Tron's no-backward-links principle, different fix layer).
