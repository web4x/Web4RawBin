...
- [ ] 🧪 [T128: Migrate all existing sprints/tasks/requirements to scenario-unit model](./task-128-migration.md)
  ...
  **Sub-tasks:**
  ...
  - **T128.4 ⏳ formal stand-up (PO 2026-06-04)** — method markers retrofit; [task file](./task-128.4-method-marker-retrofit.md). Baseline 0/44 7-hop reachable after T178 pipeline; **current snapshot 36/44** (heuristic + in-flight T128.4 UC creation lifted reach). 50 impl markers + 21 test markers missing in src/+test/. **IN FLIGHT (architect+req JOINT):** creating **6 new UseCase units for S14/S15 classes** (prerequisite — those Classes lacked UC parents, breaking chain at UC hop). 4-role: architect+req UC creation → mapping table → expert markers → tester **44/44** target. task uuid `8b405c16-…`; req-uuid R-V `5e6c2810-…`. Rule-pair likely EXEMPT (source-comment-only). UNBLOCKS T178 closure + T124/T168 final close + strict-bar per-Test CI gate.
...
- [ ] 🧪 [T168: Chain order 7-step + atomic requirements as tree ROOTS](./task-168-chain-order-7-step-requirements-as-roots.md)
  **Status:** 🧪 PO-closed 2026-06-05 — `c28c982` architect design (7-step canonical chain LOCKED + requirements as roots) + DATA-side satisfied via T178 KEYSTONE `452f8d5d` 44/44 7-hop reachability. PO: "Close T178 + the dependent T124/T168 (deep-chain was their blocker)." Tron QA pending. Done-gate checkboxes untouched per learning #9. **Append-only rework note 2026-06-05 (C3 from architect contradiction review `d7d6404a`):** body NOT edited (Rule 8 closure freeze). T168 AC2 "tree ROOTS = requirements" remains authoritative for CHAIN semantics. R18.8 (Tron 2026-06-05) added a NAVIGATION layer ABOVE the chain (Sprint→Task→coveredReqs→chain) without changing chain direction or chain-roots. Reworked AC2 wording (CHAIN root vs NAVIGATION root) carried forward to T187 in S18, which formally owns the navigation-root rework + /api/trace/sprints endpoint (C7).
...
### Phase 30 — Tron R-M (drawer UX cleanup + /scenario route + mobile width-cap)
...
### Phase 32 — Tron R-O (test-infra unblocker — headless module-load)
...
- [ ] 🧪 [T178: 7-step chain DATA-FILL + lazy-load-deeper-fails (KEYSTONE)](./task-178-7-step-chain-data-fill-44-tests.md)
  **Status:** 🧪 KEYSTONE CLOSED + tester-verified 2026-06-05 — `c6c695a4` pipeline + `cc152130`/`194d747c` heuristic + `f306e503` v0.5.82 overlay-read fix + `a7c2b6b7` S17 UC→Task 24 links + `7b7859ac` 8 UC classes[] fill + **`452f8d5d` KEYSTONE: 44/44 tests 7-hop reachable from Requirement roots (PASS); R-J + R-E satisfied in DATA**. 836/836 pass. Pre-S16 UC gap resolved Path A (architect created 13 UCs for S1-S14 tasks with tests). PO 2026-06-05: "T178 KEYSTONE CLOSED + tester-verified, close T178 + dependent T124/T168." Tron QA pending. Done-gate checkboxes untouched per learning #9. **Post-closure tree-UI iteration moved to its OWN task — T186** (PO 2026-06-05: "do NOT fold into closed T178. Distinct layer, distinct fix — own task = clean traceability"). T178 scope is the DATA-side KEYSTONE; T186 owns the seed-mode TREE-UI fix (v0.5.84).
...
- [ ] 🔧 [T180: Real CA cert (Let's Encrypt) for home.donges.it + CDP Playwright workaround (CRITICAL-PATH #1)](./task-180-real-ca-cert-letsencrypt-cdp-workaround.md)
  **Status:** 🔧 partial (Track 1 PRE-STAGED, awaiting Tron DNS) — **Track 2 SHIPPED** `9c32626b` (CDP cert bypass; SW E2E proof: 4 tests verify SW registers + activates + caches 11 STATIC_SHELL on rawbin-v0.5.84). **Track 1 PRE-STAGE SHIPPED** `bb828692` v0.5.87 (Let's Encrypt cert auto-detect + self-signed fallback; server.ts ready to pick up real cert when Tron places it). Rule-pair (a)+(b) ✓ both ships. **Track 1 final step: Tron DNS-01 certbot for `home.donges.it`** — server will auto-detect and switch on next start; SW registration unblocked on real device. TOP PRIORITY remains until Tron places the LE cert.
...
### Phase 42 — Tron R-Y (TREE-UI lazy-load at every depth)

PO direction 2026-06-05: stand up T186 RETROACTIVELY for the v0.5.84 tree-lazy-load fix. Do NOT fold into closed T178 — distinct layer (T178 = deep-chain DATA / 44/44 KEYSTONE; T186 = seed-mode TREE-UI bug). Own task = clean traceability — closed tasks don't gain post-closure commits. T186 marked ✅ impl-shipped (expert `69c3ef83` v0.5.84) + tester-verified (live 7-level expand).

- [ ] ✅ [T186: Tree-view lazy-load at every chain level — seed-mode TREE-UI fix (R-Y1 + R-Y2)](./task-186-tree-lazy-load-seed-mode.md)
  **Status:** ✅ impl-shipped + tester-verified 2026-06-05 — req-eng `58acb8e4` captured R-Y1+R-Y2 (planner replaced fake-suffix uuids with real v4); architect `c0ddf1af` diagnosed 3 bugs in seed-mode `buildSeedNode` (2a grandchildren fetch / 2b has-children flag / 2c toggle-children handler); expert `69c3ef83` v0.5.84 shipped fix. Rule-pair (a)+(b) ✓ (package.json + sw.js); (c) EXEMPT (existing route/bundles; expert-declared). Tester live-verified 7-level expand per PO confirmation when directing this stand-up. Tron QA pending.
  **Owners (CMM4):** robbin-req `58acb8e4` (R-Y1+R-Y2 capture) → robbin-architect `c0ddf1af` (3-bug diagnosis) → robbin-expert `69c3ef83` (v0.5.84 fix) → robbin-tester (live 7-level expand verified)
  **Rule-pair scope:** (a)+(b) required & shipped; (c) EXEMPT (existing route + existing bundles; no new HTML shell).
  **v4 uuids:** task `daf76389-…`; req-uuids R-Y1 `fbbbf9cb-…`, R-Y2 `75db0534-…` (planner replaced req's fake-suffix `a7b8c9d0-…-y00000000001` / `…-y00000000002`).
...
- [ ] 🧪 [T185: PlantUML class diagrams for traceability-tree + scenario-instance architecture (R-X1 + R-X2)](./task-185-traceability-architecture-puml.md)
  **Status:** 🧪 tester-verified strict 2026-06-05 — architect `c11f723a` shipped `diagrams/s17-architecture.puml` 311 lines + rendered SVG; refined via `f103929f` (align 30/38 PUML `method:uuid` to exact index UUIDs), `71a600be` (8 Method units with real v4 + fix PUML fake-suffix), `cc1851f9` (link TraceObject + RbTraceTree to UC parents — 8 methods now 7-hop reachable). PO 2026-06-05 confirmed tester-verified strict. Rule-pair EXEMPT (documentation-only). Tron QA pending.
...
### Phase 40 — Tron R-U umbrella (forward-only end-to-end; defense-in-depth)

PO direction 2026-06-05: T181 ✓ closed (display + tree filter forward-only); tester flagged RESIDUAL at /api/trace graph — 42 Tasks still carry backward `requirements` keys at the server emit layer. Display masks them, but the data isn't forward-only end-to-end. T184 strips them at the API emit layer (server) so `/api/trace` never sends backward keys to the client. LOW priority (display already clean) but completes Tron's no-back-chaos principle. 4-role planner-first. **Same R-U umbrella as T181** (architect-mapped — Tron's no-backward-links is one requirement at multiple fix layers; T181 client + T184 server = defense-in-depth). Planner R-X1→R-Y1→R-Z1 rename chain abandoned in the reconcile.

- [ ] 🧪 [T184: Forward-only API emit — strip backward keys at server (R-U umbrella with T181)](./task-184-forward-only-api-emit.md)
  **Status:** 🧪 tester-verified strict 2026-06-05 — architect `e05ddd6f` design + expert `1e210b9d` v0.5.85 shipped (FORWARD_KEYS exported from TraceModel.ts; server.ts:481 wraps `graph.toJSON()` with `forwardOnlyGraph()` filter; internal graph bidirectional; T181 client filter retained as defense-in-depth). Rule-pair (a) v0.5.85 ✓ + (b) rawbin-v0.5.85 ✓; (c) EXEMPT (server-only; expert-declared). 836/836 pass. PO 2026-06-05 confirmed tester-verified strict. Tron QA pending.
  **Owners (CMM4):** robbin-req (R-U anchor — same as T181) → robbin-architect `e05ddd6f` (FORWARD_KEYS-at-emit design) → robbin-expert (impl; rule-pair (a)+(b); (c) EXEMPT per architect — server-only) → robbin-tester (verify /api/trace zero backward keys; defense-in-depth intact with T181)
  **Rule-pair scope:** (a)+(b) required; **(c) EXEMPT** (architect-declared; server-only — no client bundle change since T181 filter stays).
  **v4 uuids:** task `ab0cbe5f-…`; req-uuid R-U `82638acb-…` (umbrella with T181).
...
- [ ] 🧪 [T179: SW reliability — auto-activation + STATIC_SHELL build-manifest derivation](./task-179-sw-auto-activation-stale-cache-fix.md)
  **Status:** 🧪 strict-bar (2b) tester-verified 2026-06-05 — `80ed9911` v0.5.78 → `886f9815` v0.5.79 HOTFIX (clients.claim REMOVED to avoid race with old-cache deletion) → **`5be49819` v0.5.86 AC11-13 SW-active E2E verification suite** (4/4 SW E2E + 836/836 vitest pass; Tron 404-repro returns 200 with SW active). Rule-pair (a)+(b) ✓ on each ship. AC2 superseded by passive activation (new SW takes over on next navigation; achieves "Tron never manually clears" with one extra page-load latency). ⚠️ One-time transition caveat: Tron's pre-v0.5.78 SW lacks SKIP_WAITING handler → ONE LAST manual clear required to get onto v0.5.78+; thereafter all updates auto-activate passively. Tron QA pending.
