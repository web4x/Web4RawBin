...
- [ ] 🔧 [T180: Real CA cert (Let's Encrypt) for home.donges.it + CDP Playwright workaround (CRITICAL-PATH #1)](./task-180-real-ca-cert-letsencrypt-cdp-workaround.md)
  **Status:** 🔧 partial — **Track 2 SHIPPED** `9c32626b` (CDP `Security.setIgnoreCertificateErrors` in `playwright.config.ts` + `test/e2e/fixtures.ts`; `module-exec-proof.spec.ts` 4 tests prove SW registers + activates + caches 11 STATIC_SHELL entries on rawbin-v0.5.84 over self-signed HTTPS). Track 2 unblocks T179 AC11-13 SW-active headless verify. **Track 1 (LE cert) still AWAITING TRON ACTION (PO 2026-06-04)** — DNS-01 certbot needs Tron DNS API access / host shell. TOP PRIORITY remains until Track 1 lands (Tron real-device unlock).
...
- [ ] ✅ [T179: SW reliability — auto-activation + STATIC_SHELL build-manifest derivation](./task-179-sw-auto-activation-stale-cache-fix.md)
  **Status:** ✅ impl-shipped — `80ed9911` v0.5.78 → **`886f9815` v0.5.79 HOTFIX**: `clients.claim` REMOVED (v0.5.78 trial showed combining claim with old-cache deletion races offline-page when new SW claims tabs before new cache populates). Current code: SKIP_WAITING postMessage handler (L27) + old-cache purge (L43) + build.mjs auto-injected STATIC_SHELL. **AC2 SUPERSEDED by passive activation** — new SW takes over on next navigation, not instantly. Achieves "Tron never manually clears" with one extra page-load latency. Rule-pair (a)+(b) ✓ on hotfix; 836/836. Tester SW-ACTIVE verify per strict-bar (2b) pending: AC11-13 (AC13 revised to "on next navigation") + AC10 Tron-repro returns ZERO 404s with SW active. ⚠️ **One-time transition caveat:** Tron's pre-v0.5.78 SW lacks SKIP_WAITING handler → ONE LAST manual clear required to get onto v0.5.78+; thereafter all updates auto-activate passively.
