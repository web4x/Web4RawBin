...
- 2026-06-03: Expert `6ee2278f` v0.5.73 ships R-M3d + R-M3e bundled per PO direction — drawer-nav scrollIntoView({behavior:smooth, block:center}) + auto-select seeded node (R-M3d); /scenario wires TraceRouter with full graph for collapse/expand/DetailView/icon-tap parity with /trace (R-M3e). Rule-pair (a)+(b)+(c) ✓ — STATIC_SHELL scenario-view-GJ6VCIZI.js; 834/834. Expert self-declared compliance.
- 2026-06-03: Architect `86b024ae` post-v0.5.73 diagnosis — **2 more bugs in same R-M3d/R-M3e atoms** (deeper iteration, no new atoms): (a) **R-M3d timing race**: setTimeout(100) races async renderSeed → navigate fires before tree DOM ready. Fix: emit `seed-ready` event; navigate AFTER renderSeed completes (deterministic). (b) **R-M3e all-children-visible**: renderSeed shows all 51 children (kids.style.display=''), but Tron spec says "ONLY that item view THEN lazy-load children". Fix: start collapsed (display:none); store pending children; populate on first expand; cascading buildChildNode() fetches /api/trace/children/<uuid> per deeper expand.
- Pending: expert fix-2 commit for the timing + collapsed-start lazy-load (rule-pair (a)+(b); (c) likely exempt — fixes existing surface). Tester one-pass re-verify R-M1..R-M4 + R-M3d + R-M3e ACs once both fixes land. Then Tron QA.
