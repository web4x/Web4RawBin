...
# T184: TraceConsistency .md-parser forward-only — strip backward keys at source so /api/trace is forward-only end-to-end (R-X1)
...
- up
  ...
  - **R-X1** `[requirement:uuid:d9c419b3-8b36-4b8b-821b-b1171da62ab7]` — TraceConsistency .md-parser must emit forward-only links; /api/trace graph forward-only end-to-end (Tron's no-back-chaos principle at source, not just display).
...
- **robbin-req** — anchor R-X1 verbatim if Tron-relayed (current capture is tester-finding-plus-PO-direction; may not need a separate Tron quote)
...
**R-X1 — parser forward-only at emit:**
