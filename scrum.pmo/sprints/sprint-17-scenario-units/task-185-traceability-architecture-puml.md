...
> **PO direction 2026-06-05:** Stand up T185 RETROACTIVELY — architect already
> shipped `c11f723a` (s17-architecture.puml + rendered SVG, 313 lines, 0 source
> change). The PUML is the deliverable. Mark code-complete (✅); tester verifies
> the PUML renders + class UUIDs resolve.
...
## Deliverable (shipped — architect `c11f723a` 2026-06-05)

| File | Lines | Purpose |
|------|-------|---------|
| `scrum.pmo/sprints/sprint-17-scenario-units/diagrams/s17-architecture.puml` | 311 | Source diagram — TraceObject base + LOCKED 7-step typed chain + view components + scenario infrastructure |
| `scrum.pmo/sprints/sprint-17-scenario-units/diagrams/s17-architecture.svg` |   2 | Rendered (PlantUML output; opens in browser/markdown viewers) |

**Coverage** (per architect's commit message):
- **TraceObject base** — `parent` / `children` T175 getters
- **LOCKED 7-step typed chain** — Requirement → Task → UC → Class → Method → Impl → Test
- **View components** — RbTraceTree / RbObjectItem / RbDetailDrawer + 3 DetailViews
- **Scenario infrastructure** — ScenarioUnit / ScenarioIndex / IORResolver / ClassRegistry / ViewTemplateRegistry / SpeakingTree / TraceConsistency
- **Annotations** — every Class + Method element carries `[class:uuid]` / `[method:uuid]` from the scenario index → feeds the UC→Class→Method chain that T178 KEYSTONE relied on for 44/44 reachability.

## Owners (CMM4 — 4-role per learning #18)
- **robbin-req** — `15dd69c1` captured R-X1 + R-X2 (verbatim Tron-implied requirement for first-class PUML class diagrams documenting both architecture surfaces); planner replaced fake-suffix uuids per learning #17
- **robbin-architect** — `c11f723a` shipped both diagrams in one file (311 lines + SVG) — code-complete deliverable
- **robbin-expert** — not required (architect ships PUML directly; no source change)
- **robbin-tester** — verify PUML file parses + SVG renders; every `[class:uuid]` / `[method:uuid]` annotation resolves to an actual scenario unit (trace-cli or grep cross-check)
