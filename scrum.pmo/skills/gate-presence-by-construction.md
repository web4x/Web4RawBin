...
- **Frozen FLOOR (manifest may not SHRINK) — authorization is SEMANTIC, not a self-set flag (PO loophole-closure):** the manifest is compared to a committed floor; a REMOVED gate is RED. ★ A shrink is valid ONLY with a committed justification that **NAMES a SUCCESSOR gate**, and `check:gate-presence` **VERIFIES** the successor: (a) it EXISTS in the manifest, and (b) it covers the removed gate's PROPERTY (e.g., `detectStatusWrites` supersedes `detectDoneWrites`). Authorization is therefore NOT a flag anyone sets ("authorized by me" is a self-serve loophole that reintroduces the exact class — whoever deleted a gate could mark their own shrink authorized) — it is the VERIFIED EXISTENCE of a real successor that subsumes the property.
  - **No self-authorization, no silent authorization, and "temporarily disabled" is NOT a category** — a removal with no successor = RED (standing rule: never remove a gate to green CI; a FAILING gate is the gate WORKING — you fix the code, not delete the gate).
  - **MECHANICAL subsumption (how "covers the property" is CHECKED, not asserted):** the `removed[]` entry carries the removed gate's own stub-must-fail defect; `check:gate-presence` FEEDS that defect to the named SUCCESSOR and the successor MUST RED on it. Existence of the successor is necessary but NOT sufficient (naming an unrelated live gate is a fake successor) — subsumption is proven only when the successor actually CATCHES the removed gate's defect (e.g. `detectStatusWrites` trips on `detectDoneWrites`'s `status='Done'` stub). A successor that doesn't trip → RED.
  - **stub-must-fail on the authorization itself:** mark a shrink authorized WITHOUT a justification, OR name a successor that does NOT exist, OR name one that exists but does NOT trip on the removed gate's defect → `check:gate-presence` MUST RED. The justification's absence or falsity is itself a caught defect.
  - Mirrors `check-controller-dominance`'s "allowlist may not GROW" bite (inverted to "manifest may not SHRINK"), but hardened so "authorized" cannot be self-served.
