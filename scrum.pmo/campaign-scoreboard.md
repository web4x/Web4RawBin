...
- **★★ SIGNABLE = 34 of the 38 at QA-Review** — carry BOTH numbers to Tron (signable ≠ at-QA-Review). **4 HELD-FROM-BATCH (visibly NOT Tron-signable until cleared; status NOT downgraded per PO — cause may be a stale gate not a broken feature):**
  - **R37.3** (T37.3 `364785b1`, Test caf74333) · **R37.7** (T37.7 `bb31965b`) · **R40.8** (T40.8 `b0be0668`, Test c4a7f1b9) = **status:pass over a currently-RED gate DET-3x** = HOLLOW-ROW class; tester diagnosing cause per-row (real-regression / gate-drift-vs-current / dirty-tree-HEAD-read artifact).
  - **R40.10** (T40.10 `9a70ce5e`) = **chain-incomplete-no-Test**.
  - _Hollow-row class now ≥3 instances (r311/R37.3/R40.8) → by-construction candidate (like R37.15/16): a status:pass with no green gate should be structurally impossible or loudly flagged. Capture once causes known._
  ...
  - _★★ CANONICAL COUNTING CONVENTION (so the number stops shifting between agents): **BELOW-QA (units-status Planned/In-Progress, minus superseded-terminal) is the denominator** — as of HEAD 018f3ffb7 = **7 = 3 ACTIONABLE + 2 DEFERRED + 2 EXCLUDED**. ACTIONABLE (the campaign number Tron cares about) = T37.4.1 `236918e9` / T37.4.3 `1b8ebc9a` / T40.11 `6e3cc1b2` (all expert-held). DEFERRED = T30.44 `06623fea` / T-R31.14 `03f5d536`. EXCLUDED = T31.6 `6be9a92d` (concept) / T37.4 `79fd2164` (coordination rollup — completes via subtasks; a "6" count drops it). ⚠ **T37.4.3 shows RIPE = SHARED-CHAIN FALSE-POSITIVE** (shares R37.11 with T37.4.2, whose Test a7f3c1e8 passed); its OWN dominance-lint facet is RED (expert-blocked) — NOT flip-ready off the shared Test (verify-owner-first). Script chain-walk credits shared-chain Tests; that is a known limitation, treated per-task by verify-owner-first._
