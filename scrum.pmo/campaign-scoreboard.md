...
- **TOTAL S30++ tasks: 151** — Done **97** · QA-Review **42** · SUPERSEDED-terminal **3** · **REMAINING (<QA-Review): 9** (151 = 97+42+3+9)
- **★ ACTIONABLE (the campaign number Tron cares about — REMAINING minus law#103 excluded/deferred): 5**
...
- ★ **DEPLOY:** prod serves **v0.8.96**. **inc-3 ON HOLD** — boot-check RED, root-caused = a boot-path SYNC loop over ALL ~5677 units (O(all-units) = a latent scaling outage), NOT deployed. **v0.8.97 build label-stashed** for protection. (Deploy = expert/architect lane; tracked here for board reality.)
