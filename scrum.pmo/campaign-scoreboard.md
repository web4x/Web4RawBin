...
2. **T40.6** `95d74272` (S40, **BUILD-COUPLED on T40.11 — NOT a fast marker-flip**) — its markerPending Impl `e009ace7` credits `DeploymentModel.buildTypedModel`, which has **0 hits in src** (expert-measured, my grep confirmed). Placing the marker would be a **FICTIONAL marker** (AST-rejected — the lying-marker class this campaign eliminated). `buildTypedModel` IS T40.11's deliverable → T40.6's flip **RIDES T40.11's build**.
3. **T40.11** `6e3cc1b2` (S40, build) — **★ LARGE / SPLIT CANDIDATE:** deploymentRefs as scenario-first units + generic M2-type default view + fail-loud (the deep depref migration, UC 249fdab6 absent = unbuilt). **★★ DESIGN CONSTRAINT (measured coupling, architect gets this): T40.11's FIRST SLICE must contain `buildTypedModel`** — that one slice closes T40.11's first increment AND unblocks T40.6 = **two campaign tasks from one slice**, boundary chosen by measured coupling, not preference.
