...
| **T37.20.4** | **Drag an in-app item onto a folder → it moves inside; a native file still nests.** (The row your iPhone caught — now fixed.) | Machine-verified v0.8.207, BOTH halves: client fires reparentUnitsIntoContainer (stub-must-fail GREEN) + server resolves the same `folder:` target and re-parents (`(root)` → `…/MoveTarget/`). Composition of two measured halves meeting at the same interface (the prefix mismatch that failed at 00:23 is closed + measured both sides). Follow-up (non-blocking): one end-to-end run once a webkit tree-render flake is handled. |
