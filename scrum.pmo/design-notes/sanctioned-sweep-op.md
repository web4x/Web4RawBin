...
6. **SET chain-of-custody — MANDATORY (PO adopted into the authorization protocol):** the op computes `setSha = sha256(sorted uuidList)` and PRINTS it PROMINENTLY in BOTH dry-run and run. **The PO's written authorization NAMES a `setSha`, not a description** ("I authorize setSha=<hash>" — a cryptographic bind between the reviewed set and the executed set; "the 34-uuid set" is a sentence anyone re-interprets against a drifted list). The op REQUIRES an authorized `setSha` argument and REFUSES to run if it is missing/empty OR ≠ the list's computed sha — **an empty/missing setSha must NEVER default to proceed** (an authorization that can be omitted is not an authorization). This is the set-level analogue of binding the 7b signature to the patched-file blobs, not to HEAD — same insight one layer up.
