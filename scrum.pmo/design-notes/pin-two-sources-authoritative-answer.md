...
- **R-C5 COLLISION NAMED (PO to rule):** assign-as-current sets DESIGNATION only — NO status write. Making a task actually in-progress (its sprint → Active) is a CHECKLIST edit by the owner (R-C5 derived-status single source), NEVER a pin-button side-effect — that side-effect is exactly what produced the 17 bare unverdicted Dones. Recommend: button = designation; reactivation = deliberate checklist edit.
