...
## ★ GUARD #5 vs INTENDED DELETE — RULING (PO question, before INC-7 build): DECLARED-FOOTPRINT, guard-DERIVED (not a caller flag)
The tension: guard #5 refuses a drop; delete drops on purpose. NEITHER (a) bite-on-legit-delete NOR (b) exempt-delete (guard blind where destruction happens = worst). The third thing:
**Guard #5 is a FOOTPRINT guard: it verifies ACTUAL-DIFF ⊆ the footprint DERIVED from the operation's declared target — refusing any change BEYOND it.** Not a blanket "no drops"; not a "this is a delete" exemption.
- **delete(U):** declared target = uuid U. The guard **DERIVES** the legitimate footprint ITSELF = `{remove U}` ∪ `{unlink every edge referencing U}` (the same refs-to-U the delete scan finds). It then checks the ACTUAL diff == that footprint EXACTLY. A delete that removes U + unlinks U's refs → PASSES. A delete that ALSO blanks/drops/changes ANY bystander (a member name, another unit, a non-U edge) → the bystander is BEYOND the derived footprint → **BITES**. So the guard stays LIVE precisely during the most destructive op.
- **save(unit, fields):** footprint = the declared field-changes of that unit; any bystander change (another unit, an undeclared field, a member-name-blank, a no-redirect member-drop) → BITES. (This is the original #5, generalized.)

**STRUCTURAL, not a phrase (the key requirement):** the footprint is **DERIVED BY THE GUARD from the declared TARGET**, NOT a caller-supplied "exempt"/"allowDrop" flag. A caller declares only WHAT (the target uuid); the guard INDEPENDENTLY computes the ALLOWED effect (U + its refs) and enforces actual⊆allowed. A caller cannot widen its footprint by asserting — there is no assertable exemption to claim. (An exemption a caller can claim by asserting = the hole reopening — rejected.)

**Pattern (book):** Design by Contract (Meyer) — the operation's declared footprint IS its postcondition; the guard enforces it. The footprint = a capability SCOPED to the target, where the guard computes the scope (unforgeable), not the caller.

**Tester BITE (mandatory — a guard that can't fail during the op it guards is not a guard):** run a delete engineered to ALSO drop a BYSTANDER member (beyond U) → assert it is REFUSED. Stub-must-fail: remove the guard → the bystander-dropping delete SUCCEEDS → gate RED. This proves #5 bites DURING a delete, not just during a save.
