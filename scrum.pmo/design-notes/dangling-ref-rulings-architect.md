...
- (a) **The guard must reject UNSAFE PREFIX-EXPANSION, not just non-v4/all-zero.** `resolvePrefix` must be **FAIL-CLOSED**: resolve ONLY to a UNIQUE full-uuid match; on non-unique OR no-match → REFUSE (never `|| keep-prefix`, never first-match, never zero-fill/synthesize). Fold into the R37.3 guard family (post-GO, R37.3 is src/ts/frozen) with a prefix-expansion BITE + the fabricated-uuid BITE.
...
**BITE (stub-must-fail):** feed the exact 3 heads → MUST be rejected via the set-delta test; PLUS a **positive control** — 100+ real `crypto.randomUUID()` → ALL pass (0 false-positive). The positive control is mandatory: an over-aggressive entropy test that rejects real uuids is worse than none ([[false-low-worse-than-absent]]) and would get disabled. Fold into the R37.3 fail-closed guard family (post-GO, src/ts) beside the prefix-expansion + non-v4/all-zero guards.
...
- (2) **Delete** the 2 all-zero Req.tests[] (live false-credit, FIRST) + the fabricated impl-links; **repoint-or-delete** the fabricated ownerIors. ROOT = truncation→expander (CORRECTED): make `resolvePrefix` fail-closed (unique-or-refuse) + prevention batch (E) + prefix-expansion BITE, all post-GO in R37.3.
