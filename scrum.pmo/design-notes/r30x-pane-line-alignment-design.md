...
## ★ v2 FAST-FOLLOW — MINT-READY SPEC (PO-confirmed 2026-07-14, schedule after v1 lands)
Full IntelliJ 3-way: show non-conflicting one-sided changes as BLUE blocks + ribbons (not just red conflicts). **Bounded impl-edit, NO new Class/Method** — RbDiffEditor stays 23m.
- **Mechanism:** `computeMergedCenter` impl-edit (Method a0b30550, marker STAYS): for each diff3 `ok` region, compare its content to the corresponding BASE slice — if **different** (a one-sided change diff3 auto-applied), push a `Conflict{ a:baseOrUnchangedSide, b:changedSide, pick:'<changed side>', kind:'change', span }` into `conflicts[]`/`centerSeq` instead of an `ok`-run; if **same** (truly stable), keep the `ok`-run. Auto-pick keeps the MERGE RESULT identical (change stays applied) — this only ADDS visibility + a take-over arrow to revert.
- **Free downstream:** `renderCenterChangeBlocks` + `renderConnectorRibbons` already iterate `conflicts[]` → the new kind=change hunks get blue blocks + ribbons automatically; `acceptChange` lets you flip; `jumpToChange` counter now counts them (N changes ⊇ M conflicts).
- **Chain to mint (when scheduled):** UC `diffEditor.threeWayChangeCoverage` → existing Method `RbDiffEditor.computeMergedCenter` (a0b30550) → impl-edit (marker stays); OR fold as an AC on a computeMergedCenter refinement req. Architect derive-confirms the impl-edit reuse (no new units). crossRef R30.16.
