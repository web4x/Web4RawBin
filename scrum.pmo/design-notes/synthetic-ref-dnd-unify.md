...
- ★ **POST-R40.81 FLAG:** `ensureViewUnit` comments say it lazy-mints to **MODEL_STORE** (INV-A2-3), but R40.81 DELETED model-store (reads repointed to scenario/index). Expert must verify ensureViewUnit's write target post-R40.81 (target scenario/index, or compute-ephemeral) — this intersects the A-vs-B shape decision and must not write to a deleted store.
