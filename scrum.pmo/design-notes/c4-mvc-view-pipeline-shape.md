...
**★ REVISION TOKEN DECISION (architect, 2026-08-11): the `revision` = a COMPUTE-ON-READ CONTENT-HASH at `/api/ior`, REUSING the federation `contentHash` (single-source — do NOT fork a 2nd hash).** NOT a monotonic counter: a stored bump would live at the `put()` chokepoint (off-limits + UNIT2-guarded) and couple currency to the guarded write; and a counter is a second source of truth about "did this change" that can desync (bump-without-change / in-memory reset on restart). The content-hash IS the content identity → currency-proof by construction, and inherently FAIL-CLOSED vs a missed event (hardening A): currency is verified by re-hashing + comparing (`rendered.hash != current.hash → stale`), never by "no event == fresh". Ordering (newer/older) is NOT needed — revalidate-or-stale is change-detection (equality), not ordering. `UNIT_CHANGED` payload = `{ior, uuid, hash}`; the client compares its rendered hash to the broadcast/refetched hash. CACHE the hash per `(uuid, version)` to bound compute cost. — `event.revision` (the freshness key):
...
R37.4 → NEW UCs per boundary (req mints scenario-first, distinct-intent, NO cross-wire): `taskController.statusNext` → Class `TaskController` (task-fsm.ts) → Method `statusNext` → Impl; `taskController.notifyTransition` → Method `notifyTransition` → Impl; `viewPipeline.revalidateOrMarkStale` → Class (client model) → Method → Impl; `mvcBoundary.assertControllerDominates` → Class `MvcBoundaryGuard` (new lint file) → Method → Impl. Single-source Done = a DELEGATION edit to R40.10 approve (no new Done Impl). **(C) shared evidence predicate** = NEW UC `evidence.evidenceForStep` → Class `StepEvidence` (shared module) → Method `evidenceForStep` → Impl, CONSUMED by BOTH statusNext AND checklist-chain-audit (the audit's existing chain-edge computation refactors to CALL this one predicate — do NOT leave a second copy; retire the inline one). This is the single-source unit that keeps the two evidence-askers from disagreeing. Each rides its C4.x container's ACs. I confirm uuids before expert wires; backstop @390 for the client stale-badge + flip Impls.

## ★ REPRESENTATION FORK + THE RETIRED HAZARD (resolved 2026-08-11; expert found, PO ruled, architect reduce/retire call)
There were TWO status representations for the SAME model: (LIVE) `model.status` = 4-state `deriveStatusEnum(statusChecklist)` — the canonical NESTED template `Planned → In Progress[refinement/testcases/impl/testing] → QA Review → Done` — which `approveByOwner` gates on (server.ts:1478 `m.status==='QA Review' → 'Done'`) and the board/pin read; and (FSM) `task-fsm.ts`'s FLAT 7-state (`Planned/Refining/CreatingTestCases/Implementing/Testing/QAReview/Done`) writing `model.status` directly. This is one model in two representations — the flat-7-state is the FORK (the four In-Progress sub-steps flattened up to top-level peers).

**Resolution:** `statusNext` drives the NESTED canonical (tick checklist → `deriveStatusEnum` → 4-state) = the ONE representation. **RETIRE** the flat-state STATUS WRITES; **REDUCE/KEEP** the FSM's `TRANSITIONS`+`guardTransition` LEGALITY, re-expressed over `(top-level, sub-step)` pairs, as `TaskPolicy`'s internal step-legality consumed by `unitController.apply`. One representation, one legality set; `task-fsm.ts` reduces to an internal legality module, not a status writer.

**★ THE RETIREMENT REMOVES A LATENT HAZARD, not just a vocabulary (record it as such):** the flat state is `'QAReview'`; approve's live gate matches `'QA Review'` (with a space). **They are not equal.** Had anyone ever wired the FSM `start*` writes to live `model.status`, approve's :1478 gate would have silently stopped matching — Tron's approve button would flip NOTHING, no error, discovered by him. That mine sat in the tree one wiring decision from detonating; it surfaced only because the expert refused to guess the fork and the strings were measured. Retiring the flat-state writes = defusing it. **Same hazard-class as the byte-exact-header and the audit's inline evidence copy: a second source of one fact, silently divergent.** Do not re-introduce a status-string writer outside `deriveStatusEnum`. **GUARD (PO-approved, folds INTO C4.8 `mvcBoundary.assertControllerDominates` — NOT a separate unit; one property, one home):** the dominance lint asserts BOTH facets — (i) no unit write bypasses `unitController.apply`, and (ii) `deriveStatusEnum` is the SOLE writer of the 4-state `model.status` strings; a literal `'QA Review'`/`'Done'` status-set anywhere else → RED. **Family: `status-string-writer`.** Stub-must-fail bite: plant a literal `model.status = 'QA Review'` → suite RED (+ lint-runs meta-bite). Same dominance shape as INV-D1's exactly-one-success-exit and R40.19's HN4 — the lint forbids the path that does not exist yet, which here is the one that would have silently broken Tron's approve.
