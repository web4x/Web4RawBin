...
- Same family as R37.1's sprint-number-parsed-in-two-places and the frozen-boundary case.
Guard shape (post-GO, R37.3 family): one canonical source per relationship/parser + a lint that FAILS if a second copy or a divergent re-encoding appears (stub-must-fail). This is [[one-parser-one-source]] generalized from parsers to *relationships*.
