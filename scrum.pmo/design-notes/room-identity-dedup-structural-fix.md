...
**Author:** robbin-architect · 2026-07-01. Trigger: Heartspaces shows 3 Marcel Donges (2703628c tombstoned, 37fcb752 tombstone-lost, 8f74dfba primary) + SystemTester = 4 members. Measured against src/ts/server/Room.ts + server.ts at HEAD.
...
3. **`redirectTo` is not immutable / not restart-durable.** 37fcb752 lost its `redirectTo` — a reconnect or IDENTIFY of the tombstoned token re-saved the profile without it. A tombstone must be permanent.
...
Heartspaces + profiles.json are already corrupted: (1) re-tombstone 37fcb752 → `redirectTo = 8f74dfba` (the lost tombstone); confirm 2703628c → 8f74dfba; (2) collapse the 3 Marcel member entries in the Heartspaces room to the single primary 8f74dfba. Dry-run + count first (never silently drop a real member).
