...
- **ACCEPTED-DEFERRED DEBT — R40.11 IOR-construction class-widen (funnel all `ior:instance:` through one ctor)** *(planner, deferred 2026-08-29; PO-directed via architect; design 042ffaabd + text-not-structure family 067ec1270)*
  **SIZE ~40-site refactor:** funnel ALL `ior:instance:` construction through a single `types.ts` `iorInstance()` constructor + a lint FORBIDDING raw `ior:instance:${...}` outside the funnel (by-construction scan-the-hazard: the dangerous OPERATION is raw IOR string-building; one funnel makes it unevadable). **DATE deferred 2026-08-29.** **WHY:** the expert MEASURED the grep-widen and it OVER-FIRES ~40 legit mint sites — flipping the lint to strict now would be an ignored/bypassed gate = worse than none (our own R40.11 lesson, twice). **STATUS = ACCEPTED-not-unknown** (sized, dated, visible). **NOTE:** the R40.11 INSTANCE fix SHIPS NOW (fail-loud, verified PASS); ONLY the class-widen (the ~40-site funnel + strict lint) is the owed item — do it as one calm pass when the funnel + per-site migration can land together, then flip the lint strict.
