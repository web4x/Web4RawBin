...
- **NAMED-DEBT — 219 legacy Done tasks carry no approvedBy/provenance FIELD (pre-field backfill, LOW)** *(robbin-planner, 2026-09-12; PO-endorsed provenance-aware ruling)*
  Status-audit (from disk) found **219 of 225 Done tasks lack `approvedBy`/`approvalProvenance`** — but **ALL 219 are LEGACY (S1–S36), ZERO are current-era (S37/S40)**. This means the `0-Done-till-Tron` discipline actually HELD in the current era; the 219 are **backfill debt** (the provenance field postdates them / Tron accepted them before it existed), **NOT false-Dones**. **DO NOT reopen them** — reopening 219 real Dones on a missing FIELD would be the lie-of-aggregation in reverse ([[done-carries-provenance]] is provenance-AWARE, not shape-mechanical). Fix pattern = a one-time provenance backfill (stamp the historical approvedAt/by from git history or a legacy-era marker), never a status flip. NOT now (LOW; no current-era exposure). Pattern: [[closure-taxonomy-subsume-retire-obsolescence]] (a missing field is not a false state).
  **Touches:** the 219 legacy Done task units (add approvedBy/approvalProvenance from the historical record), a backfill script.

- **NAMED-DEBT — ~147 hardcoded absolute host paths under test/+scripts (post-move dead-guard hygiene)** *(robbin-expert, 2026-08-30; PO scope-proportionately ruling)*
  The R37.26 dead-guard sweep repointed the 22 INERT visual GATES (the coverage) to repo-relative `__repo` (commit re-committed 22-explicit). The prevention lint `scripts/check-no-hardcoded-repo-path.mjs` (correct-by-construction, bans the absolute-host-path HAZARD pattern not the '2cuGitHub' string) found **173 sites total → 147 remain** after the 22 gates. These ~147 are NON-gate hygiene (other scripts + test helpers) — a hardcoded absolute pins to one machine/location → breaks on the next move. **PO: leave to the lint + a later sweep, do NOT boil the 147 now.** Lint is committed but NOT wired to ci:gates (would day-one fleet-RED on the 147 = right-requirement-wrong-rollout); wire it AFTER the 147 sweep. Fix pattern = derive from `fileURLToPath(import.meta.url)` / `process.cwd()` / env resolved once. Pattern: [[scan-the-hazard-not-the-actors]] (the lint found 8× what the '2cuGitHub' grep did).
  **Touches:** ~147 files under test/+scripts (repoint), then wire `check-no-hardcoded-repo-path.mjs` into ci:gates.

- **NAMED-DEBT — R37.27 /app frozen-OPEN WS zombie (iOS liveness-probe)** *(robbin-expert + architect, 2026-08-30; capture #1 owed)*
  R37.27 fact-1 fix covers Tron's /trace pin FULLY (live-bridge resync = state-independent HTTP refetch, catches a frozen-but-OPEN socket). The /app RawBinClient path handles the COMMON iOS case (socket reads CLOSED post-freeze → reconnect → server snapshot) but does NOT force-refetch a frozen-but-OPEN socket (resync no-ops when readyState lies OPEN; a risky re-IDENTIFY was correctly NOT added). **Architect-recommended safe fix = a LIVENESS PROBE** (lightweight staleness check on visible → reconnect if stale), NOT unconditional re-join. Deferred — /app is not Tron's reported surface. Pattern: [[correct-by-construction]].
  **Touches:** `src/public/ts/RawBinClient.ts` resync (add liveness-probe), `transport-lifecycle.ts` (optional shared probe helper).

- **NAMED-DEBT — atomic-put hardening (index-store.ts:57 non-atomic writeFileSync)** *(architect-flagged via req, 2026-08-31; DISTINCT from the dangling-ref / referential-integrity work R37.29 — do NOT conflate)*
  `src/ts/scenario/index-store.ts:57` writes a unit via a non-atomic `writeFileSync` — a crash or concurrent write mid-write can leave a TRUNCATED / half-written unit file on disk (a different failure class from a missing/never-carried unit). Harden = write to a temp file + **rename** (atomic on POSIX) + **shape-validate** the JSON before the rename, so a partial write can never replace a good unit. Non-blocking. Pattern: [[correct-by-construction]].
  **Touches:** `src/ts/scenario/index-store.ts:57` (writeFileSync → temp-write + shape-validate + atomic rename).
...
- **NAMED-DEBT — 9 hand-authored legacy requirements.md (S21-29) → generated views (R37.7 C7 migration)** *(planner, 2026-08-12; PO fix-(a) + req content-ruling)*
  S21-29 requirements.md are HAND-AUTHORED legacy (headerless, no `GENERATED_HEADER`, Tron-era commit 169da7451) — the generator's write-guard PRESERVES them (never writes). `check:sprint-md` was drift-flagging them (RED, blocking ci:gates fleet-wide) = a check-vs-write SCOPE inconsistency. **FIXED (a), the ci-unblock:** `driftScope` in generate-sprint-md.ts excludes non-`GENERATED_HEADER` files from the drift-check (single-source: same `GENERATED_HEADER_PREFIX` predicate the write-guard uses, via `onDiskGenerated` — NOT a sprint list) + `--bite` proves a GENERATED requirements.md with planted drift still goes RED (not blinded). req CONFIRMED (measured all 9) the files are AUTHORITATIVE AS-IS — keep hand-authored, do NOT blind-regen (units may not capture the content = data-loss). **THE DEBT (b), deferred:** migrate the 9 to generated views (add `GENERATED_HEADER`, generator takes over → back in the drift-check) — but ONLY under R37.7, Tron-bounded, **per-sprint units-completeness-proof FIRST** (else the migration LOSES hand-authored content the units don't capture; same [[generate-sprint-md-deletes-pumls]] / C7 data-loss hazard). req will not confirm units-complete without measuring each sprint. NOT now (REMAINING=4).
  **Touches:** each S21-29 requirements.md (add header only after units-completeness-proof), the sprint's Requirement units (must fully capture content first).

- **NAMED-DEBT — 10 legacy T-slug tasks have NO statusChecklist (frozen-legacy, UNVERIFIABLE)** *(planner, 2026-08-12; PO fix-(a) frozen-legacy exclusion)*
  10 old UNLINKED T-slug tasks (T176/177/179/182, T-seed-ior-room, T-file-detail-preview, T-item-init-gate, T-longpress-collapse, T-tree-room-mode, T-url-preview-restore) predate the statusChecklist schema: `status='Planned'` with NO `statusChecklist`. `assertStatusConsistent` (R37.5) categorizes them **UNVERIFIABLE** — the status-vs-checklist rule cannot apply where there is no checklist. `consistency:strict` was FAILING on them (fleet-blocking). **FIXED (a), the exclusion:** `isBlockingStatusOffender` (task-status.ts) = `kind !== 'UNVERIFIABLE'` (single-source: the derived absence-of-checklist category, NOT a hardcoded slug list) — UNVERIFIABLE is report-only, still LISTED (never silently clean, INV-C3-2). `--bite` proves a checklist-HAVING drift still blocks (non-blinding). **THE DEBT (b):** give each legacy task a real `statusChecklist` reflecting its ACTUAL state — but that is EVIDENCE its owner must author (NOT invent: authoring a checklist for a task whose state is unknown = the same data-invention trap as the requirements.md "regen"). Route per-task to its owner/req when the legacy sprints (S16-S19-era) are triaged. NOT now.
  **Touches:** the 10 task units (add statusChecklist = owner authors from real state), the S16-S19-era legacy sprints.
