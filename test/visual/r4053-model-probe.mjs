...
try {
  ...
  await ctx.addInitScript((t) => { try { localStorage.setItem('rawbin-player-id', t); } catch {} }, f.ownerHeaders()['x-player-token']);
  ...
  console.log(`POPULATED (drawer open on task): parentLinks×${before.parentLinks} Marcel×${before.marcel} checklists×${before.checklists} statusLabels×${before.statusLabels} len=${before.len} dataGap=${before.dataGap}`);
