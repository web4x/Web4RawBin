...
async function tap(browser, selected, expectUuid, shouldAdd) {
  ...
  await page.evaluate((ref) => { const d = document.createElement('rb-diagram-detail'); d.id = 'tapdg'; d.setAttribute('ref', ref); document.body.appendChild(d); }, `diagram:${FAA}`);
...
const results = [];
try {
  const ver = (await version()) === '0.8.14';
  for (let i = 1; i <= 3; i++) {
    ...
    results.push(pass);
    if (i === 1) console.log(`  ★ ROOT CAUSE: the tap DOES fire addView (POST body=${t.post}, server status=${t.status}) — but addView sends diagramUuid WITH the 'diagram:' prefix (stripRef strips only ior:instance:/modelelement:, NOT diagram:) → server 400 bad-uuid → NO box. FIX: addView strip 'diagram:' from the ref.`);
    console.log(`iter ${i}: v0.8.14=${ver} SINGLE-tap→box-appears=${tapAdded}(${before}→${t.views.length}, POST=${t.status}) BITE multi-guarded=${biteMulti} non-element-guarded=${biteNonEl} => ${pass ? 'GREEN' : 'RED (bug: tap fires but box never appears)'}`);
  }
} finally {
  await browser.close();
  fs.writeFileSync(FAA_FILE, BASELINE);
