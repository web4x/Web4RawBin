...
try {
  ...
  console.log(`designated 40.1 → slots.current=${await resolvedCurrent8()}`);
  ...
  try {
    ...
    await sleep(400);
    // OPEN the drawer on 40.1 (already current) → connectedCallback fetch races the onDetailShown show-render
    await page.evaluate((u) => document.dispatchEvent(new CustomEvent('selection-changed', { detail: { selected: ['task:' + u] } })), TASK);
    await sleep(3500); // let all renders + the :103 re-derive settle
